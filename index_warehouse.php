...
class soaXMLRPC{
    static function onGetWebServices(){
     ...
     // 报损当面
        ...
        $services['soa.getwarningInformation'] = array(
            'function' => 'soaFunctions::getwarningInformation',
            'docstring'=> 'getwarningInformation($data)',
            'signature'=> array(
                array(
                    $xmlrpcString, //Result
                    $xmlrpcString, //Request JSON data
...
// 获取整单退货订单信息
 ...
 // 缺货单个确认
    ...
    function  getwarningInformation($data, $origin_id, $key ){
        global $warehouse;
        if ( !soaHelper::auth($origin_id, $key) ){
            return 'ERROR, NO AUTHORIZED.';
        }
        ...
        return $warehouse->getwarningInformation(json_decode($data, true));
