...
LEFT JOIN oc_order AS o ON o.order_id = xsm.order_id
LEFT JOIN oc_order_product AS op ON op.order_id = xsm.order_id
AND op.product_id = smi.product_id
LEFT JOIN oc_product AS p ON p.product_id = smi.product_id
WHERE
	o.deliver_date = '" . $date . "'
AND op.weight_inv_flag = 1
GROUP BY
	smi.product_id";
        ...
        <table>
            ...
            <?php foreach($result as $key=>$value){?>
                <tr>
                    ...
                    <td><?php echo $value['sum_weight'];?></td>
                    <td><?php echo sprintf("%.2f", $value['average_weight']);?></td>
