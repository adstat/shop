...
<div align="center" style="display:block; margin:0.3rem auto" id="logo"><img src="view/image/logo.png" style="width:6em"/> 订单分拣<button class="invopt style_gray" id="reload" style="display: inline" onclick="reloadPage();" disabled="disabled">载入中...</button></div>
...
<div id="content" style="display: block">
    <div align="center" id="orderListTable" style="margin:0.5em auto; font-size: 0.8rem">
        <div style="margin:0.3rem; padding:0.3rem; color:#5e5e5e; font-style: italic; background-color: #d6d6d6; border: 1px dashed #5e5e5e;">
            <button class="invopt getOrdersFilter style_gray" id="getOrders_5" style="display: inline; margin: 0.1rem" onclick="getOrders(5);">分拣中</button>
            <button class="invopt getOrdersFilter style_gray" id="getOrders_2" style="display: inline; margin: 0.1rem" onclick="getOrders(2);">待分拣</button>
            ...
            <?php if($_COOKIE['warehouse_repack'] == 1 ){?>
                <button class="invopt getOrdersFilter style_gray" id="getOrders_50" style="display: inline; margin: 0.1rem" onclick="getOrders(50);">整件分拣</button>
                <button class="invopt getOrdersFilter style_gray" id="getOrders_51" style="display: inline; margin: 0.1rem" onclick="getOrders(51);">散件分拣</button>
            <?php } ?>
        </div>
        ...
        <table border="0" style="width:100%;" cellpadding="2" cellspacing="3" id="ordersHold">
            ...
            <tr>
                ...
                <th>商品数</th>
                <th >未分拣</th>
                <th id = "inventory_name_one" style="display:none">整件分拣人员</th>
                <th id = "inventory_name_two" style="display:none">散件分拣人员</th>
                <th id = "inventory_name_zero">分拣人</th>
                <th>订单状态</th>
                <th>操作</th>
            </tr>
            </thead>
        ...
        <div id="productList" name="productList" method="POST" style="display: none">
            ...
            <div id="fastmove_order_comment" style="margin: 1rem 0 2.5rem 0; padding: 0.5rem; display: none; background-color: #c9e2b3;">
                ...
                <div style="margin: 0.5rem 0;">
                    ...
                    <?php   if ($_COOKIE['user_repack'] == 2)  { ?>
                        <input type="text" style="font-size: 1.5rem; width:5rem; height:2.8rem; padding: 0.2rem; border: 1px #333333 solid;" id="inv_comment" class="fm_inv_comment"  readonly/>
                        <input type="text" style="font-size: 1.5rem; width:5rem; height:2.8rem; padding: 0.2rem; border: 1px #333333 solid;" id="inv_spare_comment" class="fm_inv_comment" />
                    <?php } else { ?>
            ...
            <div id="fastmove_order_frame" style="margin: 1rem 0 2.5rem 0; display: none; ">
                ...
                <input class="submit" type="button" value="提交" onclick="javascript:addOrderNum();">
...
<script>
    ...
    function getOrders(order_status_id){
        ...
        if(warehouse_repack  == 1 && user_repack == 1 ){
            $("#inventory_name_one").show();
            $("#inventory_name_zero").hide();
        }
        if(warehouse_repack  == 1 && user_repack == 2 ){
            $("#inventory_name_two").show();
            $("#inventory_name_zero").hide();
        }
        //Get RegMethod
        $.ajax({
            ...
            success : function (response , status , xhr){
                ...
                if(response){
                    ...
                    if(jsonData.status == 999){
                        alert(jsonData.msg);
                        location.href = "inventory_login.php?return=w2.php";
                    }
                    ...
                    $.each(jsonData.data, function(index, value){
                        ...
                        if(window.inventory_user_order[value.order_id] || <?php echo !in_array($_COOKIE['user_group_id'], $inventory_user_admin) ? 0 : 1;?> ){
                            ...
                            if(parseInt(value.station_id) == 2 && value.district != ''){
                                html += '<br /><span style="color:red;">' + value.district + ',' + value.area_name + '</span>';
                            }
                            ...
                            if(warehouse_repack == 1 && user_repack == 2 ){
                                ...
                                if(parseInt(value.inv_spare_comment) > 0){

                                    html += '<br><span style="color:darkgreen; font-weight: bold;">[货位' + value.inv_spare_comment + ']</span>';
                                }
                            }
                            ...
                            if(warehouse_repack == 1 && user_repack == 2){
                                html += '<td '+t_status_class+'>'+value.spare_inventory_name+'</td>';
                            }else{
                            ...
                            if(warehouse_repack == 1 && user_repack == 1 ){
                                if(value.whole_name == 'undefined'){

                                }else{
                                    html += '<td '+t_status_class+'>'+value.whole_name+'<input type="hidden" class="soringOrderStatus" value="'+value.order_status_id+'"></td>';
                                }


                            }else if(warehouse_repack == 1 && user_repack == 2){
                                if(value.spare_name == 'undefined'){

                                }else{
                                    html += '<td '+t_status_class+'>'+value.spare_name+'<input type="hidden" class="soringOrderStatus" value="'+value.order_status_id+'"></td>';
                                }

                            }else {
                                html += '<td '+t_status_class+'>'+value.name+'<input type="hidden" class="soringOrderStatus" value="'+value.order_status_id+'"></td>';
                            }
                            ...
                            if(warehouse_repack == 1 && user_repack == 2 ){
                                if(value.order_spare_status_id  == 0 ){
                                    html += '<button id="inventoryIn" class="invopt orderStartSortingButton" style="display: inline" onclick="javascript:orderInventory(' + value.order_id + ',' + value.station_id + ');">开始</button>';
                                }
                            }

                            if(warehouse_repack == 1 && user_repack == 1 ){
                                if(value.order_whole_status_id  == 0 ){
                                    html += '<button id="inventoryIn" class="invopt orderStartSortingButton" style="display: inline" onclick="javascript:orderInventory(' + value.order_id + ',' + value.station_id + ');">开始</button>';
                                }
                            }
                            /*
    ...
    function orderInventory(order_id,station_id){
        ...
        if(warehouse_repack == 1 && user_repack == 2 ){
            if(confirm("确认分拣此订单的散件吗？")) {
                var status = getOrderSpareSortingUser(order_id);
            }else {
                return false ;
            }
...
//                html += '<input class="innerSubmit" style="font-size: 0.9rem;" id="submit" type="button" value="拣" onclick="javascript:autoChooseSortingProduct(\''+thisSku+'\');">';
    ...
    function addOrderProductStation(id){
        ...
        $.ajax({
            ...
            success : function (response, status, xhr){
                console.log("Save Product Soring Info:");
                console.log(response);
                if(response){
                    ...
                    if(jsonData.status == 1){
                        ...
                        //根据分拣中间数据设置整件分拣数量, 同时更新页面显示
                        $("#box_count").val(parseInt(jsonData.boxCount));
                        $("#nonRepackBoxCount").text($("#box_count").val());
                        console.log('Saved Product Sorting Info Change BoxCount to:'+ $("#box_count").val());
                    }
                    else{
                        //var player = $("#player2")[0];
                        //player.play();
                        ...
                        alert("分拣数据缓存有误或该订单已取消，请返回重新进入当前分拣页");
                    }
    ...
    function addOrderProductToInv_pre(userPendingCheck){
        ...
        if(user_repack == 2 ){
            if(!/^[1-9]\d{0,2}$/.test(inv_spare_comment.trim())){
                alert("请输入散件的数字格式货位号");
                hideOverlay();
                return false;
            }
        }else{
            if(!/^[1-9]\d{0,2}$/.test(invComment.trim())){
                alert("请输入数字格式货位号");
                hideOverlay();
                return false;
            }
    ...
    function addOrderProductToInv(userPendingCheck){
        ...
        var inv_spare_comment = parseInt($('#inv_spare_comment').val()); //保存散件货位号
    ...
    function getOrderSpareSortingUser(order_id){
        ...
        $.ajax({
            ...
            url: 'invapi.php',
            data: {
                method: 'getOrderSpareSortingUser',
                data: {
                    warehouse_id: warehouse_id,
                    warehouse_repack: warehouse_repack,
                    user_repack: user_repack,
            ...
            success: function (response) {
                var jsonData = $.parseJSON(response);
                if (jsonData == inventoryuser ) {


                }
                if (jsonData == 2) {
                    insertOrderDistrSpare(order_id);
                }
            }
        });
    ...
    function insertOrderDistrSpare(order_id) {
        var warehouse_repack = '<?php echo $_COOKIE['warehouse_repack'];?>' ;
        var user_repack = '<?php echo $_COOKIE['user_repack'];?>' ;
        var inventoryuser = '<?php echo $_COOKIE['inventory_user'];?>';
        var warehouse_id  =  global.warehouse_id ;
