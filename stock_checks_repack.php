...
<div id="message" class="message style_light" style="">仅用于搬仓托盘打包</div>
<div id="content" style="display: block">
    ...
    <div id="inv_control" align="center">
        仓库区域：
        <select  id="warehouse_section"  style="width:200px ;height: 40px" onchange="showform(this.value);">

        </select>
        <div id="barcodescanner2">
            <form method="get" onsubmit="handleProductList3(); return false;" >
            托盘号： <input name="pallet_number"  id="pallet_number" rows="1" maxlength="10" autocomplete="off" placeholder="托盘号" style="width: 150px;me-mode:disabled; font-size: 1.2rem" /></div>
            </form>
        </div>
        <script type="text/javascript">
            $("input[name='pallet_number']").keyup(function(){
                var tmptxt=$(this).val();
...
<script>
    ...
    $(document).ready(function () {
        $.ajax({
            type: 'POST',
            url: 'invapi.php?method=getWarehouseSection',
            data: {
                method: 'getWarehouseSection',
            },
...
//                      getAddedStcokChecksProduct(pallet_number);
                      ...
                      $.each(jsonData, function(index,value){
                          ...
                          var html = '<tr class="barcodeHolder" id="bd'+ value.product_id +'">' +
                              ...
                              '<td style="width:4em;"><input class="qty" id="'+ value.product_id +'" name="'+ value.product_id +'" value="'+returnQty+'" />'+
                              '<td>' +
                      ...
                      $("#pallet_number").attr("readOnly","true");
...
//            }
    ...
    function  handleProductList3() {
        var pallet_num = $("#pallet_number").val();
    ...
    function qtyminus10(id){
        ...
        if($(prodId).val() > 10 || $("#method").val() == 'inventoryAdjust'){
            $("#Inventory"+id).show();
            var qty = parseInt($(prodId).val()) - 10;
            $(prodId).val(qty);
        }
...
//        $("#productsHold4").hide();
            ...
            $.ajax({
                ...
                complete:function () {
                    $("#pallet_number").val(stock_check_id);
                    $("#pallet_number").attr("readOnly","true");
