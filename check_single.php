...
<div id="message" class="message style_light" style="">可以扫描多个商品，商品所属分类必须正确，否则不能录入托盘中</div>
<div id="content" style="display: block">
    ...
    <table id="productsHold2" border="0" style="width:100%;" cellpadding=2 cellspacing=3>
        <tr>
            ...
            <th style="width:2rem">系统盘点库存</th>
            <th style="width:2rem">盘点数量</th>
...
<script>
                    ...
                    if(jsonData == 1){
                        ...
                        $.each(jsonData, function(index,value){
                            ...
                            var html = '<tr class="barcodeHolder" id="bd'+ value.product_id +'">' +
                                ...
                                '<span id="info'+ value.product_id +'">'+'['+value.product_id+']'+value.name+ '/'+'['+value.section_name+']'+'</span> <br/>' +

                                '<br /><span style="font-size: 0.8rem">[<span id="sku'+ value.product_id +'"></span>]</span>' +
                                '</td>' +
                            ...
                            $("#warehouse_section").val(value.warehouse_section_id);
    ...
    //盘点
    function submitStockChecksProduct(){
                    ...
                    success : function (response , status , xhr){
                        if(response){
                            ...
                            var html = '<tr class="barcodeHolder" id="bd'+ id +'">' +
                                ...
                                '<br/>'+'<span style="background: red">同原料商品</span>:'+'<span id="sku_product'+ id +'"></span>'+
                                '</td>' +

                                '<td>'+
                                '<span id="inventory_move'+ id +'"></span>' +
                                '</td>'+
                                '<td style="width:4em;"><input class="qty" id="'+ id +'" name="'+ id +'" value="1"  />'+
                                '</td>' +
                                '<td>' +
...
//                            }
                            ...
                            if(jsonData['sku_product'] != ''){
                                $("#sku_product"+id).html('['+jsonData['sku_product'].product_id+']'+jsonData['sku_product'].name + '['+ jsonData['sku_product'].stock_area + ']'+ '['+   jsonData['sku_product'].sku_barcode + ']');
                            }else{
                                $("#sku_product"+id).html('无同原料商品');
                            }

                            $("#sku"+id).html(jsonData.sku_barcode);
                            $("#stock_area"+id).html(jsonData.inv_class_sort);
                            $("#boxqty"+id).val(boxSize);
                            $("#inventory_move"+id).html(jsonData.quantity);
            ...
            success : function (response, status, xhr){
                if(response){
                    ...
                    $.each(jsonData, function(index,value){
                        ...
                        var disableDeleteButton = '<br /><input style="width: 2rem" class="addprod style_red" id="submitReturnProduct" type="button" value="删除" onclick="if(confirm(\'删除['+value.pallet_number+']信息？\')){ javascript:deleteStockChecks('+value.stock_check_id+');}"><br />';
            ...
            success : function (response, status, xhr){
                if(response){
                    ...
                    $.each(jsonData, function(index,value){
                        ...
                        html += '<tr class="barcodeHolder" >' +
                            '<td>'+value.product_id+value.name+'['+value.section_name + ']'+ '</td>' +
                            '<td>'+returnQty+'</td>' +
                            '</tr>';
                        $("#warehouse_section").val(value.warehouse_section_id);
                        $("#warehouse_section").attr("disabled","true");
