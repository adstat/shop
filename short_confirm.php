...
<hr>
 ...
 <!--  <button id="single_button" name="single_button" style="width: 100px;height: 20px;background: red" type="button"     onclick="getSingle();">单个商品缺货</button>  -->
   <button id="spare_button" name="single_button" style="width: 100px;height: 20px;background: red" type="button"     onclick="getSpareGoods();">散件商品缺货</button>
   <button id="whole_button" name="whole_button" style="width: 100px;height: 20px;background: red" hidden="hidden" type="button"     onclick="location.reload();">整件商品缺货</button>
...
<hr>
    <div align="center">
        ...
        <h2 id ="order_list2" hidden="hidden">单件缺货列表</h2>
        ...
        <input  id="shortage"   style=" margin-top: 10px; width: 100px;height: 30px;background: red" type="button" onclick="submitReturn();" value="缺货提交" >
        ...
        <input  id="shortage3"  hidden="hidden"  style=" margin-top: 10px; width: 100px;height: 30px;background: red" type="button" onclick="submitReturnSpare();" value="缺货提交">
...
//                    console.log(response.check_status)
//                    if(response.check_status ==1){
//                        alert('此订单已经进行过货位核查');
//                    }
//                }
//            });
                ...
                success: function (response) {
                    ...
                    if (response.status == 2) {
                        ...
                        $.each(response.product[1], function (i, v) {
                            ...
                            html += "</td>";
                            html += "<td id='needsorted" + v.product_id + "' >" + v.out_qty + "</td>";
                            html += "<td style='font-weight:bold' id='unsorted" + v.product_id + "'>" + v.out_qty + "</td>";
            ...
            success: function (response, status, xhr) {
                ...
                if(jsonData.status == 2){
                    ...
                    $.each(jsonData.product,function(i,v){
                        ...
                        html += "<div>"+"<span style='font-weight:bold' >"+ v.sku +"</span>" +  "</div>"
                        html += "</td>";
                        html += "<td id='spareneedsorted"+ v.product_id+"' >"+ v.out_qty + "</td>";
                        html += "<td style='font-weight:bold' id='spareunsorted"+ v.product_id+"'>"+ v.out_qty +  "</td>";
    ...
    function submitReturn(){
        ...
        if(confirm("确认提交么？")) {
            ...
            $.ajax({
                ...
                success: function (response) {
                     alert('缺货数量小于分拣数量的提交成功,缺货数量大于分拣数量的将给出提示,以便确认.');
                    $.each(response,function(i,v){
                        ...
                        $.each(v,function(i,v1){
                            if(v1[0][0]['product']){
                                ...
                                alert("该商品的缺货数量已经大于分拣数量：商品："+v1[0]+":数量:"+v1[1]+"：分拣数量"+v1[2]);
