...
<div align="center" style="display:block; margin:0.5em auto" id="logo"><img src="view/image/logo.png" style="width:6em"/></div>
            ...
            <div style="float: none; clear: both">
                <input class="submit" type="button" value="查找" onclick="javascript:getAddedStcokChecksProduct();">
...
<!--                <input style="float: right" class="input_default" id="searchDate" type="text" value="--><?php //echo date('Y-m-d', time());?><!--">-->
                ...
                <input style="float: right" class="input_default" id="input_vg_frame" name="input_vg_frame"  type="text" value="" placeholder="托盘号">
                <input type="hidden" id="frame_vg_list" class="fm_frame_vg_list" />

                <div  style="float: left " id="vg_list" class="fm_vg_list"></div>
            </div>

            <?php if( $_COOKIE['warehouse_id']   ==  11 ) { ?>
                <div>
                    <input class="submit" type="button" value="出库" onclick="javascript:palletMove();">
                </div>
            <?php } ?>
            <?php if( $_COOKIE['warehouse_id']   ==  14 ) { ?>
                <div>
                    <input class="submit" type="button" value="入库" onclick="javascript:palletMove();">
                </div>
            <?php } ?>
...
<script>
    ...
    function getAddedStcokChecksProduct(){

        var warehouse_id = $("#warehouse_id").text();
        var pallet_number = $("#frame_vg_list").val();
...
//                            var disableReutrnButton = '<br /><input style="width: 2rem" class="addprod style_red" id="submitReturnProduct" type="button" value="调整" onclick="if(confirm(\'调整['+value.product_id+']盘点吗？\')){ javascript:changeCheckSingleProduct('+value.stock_check_id+','+value.sku_barcode+');}"><br />';
//
//                            var confirmReutrnButton = ' <br/><input    style="width: 2rem"  class="addprod style_red" id="submitReturnSingleProduct"   type="button" value="确认" onclick="if(confirm(\'确认['+value.product_id+']盘点吗？\')){javascript:confirmCheckSingleProduct('+value.stock_check_id+');}">';


                        if(global.warehouse_id == 11){


                            if(parseInt(value.status)==0){
                              var  palletnmove = '<br /><span style="font-size: 0.8rem;background: red">[未出库]</span>';

                            }

                            if(parseInt(value.status)==1){
                               var palletnmove = '<br /><span style="font-size: 0.8rem">[已出库]</span>';

                            }
                        }

                        if(global.warehouse_id == 14){
                            if(parseInt(value.status)==1){
                                var  palletnmove = '<br /><span style="font-size: 0.8rem;background: red">[未入库]</span>';

                            }

                            if(parseInt(value.status)==2){
                                var palletnmove = '<br /><span style="font-size: 0.8rem">[已入库]</span>';

                            }
                        }

                        html += '<tr class="barcodeHolder" >' +
                            '<td>'+value.pallet_number+palletnmove+'</td>' +
...
//            }
    ...
    function palletMove() {
        var warehouse_id = $("#warehouse_id").text();
        var pallet_id = $("#frame_vg_list").val();
        $.ajax({
            type: 'POST',
            url: 'invapi.php?method=palletMove',
            data: {
                method: 'palletMove',
                data: {
                    warehouse_id: warehouse_id,
