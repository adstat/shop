...
FROM
	oc_order oo
LEFT JOIN oc_order_product op ON op.order_id = oo.order_id
WHERE
	oo.customer_id = '".$customer_id."'
 AND op.product_id IN('".$product_id."')";
    $list = $db->query($sql);
    $customerData = $list->rows;
}
...
$sql = "select date(ios.uptime) adate, date_format(ios.uptime,'%m%d') sortdate, ios.deliver_order_id from oc_x_inventory_order_sorting ios
...
$sql = "
    select
      ...
      GROUP_CONCAT(DISTINCT doo.deliver_order_id) AS deliverIdList
    ...
    LEFT JOIN oc_x_deliver_order doo ON wrp.relevant_id=doo.relevant_id
...
foreach($wrInRows as $k=>$v){
    ...
    $deliverIdList = explode(',',$v['deliverIdList']);

    $seedRows = array();
    if(is_array($relevantList) && sizeof($relevantList)){
        $sql = "select
               group_concat(distinct if(b.container_id>0,b.container_id,null)) containerList,group_concat( DISTINCT a.deliver_order_id) deliver_order
...
//        var_dump($seedRows);die;
    ...
    $seedData[$k]['deliverData'] = array_diff($deliverIdList, $seedData[$k]['deliver_order']);
...
<!--    <script type="text/javascript" src="static/jquery-2.1.0.min.js"></script>-->
    ...
    <script>
        ...
        // function getData(){
        //     var warehouse_id = $("input[name='']").val();
        //     var gap = $("#gap").val();
        //     var url = 'operate_manager_report.php?warehouse_id='+warehouse_id+'&gap='+gap;
        //
        //     location=url;
...
<hr />
    ...
    <table style="margin-top: 3rem; height: 100px; overflow: hidden;">
        ...
        foreach($briefData as $m){
            ...
            echo '<br />';
            echo '异常分拣单号:'. implode(', ',array_diff(explode(',',$m['deliverIdList']), explode(',',$m['deliverIdList'])));
            echo '<br />';
    ...
    <table style="margin-top: 1.5rem; height: 100px; overflow: hidden;">
        ...
        foreach($seedData as $m){
            ...
            echo '未投篮分拣单号:'. implode(', ',$m['deliverData']);
            echo '</td>';
            echo '</tr>';
        }
        ?>
...
<hr />
    <h3>客户ID </h3><input type="text" style="border: 0.5px solid yellowgreen;border-radius:4px;" name="customer_id" id="beginDate">
    <h3>商品ID </h3><input type="text" style="border: 0.5px solid yellowgreen;border-radius:4px;" name="product_id" id="endDate">
    <table style="margin-top: 1.5rem; height: 100px; overflow: hidden;">
        <caption>》查询商户下的商品</caption>
        <tr>
            <th style="width: 3rem">订单号</th>
            <th style="background-color: #ccffcc">商品ID</th>
            <th>数量</th>
        </tr>
