...
<!--    日期选择-->
    ...
    <!--日期选择-->
<hr />
    <button id="transferBoxes" class="invopt" style="display: block" onclick="javascript:inventoryMethodHandler('transferBoxes');">调拨周转筐</button>
    <button id="transferBoxes" class="invopt" style="display: block" onclick="javascript:inventoryMethodHandler('transferProducts');">调拨商品</button>
</div>
<div id="itemList" name="itemList" method="POST" style="display: none;">
    ...
    <table id="itemHold" border="0" style="width:100%;" cellpadding=2 cellspacing=3>
        ...
        <tr id="product_lists" style="display: none;">
            ...
            <th style="width:2rem">总数量</th>
            <th style="width:2rem">扫描数量</th>
...
<!--    </script>-->
    ...
    <div id="creat_shipping_cost" style="float:left;display: none;" class="col-sm-4"  >
...
function inventoryMethodHandler(method){
    ...
    if(method == 'transferBoxes'){
        ...
        getTransferBoxes(global.warehouseId,selectedToWarehouseId,deliverDate,1);
    } else if (method == 'transferProducts') {
        $('#container_lists').hide();
        $('#product_lists').show();

        $('#message').html('调拨商品需要建议一次全部调拨，最后生成调拨单。');
        $('#transferProduct').val(2); //添加商品调拨

        getTransferBoxes(global.warehouseId,selectedToWarehouseId,deliverDate,2);
    } else {
        $('#getplanned').hide();
    }
}
...
// /*zx
// * 出库*/
// function addOrderProductToInv_pre(relevant_id){
//     showOverlay();
//     var warehouse_id2 = parseInt(global.warehouseId);
//     var date_added = $("#deliverDate").val();
//     var warehouse_id1 =parseInt($('#toWarehouseId').val());
//     var inventory_user =$('#inventory_user').text();
//     var inventory_user_id =parseInt(global.userId,);
//     var relevant_id = [];
//     var container_id = [];
//     var product_ids = [];
//     var quantitys = [];
//     relevant_id.push(relevant_id);
