...
?>
<button class="invopt" id="return_index" style="width:4em;" onclick="javascript:window.location.href='allocation_new_warehouse.php?auth=xsj2015inv&ver=db'">返回</button>
...
<script>
    ...
    function handleProductList2(){
        ...
        $.ajax({
            type: 'POST',
            url: 'invapi.php?method=getContainerDeliver',
            data: {
                method: 'getContainerDeliver',
                data: {
                ...
                if(response) {
                    ...
                    $.each(jsonData[1], function (i, v) {
                        ...
                        if(v.quantity >0 ){
                            ...
                            html += '</td>';
                            html += '<td>';
                            html +='<input class="qtyopt"  id="button2_id'+ v.container_id +'" type="button" value="确认" onclick="javascript:confirmDoRelevantC('+ v.container_id+','+ v.deliver_order_id +', '+v.order_id+'  );" >';
                            html += '</td>';
                            html += '<td>';
                            html +='<input class="qtyopt"  id="button2_id'+ v.container_id +'" type="button" value="删除确认数据" onclick="javascript:updateDoRelevantC('+ v.container_id+','+ v.deliver_order_id +', '+v.order_id+'  );" >';
                            html += '</td>';
                        }else{
                            html += '<td>';
                            html +='<input class="qtyopt"  id="button2_id'+ v.container_id +'" type="button" value="删除确认数据" onclick="javascript:updateDoRelevantC('+ v.container_id+','+ v.deliver_order_id +', '+v.order_id+'  );" >';
                    ...
                    if (jsonData ) {
                        ...
                        $.each(jsonData[0], function (i, v) {
                            ...
                            if(v.quantity > 0 ){
                                ...
                                html +='<input class="qtyopt"  id="button2_id'+ v.product_id +'" type="button" value="确认" onclick="javascript:confirmDoRelevant('+ id +', '+v.deliver_order_id+' , '+v.order_id+');" >';
                                html +='<input class="qtyopt"  id="button2_id'+ v.product_id +'" type="button" value="删除确认数据" onclick="javascript:updateDoRelevant('+ id +', '+v.deliver_order_id+' , '+v.order_id+');" >';
                            }else{
                                html +='<input class="qtyopt"  id="button2_id'+ v.product_id +'" type="button" value="删除确认数据" onclick="javascript:updateDoRelevant('+ id +', '+v.deliver_order_id+' , '+v.order_id+');" >';
                            }
    ...
    function  confirmDoRelevant(id,deliver_order_id ,order_id ) {
        var product_id = $("#product3").val();
        var s_count_id = $('#product2').val();
        var count_id =  s_count_id.substr(0, 6);
    ...
    function  updateDoRelevant (id,deliver_order_id ,order_id){
        var product_id = $("#product3").val();
        var s_count_id = $('#product2').val();
        var count_id =  s_count_id.substr(0, 6);
