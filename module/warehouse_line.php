...
//            }
	...
	ios.product_id , 
	sum(dop.quantity)  order_quantity , 
  if(SUM(ios.quantity) > sum(dop.quantity)  , sum(dop.quantity) , SUM(ios.quantity) )  sorting_quantity ,
  (dop.quantity - sum(ios.quantity)) return_quantity  , 
  NOW(),
...
LEFT JOIN oc_x_inventory_order_sorting ios on ios.deliver_order_id = dop.deliver_order_id  and ios.product_id = dop.product_id and ios.status = 1
...
warehouse_id = '".$warehouse_id ."'";
                    ...
                    $sql_info = "SELECT doo.deliver_order_id,doo.order_id,p.name,p.product_id,
	(SUM(dop.quantity) -IF (SUM(dos.quantity) > 0,SUM(dos.quantity),0)) quantity
FROM
	oc_x_deliver_order doo
LEFT JOIN oc_x_deliver_order_product dop ON doo.deliver_order_id = dop.deliver_order_id
...
ORDER BY
	quantity DESC";
                    ...
                    $sql_info = "SELECT ".$product_id." AS container_id,doi.order_id,doi.deliver_order_id,
...
//                                return $sql_info;
            ...
            return $result_info = $order_array;
        }else{
            return 2 ;
        }
...
//return $psize;
        if($warehouse_id){
            $return = $this->get_come_deliver_order_ids($warehouse_id,$psize,$start_row);
            if ($this->check_result_is_true($return)){
                return $return;
            }
...
//return $result_info;
            ...
            $array = ['data'=>$order_array,'pages'=>$pages,'count'=>$count];
            $return = array(
                'return_code' => 'SUCCESS',
                'return_msg'  => '',
                'return_data' => $array
     ...
     * */
    function get_come_deliver_order_ids($warehouse_id,$psize,$start_row){
        global  $db ;
        $order_info = [];
        $deliver_order_ids = '';
        $order_ids = '';
...
FROM
 oc_x_deliver_order doo 
LEFT JOIN oc_order o ON o.order_id = doo.order_id
LEFT JOIN oc_x_deliver_order doot ON doot.order_id = doo.order_id and doot.deliver_order_id != doo.deliver_order_id and doot.is_repack = 0
WHERE
doo.do_warehouse_id != '".$warehouse_id."'
AND doo.warehouse_id = '".$warehouse_id."'
AND doo.order_status_id IN (6, 8, 12)
AND doo.order_deliver_status_id IN (1, 2)
AND o.order_deliver_status_id = 1
AND o.order_status_id IN (5, 8)
AND (doot.order_status_id in (6,12) or doot.deliver_order_id is null)
 ";
//            return $sql_count;
	...
	doo.order_id,
	IF(doi.inv_comment>0,GROUP_CONCAT(1,'@',doi.inv_comment),GROUP_CONCAT(0,'@',doit.inv_comment)) AS  inv_comment
FROM
 oc_x_deliver_order doo 
LEFT JOIN oc_order o ON o.order_id = doo.order_id
LEFT JOIN oc_x_deliver_order doot ON doot.order_id = doo.order_id and doot.deliver_order_id != doo.deliver_order_id and doot.is_repack = 0
LEFT JOIN oc_x_deliver_order_inv doi ON doi.order_id = doot.order_id and doi.deliver_order_id = doot.deliver_order_id
LEFT JOIN oc_x_deliver_order_inv doit ON doit.order_id = doo.order_id and doit.deliver_order_id = doo.deliver_order_id
WHERE
doo.do_warehouse_id != '".$warehouse_id."'
AND doo.warehouse_id = '".$warehouse_id."'
AND doo.order_status_id IN (6, 8, 12)
