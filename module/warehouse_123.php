...
//        }
 ...
 ";
    ...
    public function confirm_orderStatus(array $data){
        ...
        $sql = "SELECT A.order_id ,A.order_status_id,A.order_payment_status_id  FROM oc_x_logistic_allot_order B   LEFT JOIN  oc_order A ON A.order_id = B.order_id WHERE B.order_id = '". $order_id ."' AND B.logistic_allot_id = '". $logistic_allot_id ."' and A.order_deliver_status_id in(1,11)";
        $query = $db->query($sql);
        $result = $query->row;
        if($result){
    ...
    public function submitDeliverStatus(array $data){
        ...
        if($check_value){
            foreach($check_value as $value){
                $sql = "SELECT A.order_id ,A.order_status_id,A.order_payment_status_id  FROM oc_x_logistic_allot_order B   LEFT JOIN  oc_order A ON A.order_id = B.order_id WHERE B.order_id = '". $value ."'  and A.order_deliver_status_id in(1,11) AND B.logistic_allot_id = '". $logistic_id ."' and A.order_status_id = 6";
...
//        $result = $query->row;
        ...
        $sql_info = "SELECT
                        ...
                        AA.ios_quantity,
                        pt.safe_stock,
                        pt.stock_area_quantity,
                        pt.storage_capacity_quantity,
                        ptw.sku_barcode
                    FROM
                        oc_product_transfer pt
                    LEFT JOIN (
                        SELECT
                            op.product_id,
                        ...
                        AND ios.move_flag = 0
                        WHERE
                            DATE(o.date_added) BETWEEN '".$date1."'
                        AND '".$date."'
                        AND o.warehouse_id = '".$warehouse_id."'
                        GROUP BY
                            op.product_id
                   ...
                   LEFT JOIN oc_product_to_warehouse ptw ON p.product_id = ptw.product_id  and ptw.warehouse_id = '".$warehouse_id."'
                     WHERE
                        pt.warehouse_id ='".$warehouse_id."' and pt.status = 1
";
    ...
    public function getTransferProductInfo(array $data)
    {
        ...
        if (strlen($sku) > 6) {
            $sql = " select  CONCAT(ptw.product_id,'/', p.name) product_id , ptw.product_id transfer_id  , ptw.stock_area , ptw.sku_barcode , pt.safe_stock , pt.stock_area_quantity , pt.storage_capacity_quantity , if(pt.status = 1 , '启用' , '停用') status
              from  oc_product_to_warehouse ptw
              LEFT JOIN  oc_product p ON  ptw.product_id  = p.product_id
              LEFT JOIN  oc_product_transfer pt  ON  ptw.product_id = pt.product_id and pt.warehouse_id = '" . $warehouse_id . "'
    ...
    public function addChangeProductTransfer(array $data)
    {
        ...
        $warehouse_id = $data['data']['warehouse_id'] ? $data['data']['warehouse_id'] : '';
        $product_id_transfer = isset($product_id_transfer) ? $product_id_transfer : false;
        if (!$product_id_transfer) {
            return false;
        }


        $sql = "select  product_id , status   from oc_product_transfer  WHERE  product_id = '" . $product_id_transfer . "' and  warehouse_id = '" . $warehouse_id . "'";

        $query = $db->query($sql);
        $result = $query->row;

        if ($result['product_id']) {
            $sql = " update  oc_product_transfer
                  set safe_stock = '" . $input_safe_area_quantity . "' ,  stock_area_quantity = '" . $input_stock_area_quantity . "' ,  storage_capacity_quantity = '" . $input_capacity_quantity . "' where   warehouse_id =  '" . $warehouse_id . "' and product_id = '" . $product_id_transfer . "' ";
            ...
            $sql = "insert into oc_product_transfer ( `product_id` ,`warehouse_id` , `stock_area` , `safe_stock`, `stock_area_quantity`,`storage_capacity_quantity`,`date_added` , `added_by`) VALUES  ('" . $product_id_transfer . "','" . $warehouse_id . "', '" . $stock_area . "', '" . $input_safe_area_quantity . "', '" . $input_stock_area_quantity . "', '" . $input_capacity_quantity . "', NOW(), '" . $inventory_user_id . "'  )";
    ...
    public function ChangeProductTransferStatus(array  $data)
    {
        ...
        $sql = "select  product_id , status ,stock_area_quantity ,   safe_stock  ,storage_capacity_quantity from oc_product_transfer  WHERE  product_id = '" . $product_id_transfer . "' and  warehouse_id = '" . $warehouse_id . "'";
        ...
        if ($result['product_id']) {
            if ($result['status'] == 1) {
                $sql = " update  oc_product_transfer
                  set status =0  where   warehouse_id =  '" . $warehouse_id . "' and product_id = '" . $result['product_id'] . "' ";
                ...
                $sql = " update  oc_product_transfer
                  set status =1  where   warehouse_id =  '" . $warehouse_id . "' and product_id = '" . $result['product_id'] . "' ";
