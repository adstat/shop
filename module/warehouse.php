...
class WAREHOUSE{
    public function getAccomplishFrame(array $data)
    {
//        return $data;
        global $db;
//        $str=json_decode($data,true);
//        $str=array_values($data['data']['product_id_arr']);
//        $str=implode(',',$str);
        ...
        $sql="SELECT
            ...
            WHERE
                deliver_order_id ='".$data['data']['order_id']."' AND 
                status ='1' AND 
                container_id >0 
            GROUP BY
...
//        return $sql;
        ...
        foreach ($list as $value) {
            $product_ids[$value['product_id']]['quantity'] = isset($product_ids[$value['product_id']]['quantity']) ? (int)$product_ids[$value['product_id']]['quantity']+(int)$value['quantity'] : (int)$value['quantity'];
        }

        $sql2="SELECT  product_id,name as product_name,SUM(quantity) as product_num FROM oc_x_deliver_order_product WHERE deliver_order_id ='".$data['data']['order_id']."' group by product_id ";
//        return $sql2;
        ...
        foreach ($list as $v2){
            $arr_[$v2['product_id']]=$v2;
            $arr_[$v2['product_id']]['product_num']=empty($arr[$v2['product_id']])?'0':$arr[$v2['product_id']]['product_num'];
            $arr_[$v2['product_id']]['product_name']=empty($arr[$v2['product_id']])?'0':$arr[$v2['product_id']]['product_name'];
        }
        $arr2=[];
        foreach ($arr_ as $k=>$v){

            $arr2[$v['container_id']][]=$v;
        }
        return $arr2;*/

        /*foreach ($list as $k=>$v){
            $arr[$v['container_id']][]=$v;
        }
        return $arr;*/
        foreach ($list2 as $k=>$v){
            $arr[$v['product_id']]=$v;
...
//        }
        ...
        foreach($list as $m){
            ...
            $containerList[$m['container_id']][$m['product_id']] = $m;
            $containerList[$m['container_id']][$m['product_id']]['product_num'] = $arr[$m['product_id']]['product_num'];
            $containerList[$m['container_id']][$m['product_id']]['product_name'] = $arr[$m['product_id']]['product_name'];
            $containerList[$m['container_id']][$m['product_id']]['other_number'] = isset($product_ids[$m['product_id']]['quantity']) ? (int)$product_ids[$m['product_id']]['quantity']- (int)$m['quantity']:0;
            $containerList[$m['container_id']][$m['product_id']]['sorting_num'] = isset($product_ids[$m['product_id']]['quantity']) ? (int)$product_ids[$m['product_id']]['quantity']:0;
        }
...
AND '2018-10-10' ";
    ...
    public function delOneProductInv(array $data)
    {
        global $db;
//        return $data;
        $container_id = $data['data']['container_id'] ? $data['data']['container_id'] : '';
        $product_id = $data['data']['product_id'] ? $data['data']['product_id'] : '';
        $sql="delete from  oc_x_inventory_order_sorting WHERE container_id= $container_id AND product_id= ".$product_id."";
        $query = $db->query($sql);
        return $query;
    }

    public function getFrameProductNumber(array $data)
    {
...
//        return $order_id;
        $sql="select 
              ...
              A.product_id,
              A.deliver_order_id,
              COUNt(A.product_id) AS onFrameNumber 
              from (
                select 
                container_id,
                product_id,
                deliver_order_id 
                FROM `oc_x_inventory_order_sorting` 
                WHERE 
                  deliver_order_id = ".$order_id." 
                  AND STATUS = 1 
                  AND container_id > 0 
                  GROUP BY container_id,product_id) A 
...
//return 2;
        ...
        select
             ...
             group by O.order_id, OP.product_id ORDER BY O.order_id desc ;<br />";
            }
            return $array;
...
//        }
 ...
 WHERE  A.order_id = '" . $order_id . "' ";
    ...
    public function getOrderByDriver(array $data){
        ...
        if($warehouse_id>0){
            ...
            if(  $result1['total'] &&  $result1['total'] > 300000){
                return 1  ;
            }else{
                $sql = "SELECT
                C.inv_comment,
 ...
 ";
    ...
    public function confirm_orderStatus(array $data){
        ...
        $sql = "SELECT A.order_id ,A.order_status_id,A.order_payment_status_id ,oi.frame_vg_list , A.customer_id  , A.warehouse_id FROM oc_x_logistic_allot_order B  LEFT JOIN  oc_order A ON A.order_id = B.order_id LEFT JOIN  oc_order_inv oi ON  A.order_id = oi.order_id WHERE B.order_id = '". $order_id ."' AND B.logistic_allot_id = '". $logistic_allot_id ."'";
//         and A.order_deliver_status_id in(1,11)";

        $query = $db->query($sql);
        $result = $query->row;

        $sql_return = "select order_id  from oc_x_inventory_order_return_quantity  where order_id  = '".$order_id ."'  group by order_id ";
        $query = $db->query($sql_return);
        $result_return = $query->row;
        
        if($result_return['order_id'] == ''){
            return $result['status'] = 4;
        }
        
//        if($result['frame_vg_list'] ) {
//            $result['frame_vg_list'] = trim($result['frame_vg_list'], ',');
//            if ($result['frame_vg_list']) {
...
//            $sql_order_move = "select  order_id from oc_x_stock_move where order_id = '".$order_id."' and inventory_type_id  = 12  ";
//
//            $query = $db->query($sql_order_move);
//            $result_order_move = $query->row;
//            if($result_order_move['order_id']> 0 ){
//
//                $sql = "update oc_x_inventory_order_sorting set move_flag = 1 where status = 1 and  order_id  in (" . $order_id . ")";
//                $query = $db->query($sql);
//
...
//                $query = $db->query($sql);
//
//                $sql = "insert into oc_order_history (`order_id`,`comment`,`date_added`,`order_status_id`,`order_payment_status_id`,`order_deliver_status_id`,`modified_by` ) VALUES ('" . $order_id . "','" . $comment . "',current_timestamp() , '" . $result['order_status_id'] . "','" . $result['order_payment_status_id'] . "','2','" . $user_id . "')";
//                $query = $db->query($sql);
//
//                return $result['status'] = 1;
//            }else {
//                $inventory_op = -1;
//                $sql_stock_move = " insert into oc_x_stock_move  ( `station_id` , `timestamp` ,`order_id` , `inventory_type_id` , `date_added` , `add_user_name` , `warehouse_id` )  VALUES  ('2' ,unix_timestamp(), '" . $order_id . "' , '12' , NOW() , '" . $user_id . "' , '" . $result['warehouse_id'] . "' )";
////return $sql_stock_move;
//                $query = $db->query($sql_stock_move);
//                $stock_move_id = $db->getLastId();
//
//
//                $sql_stock_move_item = "INSERT INTO `oc_x_stock_move_item` (`inventory_move_id`, `station_id`, `due_date`, `product_id`, `price`, `product_batch`, `quantity`, `box_quantity`, `weight`, `is_gift`, `checked`, `status`, `sku_id`)(SELECT
//'" . $stock_move_id . "' ,
// '2',
// '0000-00-00' ,
// iorq.product_id ,
// op.price  ,
// '0',
// iorq.sorting_quantity *$inventory_op,
// '1',
// '0',
// '0',
// '0',
...
//
//FROM
//	oc_x_inventory_order_return_quantity iorq
//LEFT JOIN oc_product p ON iorq.product_id = p.product_id
//left join oc_order_product op on iorq.product_id = op.product_id and op.order_id = iorq.order_id
//WHERE
//	iorq.order_id = '" . $order_id . "'
//	 group by iorq.product_id)";
//
...
//                    $bool = true;
                    ...
                    $a = $this->addReturn(array($order_id), $result['warehouse_id']);return $a;
//                    $sql_order_move = "select  order_id from oc_x_stock_move where order_id = '".$order_id."' and inventory_type_id  = 12  ";
//
//                    $query = $db->query($sql_order_move);
//                    $result_order_move = $query->row;
    ...
    public function submitDeliverStatus(array $data){
        ...
        if($check_value){
            foreach($check_value as $value){
                ...
                $sql_return = "select order_id  from oc_x_inventory_order_return_quantity  where order_id  = '".$value ."'  group by order_id ";
                $query = $db->query($sql_return);
                $result_return = $query->row;
                ...
                if($result){
                    ...
                    $sql_order_move = "select  order_id from oc_x_stock_move where order_id = '".$value."'  and inventory_type_id  = 12 ";

                    $query = $db->query($sql_order_move);
                    $result_order_move = $query->row;
                    if($result_order_move['order_id'] >0){
                        $sql = "update oc_x_inventory_order_sorting set move_flag = 1 where status = 1 and  order_id  in (" . $value . ")";
                        ...
                        $inventory_op =  -1 ;
                        $sql_stock_move = " insert into oc_x_stock_move  ( `station_id` , `timestamp` ,`order_id` , `inventory_type_id` , `date_added` , `add_user_name` , `warehouse_id` )  VALUES  ('2' ,unix_timestamp(), '".$value."' , '12' , NOW() , '".$user_id."' , '".$result['warehouse_id'] ."' )";
...
left join oc_order_product op on iorq.product_id = op.product_id and op.order_id = iorq.order_id 
WHERE
	iorq.order_id = '".$value."'  and iorq.status =1 
	group by iorq.product_id)";
                        ...
                        if($query){
                            ...
                            $db->begin();
                            $bool = true;
                            $this->addReturn(array($value),$result['warehouse_id']);

//                            $sql_order_move = "select  order_id from oc_x_stock_move where order_id = '".$value."'  and inventory_type_id  = 2 ";
//
//                            $query = $db->query($sql_order_move);
//                            $result_order_move = $query->row;
    ...
    public function getWarehouseId( ){
        ...
        $sql = "SELECT  warehouse_id ,title,station_id,is_dc  FROM  oc_x_warehouse  where  status = 1 ";
    ...
    public function getUseWarehouseId( ){
            ...
            $sql = "SELECT  warehouse_id ,title,relevant_suppler_id FROM  oc_x_warehouse WHERE status = 1";
...
//        return $sql;
            ...
            $sql = " SELECT oco.uptime,oco.added_by,doo.deliver_order_id,if (doo.is_repack=0,odi.inv_comment,odi.frame_vg_list) AS stock_area,doo.order_id,doo.is_repack, doo.relevant_id
...
根据商品条码获取商品信息*/
    ...
    public function getinventoryname(array $data){
        ...
        $check_is_dc = $this->checkWarehouseIsDc($warehouse_id);

        $sql = "select username from oc_w_user  WHERE  warehouse_id = '".$warehouse_id."'and status = 1 and user_group_id =15 ";

        if($warehouse_id == 12 || $check_is_dc){
    ...
    public function getinventorynamerepack(array $data){
        global  $db ;
        $warehouse_id  = $data['data']['warehouse_id'];
        $repack  = $data['data']['repack'] ? $data['data']['repack'] : false ;
        $user_warehouse_id  = $data['data']['user_warehouse_id'];
        $check_is_dc = $this->checkWarehouseIsDc($warehouse_id);

        $sql = "select username from oc_w_user  WHERE  warehouse_id = '".$warehouse_id."'and status = 1 and user_group_id =15 ";
        if ($repack) {
            $sql .= " and repack ='".$repack."' ";
        }
         if($warehouse_id == 12 || $check_is_dc){
             $sql .=  "  and to_warehouse_id = '".$user_warehouse_id."' ";
         }
...
//            AND wr.to_warehouse =" . $to_warehouse_id;
//            查询出库时有多少周转筐
                $sq2 = "SELECT  COUNT(DISTINCT IF(wri.container_id>0,wri.container_id,NULL)) AS container_nums
FROM oc_x_warehouse_requisition wr  
LEFT JOIN oc_x_warehouse_requisition_item wri ON  wri .relevant_id = wr.relevant_id  
WHERE  1=1 ";
                if ($relevant_status_id == 2) {                             //出库
...
//            AND wr.to_warehouse =" . $to_warehouse_id;
                ...
                //            查询出库时扫描了多少周转筐
                $sq3 = "SELECT  COUNT(DISTINCT IF(wrt.container_id>0,wrt.container_id,NULL)) AS container_nums
FROM oc_x_warehouse_requisition wr  
...
//            AND wr.to_warehouse =" . $to_warehouse_id;
//            查询入库时有多少周转筐
                $sq2 = "SELECT  COUNT(DISTINCT IF(wrt.container_id>0,wrt.container_id,NULL)) AS container_nums
FROM oc_x_warehouse_requisition wr  
LEFT JOIN oc_x_warehouse_requisition_temporary wrt ON  wrt .relevant_id = wr.relevant_id  
WHERE  1=1 ";
                if ($relevant_status_id == 4) {                      //入库
...
//            AND wr.to_warehouse =" . $to_warehouse_id;
            ...
            $return['container_numbers2'] = $resultsContainer2[0]['container_nums'];
                //判断临时表是否有
            $relevant_ids = intval($results['relevant_id']); //字符串
            ...
            if ($warehouse_id == $from_warehouse_id) {
                ...
                //出库查询插入临时表的信息
                $sql1 = " SELECT wri.container_id ,wri.relevant_id,wri.product_id,IF(wrt.quantity>0,wrt.quantity,0) AS num1,IF(wri.num>0,wri.num,0) AS num2,ptw.sku_barcode , p.name
FROM (
    SELECT
    product_id,
    relevant_id,
    SUM(num) as num,container_id
    FROM
    oc_x_warehouse_requisition_item
    WHERE
...
LEFT JOIN (
    ...
    product_id,
    relevant_id,
    SUM(quantity) as quantity,container_id
    FROM
    oc_x_warehouse_requisition_temporary
    WHERE
    relevant_id = '".$relevant_ids."'
    ...
    product_id,
    container_id
) wrt ON  wri.product_id = wrt.product_id  AND wri.container_id = wrt.container_id AND wri.relevant_id = wrt.relevant_id
LEFT JOIN oc_product_to_warehouse ptw ON wri.product_id = ptw.product_id AND ptw.warehouse_id = ".$warehouse_id."
LEFT JOIN oc_product p ON wri.product_id = p.product_id                     
WHERE wri.product_id>0 GROUP BY wri.product_id,wri.container_id ORDER BY wrt.container_id";
            } else {
                //入库查询插入临时表的信息
                $sql1 = " SELECT wrts.container_id ,wrts.relevant_id,wrts.product_id,IF(wrt.quantity>0,wrt.quantity,0) AS num1,IF(wrts.quantity>0,wrts.quantity,0) AS num2,ptw.sku_barcode,p.name
FROM (
    SELECT
    product_id,
    relevant_id,
...
LEFT JOIN (
    ...
    product_id,
    container_id
) wrt ON  wrts.product_id = wrt.product_id  AND wrts.container_id = wrt.container_id AND wrts.relevant_id = wrt.relevant_id
LEFT JOIN oc_product_to_warehouse ptw ON wrts.product_id = ptw.product_id AND ptw.warehouse_id = ".$from_warehouse_id."
LEFT JOIN oc_product p ON wrts.product_id = p.product_id 
WHERE wrts.product_id>0 GROUP BY wrts.product_id,wrts.container_id ORDER BY wrt.container_id";
            }
//return $sql1;
            $invalid = $db->query($sql1)->rows;
            foreach($invalid as $values){
                $quantity = intval($values['num1']);
                if ($quantity>0) {
                    $return['product1'][] = $values;
                } else {
    ...
    public function addPurchaseOrderRelevantToInv(array  $data){
        ...
        $warehouse_id = isset($data['data']['warehouse_id']) ? $data['data']['warehouse_id'] : false;
        $relevant_suppler_id = isset($data['data']['relevant_suppler_id']) ? $data['data']['relevant_suppler_id'] : false;
...
//        return $data;
        /*zx
        t退货调拨单*/
        if (trim($warehouse_out_type) == 4  || trim($warehouse_out_type) == 5 ) {
            $warehouse_cost = isset($data['data']['warehouse_cost']) ? intval($data['data']['warehouse_cost']) : false;
            $shipping_cost = isset($data['data']['shipping_cost']) ? intval($data['data']['shipping_cost']) : false;
            /*zx
            入库*/
...
GROUP BY wrt.product_id ORDER BY wrt.container_id";
                ...
                VALUES('" . $relevant_ids2 . "','2',current_date(),unix_timestamp(),'0','23',now(),'" . $added_by_id . "','仓间调拨单入库','" . $warehouse_id . "')";
                    ...
                    /*zx
                    生成调拨单*/
                    $sql5 = "INSERT INTO oc_x_warehouse_requisition (from_warehouse,to_warehouse,date_added,added_by,relevant_status_id,confirm_by,deliver_date,out_type,`shipping_cost`,`warehouse_cost`,`total_cost`) VALUES 
    (".$warehouse_id.",'".$to_warehouse_id."',NOW(),'".$added_by_id."',4,'".$added_by_id."',NOW(),4,'".$shipping_cost."','".$warehouse_cost."','".($shipping_cost+$warehouse_cost)."')";
                    $query5 = $db->query($sql5);
                ...
                $sql5 = "INSERT INTO oc_x_warehouse_requisition (from_warehouse,to_warehouse,date_added,added_by,relevant_status_id,confirm_by,deliver_date,out_type,`shipping_cost`,`warehouse_cost`,`total_cost`) VALUES 
(" . $warehouse_id . ",'" . $to_warehouse_id . "',NOW(),'" . $added_by_id . "',6,'" . $added_by_id . "',NOW(),5,'".$shipping_cost."','".$warehouse_cost."','".($shipping_cost+$warehouse_cost)."')";
                ...
                /*zx
                入库记录*/
                $sql5 = "INSERT INTO oc_x_stock_move (station_id,timestamp,date_added,added_by,relevant_id,inventory_type_id,warehouse_id , memo ) VALUES (2,'" .
                    time() . "',NOW(),'" . $added_by_id . "','" . $new_relevant_id . "','23','" . $warehouse_id . "' , '调拨商品错误异常收货')";
                $query5 = $db->query($sql5);
                $stock_move_id = $db->getLastId();
                $sql6 = "INSERT INTO `oc_x_stock_move_item` (`inventory_move_id`, `station_id`, `due_date`, `product_id`,quantity, `box_quantity`,  `status`) VALUES ";
                foreach ($product_ids as $key => $values) {
                    $sql6 .= "(" . $stock_move_id . ",2,NOW()," . $values . ",'" . ( $quantitys[$key]) . "',1,'1'),";
                }
                $sql6 = rtrim($sql6, ',');
                $query6 = $db->query($sql6);
                /*zx
                修改前台库存*/
                $sql5 = "INSERT INTO oc_x_inventory_move (`relevant_id`,`station_id`, `date`, `timestamp`, `from_station_id`, `inventory_type_id`, `date_added`, `add_user_name`, `memo`,`warehouse_id`)
                VALUES('" . $new_relevant_id . "','2',current_date(),unix_timestamp(),'0','23',now(),'" . $added_by_id . "','调拨商品错误异常收货','" . $warehouse_id . "')";
                $query5 = $db->query($sql5);
                $inventory_move_id = $db->getLastId();
                $sql6 = "INSERT INTO `oc_x_inventory_move_item` (`inventory_move_id`, `station_id`, `due_date`, `product_id`, `quantity`,`warehouse_id`) VALUES ";
                foreach ($product_ids as $key => $values) {
                    $sql6 .= "(" . $inventory_move_id . ",2,NOW()," . $values . ",'" . ( $quantitys[$key]) . "','" . $warehouse_id . "'),";
                }
                $sql6 = rtrim($sql6, ',');
                $query6 = $db->query($sql6);
            }
...
WHERE wr.relevant_id =" . $relevant_ids2;
            ...
            if (trim($to_warehouse_id) == trim($warehouse_id)) {
                 ...
                 *zx
                 * 获取周转筐中的所有商品，并进行商品分类统计数量*/
                $sql4 = "SELECT wrt.product_id,SUM(wrt.quantity) AS quantity,wri.purchase_price,ptw.sku_id,oxw.station_id,ptw.name AS product,ptw.price,wrt.container_id 
FROM oc_x_warehouse_requisition_temporary wrt
LEFT JOIN oc_x_warehouse_requisition_item wri ON wri.product_id = wrt.product_id AND wri.relevant_id = wrt.relevant_id and wrt.container_id = wri.container_id 
LEFT JOIN oc_product ptw ON ptw.product_id = wrt.product_id
LEFT JOIN oc_x_warehouse oxw ON oxw.warehouse_id = wrt.warehouse_id AND oxw.warehouse_id =" . $warehouse_id . " 
WHERE wrt.warehouse_id = " . $warehouse_id . "
AND wrt.relevant_id IN (" . $relevant_ids2 . ")
...
//                        'add_user_name' => $added_by,
//                        'supplier_type' => $relevant_suppler_id[$warehouse_id]['relevant_suppler_id'],
//                        'order_comment' => "仓间调拨".$relevant_suppler_id[$warehouse_id]['title']."入库",
//                        'warehouse_id' => $warehouse_id
//                    ];
//                    $purchase_order_product = $results2;
//                    $return = $this->insert_into_purchase_order($purchase_order,$purchase_order_product,1);
//                    if ($this->check_result_is_true($return)) {
//                        return $return;
//                    }
//
//                }
...
GROUP BY wrt.product_id ORDER BY wrt.container_id";
                ...
                $select_deliver_orders = $db->query("SELECT
	GROUP_CONCAT(
		DISTINCT
		IF (
			deliver_order_id > 0,
	...
	) AS deliver_order_ids
FROM
	oc_x_warehouse_requisition_item
WHERE
	relevant_id IN (".$relevant_ids2.")")->row;
                if (!empty($select_deliver_orders['deliver_order_ids'])) {
                    $sql3 = "UPDATE oc_x_deliver_order SET order_status_id = 6,repack_status_id = 6,order_deliver_status_id = 2 WHERE deliver_order_id IN (".trim($select_deliver_orders['deliver_order_ids'],'\'').") AND order_status_id != 12";
                    $db->query($sql3);
                    $sql = "UPDATE  oc_x_deliver_order a INNER JOIN oc_order b on a.order_id = b.order_id  SET b.order_status_id = 5 WHERE b.order_status_id = 2 AND a.deliver_order_id IN(".trim($select_deliver_orders['deliver_order_ids'],'\'').")";
                    $db->query($sql);
                }
...
//                        'add_user_name' => $added_by,
//                        'supplier_type' => $relevant_suppler_id[$warehouse_id]['relevant_suppler_id'],
//                        'order_comment' => "仓间调拨".$relevant_suppler_id[$warehouse_id]['title']."出库",
//                        'warehouse_id' => $warehouse_id
//                    ];
//                    $purchase_order_product = $results2;
//                    $return = $this->insert_into_purchase_order($purchase_order,$purchase_order_product,2);
//                    if ($this->check_result_is_true($return)) {
//                        return $return;
//                    }
//
...
//                if (abs($returnedTotal)+abs($currentReturnTotal) == $subTotal) {
//                    $returnCredits = 0;
//                }
            }
            else{
...
//            }
            ...
            if (!$bool) {
                ...
                if($return_action_id == 2 || $return_action_id == 4){
                    ...
                    $bool = $bool && $dbm->query($sql);
                    $bool && $this->addInventoryMoveOrders($stockMoveData, $stationId,$warehouse_id);
...
";
 ...
 ('".$warehouse_id."' , '".$stock_area."' , '".$warehouse_section_id."'   , NOW(), '".$inventory_user_id."')";
    ...
    public  function  getOrderByFrame(array  $data){
        global $db;
        $logistic_allot_id = $data['data']['logistic_allot_id'] ? trim($data['data']['logistic_allot_id'],',') : '';
        $old_logistic_allot_id = $data['data']['old_logistic_allot_id'] ? $data['data']['old_logistic_allot_id'] : '';
        $warehouse_id = $data['data']['warehouse_id'] ? $data['data']['warehouse_id'] : '';
        $add_user_id = $data['data']['user_id'] ? $data['data']['user_id'] : '';
        $logistic_driver_id = $data['data']['logistic_driver_id'] ? $data['data']['logistic_driver_id'] : '';
...
//                group by cm.container_id order by cm.order_id  ";
        $sql = "SELECT cdr.container_id,cdr.order_id,cdr.checked,cdr.warehouse_id,cdr.logistic_allot_id,la.logistic_driver_title,la.logistic_driver_id,oc.type,oct.type_name
 FROM oc_x_container_deliver_return cdr
LEFT JOIN oc_x_logistic_allot la ON la.logistic_allot_id = cdr.logistic_allot_id 
LEFT JOIN oc_x_container oc ON oc.container_id = cdr.container_id
LEFT JOIN oc_x_container_type oct ON oct.type_id = oc.type
 WHERE 
cdr.logistic_allot_id IN(".$logistic_allot_id.") AND cdr.status = 1 ";
//        return $sql;
        $array_data = $db->query($sql)->rows;
//return $array_data;
        if (empty($array_data)) {
            $sql = "  SELECT
	cm.frame_vg_list,
	...
	la.logistic_driver_id,
	la.logistic_allot_id,0 'checked'
FROM
	oc_order_inv cm
LEFT JOIN oc_x_logistic_allot_order lao ON cm.order_id = lao.order_id
LEFT JOIN oc_x_logistic_allot la ON la.logistic_allot_id = lao.logistic_allot_id
WHERE
	la.logistic_allot_id IN(".$logistic_allot_id.")
GROUP BY
	la.logistic_allot_id,cm.order_id
ORDER BY
	cm.order_id  ";
//return $sql;
            $query = $db->query($sql);
...
//return $result;
            if (!empty($result)) {
                foreach ($result as $value1) {
                    $container_types = [];
                    if (!empty($value1['frame_vg_list'])) {
                        $value1['frame_vg_list'] = trim($value1['frame_vg_list'],',');
                        $sql_container = "SELECT
	oc.container_id,oc.type,oct.type_name
FROM
	`oc_x_container` oc
LEFT JOIN oc_x_container_type oct ON oct.type_id = oc.type
WHERE
	oc.container_id IN (".$value1['frame_vg_list'].")";
                        $query_container = $db->query($sql_container);
                        $container_type = $query_container->rows;
                        if (!empty($container_type)) {
                            foreach ($container_type as $value) {
                                $container_types[(int)$value['container_id']] = $value;
                            }
                        }
...
//
//                    if (!empty($value1['frame_vg_list'])) {
                        $container_lists = explode(',',$value1['frame_vg_list']);

                        foreach ($container_lists as $value2) {
                            if ($value2>0) {
                                $array_data[$value2] = $value1;
                                $array_data[$value2]['container_id'] = $value2;
                                $array_data[$value2]['type'] = isset($container_types[(int)$value2]) ? $container_types[(int)$value2]['type'] : 0 ;
                                $array_data[$value2]['type_name'] = isset($container_types[(int)$value2]) ? $container_types[(int)$value2]['type_name'] : "异常周转筐" ;
                            }
                        }
                    }
...
//                return $array_data;
                ...
                if (!empty($array_data)) {
                    $sql = "INSERT INTO `oc_x_container_deliver_return` 
(`logistic_allot_id`, `warehouse_id`, `order_id`, `container_id`, `add_w_user_id`, `date_added`, `checked`, `status`) VALUES";
                    foreach ($array_data as $value3) {
                        $sql .= "('".intval($value3['logistic_allot_id'])."','".$warehouse_id."','".intval($value3['order_id'])."','".intval($value3['container_id'])."','".$add_user_id."',NOW(),0,1),";
                    }
                    $sql  = rtrim($sql,',');
                    $db->query($sql);
                    if (intval($logistic_driver_id)>0 && intval($logistic_driver_id) == intval($result[0]['logistic_driver_id']) && !in_array(intval($result[0]['logistic_allot_id']),explode(',',$old_logistic_allot_id))) {
                        $return = array(
                            'return_code' => 'OK',
                            'return_msg'  => 'OK',
                            'return_data' => $array_data
                        );
                    } else if (in_array(intval($result[0]['logistic_allot_id']),explode(',',$old_logistic_allot_id))) {
                        $return = array(
                            'return_code' => 'ERROR',
                            'return_msg'  => '请不要重复扫描',
                            'return_data' => $array_data
            ...
            if (intval($logistic_driver_id)>0 && intval($logistic_driver_id) == intval($array_data[0]['logistic_driver_id']) && !in_array(intval($array_data[0]['logistic_allot_id']),explode(',',$old_logistic_allot_id))) {
                $return = array(
                    'return_code' => 'OK',
                    'return_msg'  => 'OK',
                    'return_data' => $array_data
   ...
   public function selectContainer(array $data) {
    global $db;
    // return '123321';
    $user_id = $data['data']['user_id'];
    $warehouse_id = $data['data']['warehouse_id'];
    $logistic_allot_id = $data['data']['logistic_allot_id'];
    $container_id = $data['data']['container_id'];
    $sql_select = "SELECT container_id FROM oc_x_container WHERE container_id = '".$container_id."' AND occupy = 1 ";
    // return $sql_select;
    $result = $db->query($sql_select)->row;
    if (!empty($result)) {
        $sql = "INSERT INTO `oc_x_container_deliver_return` 
            (`logistic_allot_id`, `warehouse_id`, `order_id`, `container_id`, `add_w_user_id`, `date_added`, `checked`, `status`) VALUES ";
        $sql .= "('" . $logistic_allot_id . "','" . $warehouse_id . "',0,'" . $container_id . "','".$user_id."',NOW(),0,1)";
        // return $sql;
        $db->query($sql);
        $return = array(
             'return_code' => 'SUCCESS',
   ...
   public  function  confirmFrameInStatus(array $data ){
        global $db;
        // return $data;
        $container = $data['data']['container_id'] ? $data['data']['container_id'] : '';
        $user_id = $data['data']['user_id']? $data['data']['user_id'] : '';
        $order_id = $data['data']['order']? $data['data']['order'] : '';
        $logistic_allot_id = $data['data']['logistic_allot_id']? $data['data']['logistic_allot_id'] : '';
        $warehouse_id = $data['data']['warehouse_id']? $data['data']['warehouse_id'] : '';
        $is_admin = $data['data']['is_admin']? $data['data']['is_admin'] : 0;
        $admin_id = intval($is_admin) == 1 ? $user_id : 0 ;
//return $user_id;
$db->begin();
try{
foreach($container as $container_id) {
        if($container_id){
            $sql = "SELECT * FROM oc_x_container_deliver_return WHERE logistic_allot_id = '".$logistic_allot_id."'";
//        return $sql;
            $array_data = $db->query($sql)->rows;
            // return $array_data;
            if (!empty($array_data)) {
                ...
                foreach ($array_data as $value) {
                    $array_container[intval($value['container_id'])] = $value;
                }
                if (empty($array_container[intval($container_id)])) {
                    $sql_select = "SELECT container_id FROM oc_x_container WHERE container_id = '".$container_id."' AND occupy = 1 ";
                    $result = $db->query($sql_select)->row;
                    if (!empty($result)) {
                        $sql = "INSERT INTO `oc_x_container_deliver_return` 
(`logistic_allot_id`, `warehouse_id`, `order_id`, `container_id`, `add_w_user_id`, `date_added`, `checked`, `status`,added_by_manager,date_checked) VALUES ";
                        $sql .= "('" . $logistic_allot_id . "','" . $warehouse_id . "','" . $order_id . "','" . $container_id . "','".$user_id."',NOW(),1,1,'".$admin_id."',NOW())";
                    } else {
                        $return = array(
                            'return_code' => 'ERROR',
                            'return_msg'  => '周转筐号有误',
                ...
                } else if (intval($array_container[intval($container_id)])['checked'] == 0) {
                    $sql = "UPDATE  oc_x_container_deliver_return SET checked = 1,added_by_manager='".$admin_id."',date_checked=NOW()  WHERE logistic_allot_id = '".$logistic_allot_id."' AND container_id = '".$container_id."'";
            ...
            $sql = " insert  into  oc_x_container_fast_move  (`container_id` , `customer_id` ,`order_id` , `move_type` ,`date_added` , `add_w_user_id`)
                          VALUES  ('".$container_id."' , '".$result['customer_id']."','".$logistic_allot_id."' , '-1' , NOW() , '".$user_id."' )  ";
...
$db->commit();
    ...
    public  function checkContainer(array $data){
        ...
        /*zx
    判断之前的周转筐是否提交已满*/
        $frame_vg_list = isset($data['data']['fram_lists_merge']) ? $data['data']['fram_lists_merge'] : false;
        $frame_vg_list = join(',', $frame_vg_list);
        if (!empty($frame_vg_list)) {

            $sql = "SELECT occupy  
FROM oc_x_container 
WHERE container_id IN(" . $frame_vg_list . ")";
        $query = $db->query($sql);
        $result['jsonData2'] = $query->rows;
        }

        $sql =  " select  container_id , occupy , instore from oc_x_container where container_id = '".$container_id."' ";
//        and warehouse_id in ( ".$warehouse_id ." , 0  )
//        ";

        $query = $db->query($sql);
        $result['jsonData'] = $query->row;
     ...
     * */
    //分拣数据
    public function getAbnormalSort(array $data)
    {
        return '111111';
//        echo '111';
        global $db;
...
//and do.warehouse_id = '".$warehouse_id."'
        ...
        if($result_dan['num'] == 1 ) {
            ...
            $sql = " select  ddo.deliver_order_id  order_id , od.inventory_name , doi.inv_comment , doi.frame_vg_list  ,  dos.name ,ddo.order_id order_order_id  , sm.inventory_move_id   ,if(ddo.is_repack = 0  , '整单' , '散单') is_repack     from oc_x_deliver_order  ddo left join oc_order_distr od on ddo.order_id =od.order_id  left join  oc_x_deliver_order_inv doi  on  doi.order_id = ddo.order_id LEFT JOIN oc_x_deliver_order_status dos ON dos.order_status_id = ddo .order_status_id   left join oc_x_stock_move  sm on sm.order_id = ddo.order_id   where ddo.ori_order_id  = '".$result['order_id']."'";

            $query = $db->query($sql);
            $result_dan = $query->rows;
            return $result_dan ;
        }else{
            $sql_deliver_order = "SELECT 
    DO.deliver_order_id order_id ,
	od.inventory_name,
	doi.inv_comment,
	doi.frame_vg_list,
	dos.name ,
	if(DO.is_repack = 0  , '整单' , '散单') is_repack    , 
	sm.order_id inventory_move_id   , 
		if(DO.is_repack = 0  , DO.order_status_id , DO.order_status_id) order_status_id     , 
		DO.order_id  order_order_id  , 
		DO.do_warehouse_id , 
		DO.warehouse_id 
FROM
	oc_x_deliver_order DO
	LEFT JOIN oc_order_distr od ON DO.deliver_order_id = od.deliver_order_id 
AND DO.order_id = od.order_id
	LEFT JOIN oc_x_deliver_order_inv doi ON DO.deliver_order_id = doi.deliver_order_id
	LEFT JOIN oc_x_deliver_order_status dos ON dos.order_status_id = DO.order_status_id 
	left join oc_x_consolidated_order  sm on sm.order_id = DO.order_id 
WHERE
DO.order_id = '".$result['order_id']."'
    ...
    public  function  confirmOrderInfoByCount(array $data){
        ...
        $sql = " select GROUP_CONCAT(deliver_order_id) deliver_order_id   from oc_x_deliver_order where order_id = '".$result_order['order_id']."' ";
        $query = $db->query($sql);
        $result = $query->row;

        if($result['deliver_order_id']){
            $arr_deliver_order_id = explode(',', $result['deliver_order_id']);

            foreach ($arr_deliver_order_id as $deliver_order_id){
                $sql = "select deliver_order_id from oc_x_inventory_order_return_quantity where deliver_order_id = '".$deliver_order_id ."'";
 
                $query = $db->query($sql);
                $result_order_return = $query->row;

                if($result_order_return['deliver_order_id'] == '' ||  $result_order_return['deliver_order_id'] < 0   ){
  ...
  (dop.quantity - sum(ios.quantity)) return_quantity  , 
  NOW(),
  '".$inventory_user."'
FROM
...
where  ios.status = 1  and ios.deliver_order_id  = '".$deliver_order_id."'
GROUP BY  ios.product_id )";

                    $bool = $bool &&  $db->query($sql_return_quantity);
                    if(!$bool) {
                        //$log->write('[分拣]分拣提交失败[' . __FUNCTION__ . ']' . "\n\r");
                        $db->rollback();
                        return array('status' => 0, 'timestamp' => $data['timestamp']);
        ...
        $sql_deliver_order_inv  = "  select inv_comment   from oc_x_deliver_order doo LEFT join  oc_x_deliver_order_inv doi on doo.deliver_order_id = doi.deliver_order_id  where doo.do_warehouse_id = '".$warehouse_id."' and doo.is_repack = 0 and doo.order_id = '".$result_order['order_id']."' ";

        $query = $db->query($sql_deliver_order_inv);

        $result_deliver_order_inv = $query->row;
        ...
        if($result['order_id']){
            ...
            $bool =  $db->query($sql);
            $this->addOrderSortingHistory($result_order['order_id']);
            $sql = "update oc_x_deliver_order set order_status_id = 6,repack_status_id = 6 where order_id = '". $result_order['order_id'] ."'";
            $db->query($sql);
            foreach ($arr_deliver_order_id as $deliver_order_id){
                $this->addDeliverOrderSortingHistory($deliver_order_id);
            }
...
GROUP BY order_id 
	...
	SELECT
		...
		`frame_ice_list`,
		sum(box_count),
		'" . $result_deliver_order_inv['inv_comment'] . "',
		`inv_status`,
		NOW(),
		`date_modified`,
		`check_status`,
...
GROUP BY order_id 
            ...
            $bool = $db->query($sql_order_inv);
            $this->addOrderInvHistory($result_order['order_id']);

        }
        if($bool){

            $sql = "select frame_vg_list from oc_order_inv  where  order_id =  '".$result_order['order_id'] ."'" ;
            $query = $db->query($sql);
            $result_order_inv  = $query->row;

            $frame_vg_list= trim($result_order_inv['frame_vg_list'], ',');

            $sql = " update oc_order_inv set frame_vg_list = '".$frame_vg_list."' where order_id = '".$result_order['order_id'] ."' ";
            $bool =  $db->query($sql);
            $this->addOrderInvHistory($result_order['order_id']);
...
//                $dbm->getLastId();
    ...
    public  function addInvComment(array $data){
        ...
        $invComment = $data['data']['invComment']? $data['data']['invComment'] : 0;
        $user_id = $data['data']['user_id ']? $data['data']['user_id'] : 0;

        $bool = false;
        if($order_id && $invComment){
            $sql = "update  oc_order_inv set inv_comment = '".$invComment ."' WHERE  order_id = '".$order_id ."' ";
            $bool  = $db->query($sql);

            $this->addOrderInvHistory($order_id, $user_id);
        }
...
LEFT JOIN oc_product_to_warehouse ptw ON sci.product_id = ptw.product_id and  ptw.warehouse_id = '".$warehouse_id."'  WHERE sci.warehouse_id = '".$warehouse_id."' " ;
    ...
    public function getStockChecksMove(array $data){
        ...
        $warehouse_id = $data['data']['warehouse_id']? $data['data']['warehouse_id'] : '';
        $pallet_number = $data['data']['pallet_number']? $data['data']['pallet_number'] : '';
        $sql = " select sc.pallet_number , ws.name , w.username ,sc.status ,ws.code , sc.stock_check_id from oc_x_stock_checks sc LEFT join oc_x_warehouse_section ws on sc.stock_check_category = ws.warehouse_section_id LEFT  JOIN oc_w_user w on sc.added_by = w.user_id  where sc.pallet_number = '".$pallet_number."'   ";
             ...
             //   } else {
              ...
              VALUES ('" . $result['pallet_number'] . "' ,'2',UNIX_TIMESTAMP(NOW()), '22' ,NOW(),'" . $add_user . "' ,'" . $warehouse_id . "','搬仓出库')";
                    ...
                    if (!empty($results)) {
                        ...
                        $query = $db->query($sql);
                        $sql = "insert  into oc_x_inventory_move
              (`relevant_id`,`station_id`,`timestamp`,`inventory_type_id`,`date_added`,`add_user_name` ,`warehouse_id` ,`memo`,`date` )
              VALUES ('" . $result['pallet_number'] . "' ,'2',UNIX_TIMESTAMP(NOW()), '22' ,NOW(),'" . $add_user . "' ,'" . $warehouse_id . "','搬仓出库',NOW())";


                        $query = $db->query($sql);

                        $inventory_move_id = $db->getLastId();
                        $sql = "INSERT INTO `oc_x_inventory_move_item` (`inventory_move_id`, `station_id`, `product_id`, `quantity`, `status`,`warehouse_id`,`due_date`) VALUES ";
                        $m = 0;
                        foreach ($results as $product) {
                            $inventory_op = -1;
                            $sql .= "(" . $inventory_move_id . ", " . 2 . ",  '" . $product['product_id'] . "', '" . $product['quantity'] * $inventory_op . "',  1,'".$warehouse_id."',NOW() )";
    ...
    public  function addStockIn(array $data ){
        ...
        if($result['status'] == 2 || $result['status'] == 0){
            ...
            if($result_id['relevant_id'] > 0 ){
                ...
                $sql = "insert  into oc_x_stock_move
              (`relevant_id`,`station_id`,`timestamp`,`inventory_type_id`,`date_added`,`add_user_name` ,`warehouse_id` ,`memo`,`date` )
              VALUES ('" . $result['stock_check_id'] . "' ,'2',UNIX_TIMESTAMP(NOW()), '23' ,NOW(),'" . $add_user . "' ,'" . $warehouse_id . "','搬仓入库',NOW())";
                ...
                if (!empty($results)) {
                    ...
                    $sql = "insert  into oc_x_inventory_move
              (`relevant_id`,`station_id`,`timestamp`,`inventory_type_id`,`date_added`,`add_user_name` ,`warehouse_id` ,`memo` )
              VALUES ('" . $result['stock_check_id'] . "' ,'2',UNIX_TIMESTAMP(NOW()), '23' ,NOW(),'" . $add_user . "' ,'" . $warehouse_id . "','搬仓入库')";
                    ...
                    $m = 0;
                    foreach ($results as $product) {
                        $sql .= "(" . $inventory_move_id . ", " . 2 . ",  '" . $product['product_id']  . "', '" . $product['quantity'] . "',  1 ,'".$warehouse_id."',NOW())";
...
//            $sql =  "select  return_confirmed  , product_id  from oc_return_product  where warehouse_id = '".$warehouse_id."' and product_id = '".$product_id ."'  and return_confirmed = 0     ";
    ...
    function  getInventoryOrderSoring(array $data){
        ...
        $sql  =  "select doo.warehouse_id ,doo.order_status_id, doo.do_warehouse_id ,doo.order_id , doi.frame_vg_list   from oc_x_deliver_order  doo left join  oc_x_deliver_order_inv doi on doi.deliver_order_id = doo.deliver_order_id  where doo.deliver_order_id = '".$order_id."'" ;

        $query = $db->query($sql);
        $result = $query->row;
        if(intval($result['order_status_id']) == 6 || intval($result['order_status_id']) == 12){
...
//        $sql_order_status = " update  oc_order  set  order_status_id  = 6 where order_id = '".$result['order_id']."' ";
//        $query = $db->query($sql_order_status);
        if($query){
            return 1 ; 
        }else {
            return 2 ;
	...
	GROUP_CONCAT(DISTINCT IF (
			odo.order_deliver_status_id = 2
			AND odo.box_only = 0,
			'0',
			odo.relevant_id
		)) AS relevant_id,
	GROUP_CONCAT(DISTINCT bsi.deliver_order_id) AS deliver_order_id,
	GROUP_CONCAT(DISTINCT odo.order_status_id) AS order_status_ids,
	GROUP_CONCAT(DISTINCT IF(odo.order_deliver_status_id = 2 AND odo.box_only = 0,'1',odo.order_deliver_status_id)) AS order_deliver_status_ids,
	obs.deliver_date,
	obs. STATUS,
	obs.batch_id,
	w.title,
	doi.inv_comment,dos.name
FROM
	oc_x_batch_sorting obs
LEFT JOIN oc_x_batch_sorting_item bsi ON bsi.batch_id = obs.batch_id
LEFT JOIN oc_x_warehouse w ON w.warehouse_id = obs.warehouse_id
LEFT JOIN oc_x_deliver_order_inv doi ON doi.deliver_order_id = bsi.deliver_order_id
LEFT JOIN oc_x_deliver_order odo ON odo.deliver_order_id = bsi.deliver_order_id
LEFT JOIN oc_x_deliver_order_status dos ON odo.order_status_id = dos.order_status_id
WHERE
	obs.do_warehouse_id = '".$do_warehouse_id."'
AND obs.deliver_date = '".$deliver_date."'
AND obs.warehouse_id = '".$warehouse_id."'
";
        if($do_relevant_id == 0){
            $sql .= "  AND obs.`status` = 6 ";
        }else{
            $sql .= " AND obs.`status` = 7 " ;
        }
        $sql .= "
GROUP BY
	obs.batch_id HAVING order_status_ids IN (6,8) 
AND order_deliver_status_ids = '1' ";
...
//        return $result;
    ...
    function  addDoOrderRelevant(array $data){
        ...
        $check_value_lists = implode(',',$check_value);
        $sql = "SELECT
	GROUP_CONCAT(
		DISTINCT
		IF (
			trim(deliver_order_id) = '',
			NULL,
			deliver_order_id
		)
	) deliver_order_id
FROM
	oc_x_batch_sorting_item
WHERE
	batch_id IN (".$check_value_lists.")";
        $check_value_list = $dbm->query($sql)->row['deliver_order_id'];
        if (empty($check_value_list)) {
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => '所选整件波次单异常，请刷新页面后重试',
                'return_data' => ''
            );
            return $return;
        }
        $bool = true;
        $dbm->begin();
//return $check_value_lists;
        $sql = " insert into  oc_x_warehouse_requisition (`relevant_status_id` , `from_warehouse` , `to_warehouse`, `date_added` , `added_by` ,`status` ,  `out_type` ,`deliver_date`,`confirm_by`,`shipping_cost`,`warehouse_cost`,`total_cost`) values( 2, '".$do_warehouse_id."' , '".$warehouse_id."' , NOW(),'".$user_id."' ,  2 , '1'  , NOW(),'".$user_id."','".$shipping_cost."','".$warehouse_cost."','".($shipping_cost+$warehouse_cost)."')";
//return $sql;
        $bool && $dbm->query($sql);
        $requ_id = $dbm->getLastId();
        $this->insert_warehouse_requisition_history(2,$requ_id,$user_id);
...
//        $sql_item = "  insert into oc_x_warehouse_requisition_item ( `relevant_id` , `product_id` , `container_id`  , `num` , `container_x`  , `date_added`)
//  (select '".$requ_id."' ,  dop.product_id , ios.container_id , dop.quantity  , 'X'  , NOW()
//  from  oc_x_deliver_order_product dop
//  LEFT  join  oc_x_inventory_order_sorting   ios on dop.order_id  = ios.order_id  and  dop.product_id = ios.product_id
//   where ios.status = 1 and  dop.deliver_order_id in ($check_value_list)  and ios.container_id > 0  )";
//
//        $bool && $dbm->query($sql_item);

        $sql_item = "  insert into oc_x_warehouse_requisition_item ( `relevant_id` , `product_id` , `container_id`  , `num` , `date_added`)   
  (select '".$requ_id."' ,  ios.product_id , ios.container_id , sum(ios.quantity) quantity, NOW()  
  from  oc_x_inventory_order_sorting ios LEFT JOIN oc_product op ON op.product_id = ios.product_id  
  where ios.status = 1 AND  ios.deliver_order_id IN ($check_value_list) AND ios.container_id >= 0 AND ios.product_id >= 0 AND op.repack= 0 group by ios.product_id,ios.container_id )";
        $bool && $dbm->query($sql_item);

        $sql =  " update  oc_x_deliver_order   set  relevant_id = '".(int)$requ_id ."'  where deliver_order_id in ($check_value_list) ";
        $bool && $dbm->query($sql);

        $sql_status =  " update  oc_x_deliver_order set order_deliver_status_id  = '2'  where deliver_order_id in ($check_value_list) ";
        $bool && $dbm->query($sql_status);
        $sql_status =  " update  oc_x_batch_sorting set status  = '7'  where batch_id in ($check_value_lists) ";
        $bool && $dbm->query($sql_status);
        ...
        if($bool){
            ...
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => '整件波次单生成调拨单异常，请刷新页面后重试',
                'return_data' => $sql_item
            );
        }
        return $return;
    }
    ...
    function  confirmDoRelevant(array $data ){
        ...
        $sql = "select ios.order_id,ios.quantity,SUM(odo.quantity) as quantity2 from  oc_x_inventory_deliver_order_sorting ios left join oc_x_deliver_order_product odo on odo.deliver_order_id = ios.deliver_order_id and odo.product_id = ios.product_id where ios.warehouse_id = '".$warehouse_id."' and ios.product_id = '".$product_id."' and ios.deliver_order_id = '".$deliver_order_id."' GROUP BY ios.product_id";

        $query = $db->query($sql);
        $result = $query->row;
        if($result['order_id'] > 0 ){
            if (intval($result['quantity']) >= intval($result['quantity2'])) {
                return 3;
            } else {
                $sql = "update oc_x_inventory_deliver_order_sorting set quantity = '".intval($quantity)."' where warehouse_id = '".$warehouse_id."' and product_id = '".$product_id."' and deliver_order_id = '".$deliver_order_id."'";
...
//            }
            ...
            if ($query) {
                ...
                $sql =  " update  oc_x_deliver_order  set order_status_id  =  12,repack_status_id = 12 where order_id = '".$order_id ."' and order_status_id = 6 ";
                $query = $db->query($sql);
    ...
    function addConsolidatedRelevant(array $data )
    {
        ...
        $query = $db->query($sql);
        $result = $query->row;
        $return['order_status_id'] = $result['order_status_id'];
        ...
        $sql = "SELECT sum(xis.quantity) as quantity,odo.order_id FROM oc_x_inventory_order_sorting xis LEFT JOIN oc_x_deliver_order odo on odo.deliver_order_id = xis.deliver_order_id
WHERE
	odo.order_id = '".$result['order_id']."' and (odo.do_warehouse_id != odo.warehouse_id or (odo.do_warehouse_id = odo.warehouse_id and odo.is_repack = 1))
AND xis.status = 1 and  xis.move_flag = 0 ";
        $query = $db->query($sql);
        $result1 = $query->row;


        $sql = "SELECT
                    o.order_id order_id,os.`name`,SUM(op.quantity) as quantity,GROUP_CONCAT(DISTINCT o.deliver_order_id) deliver_order_ids
                FROM
                ...
                WHERE o.order_id = '" . $result['order_id']."' and (o.do_warehouse_id != o.warehouse_id or (o.do_warehouse_id = o.warehouse_id and o.is_repack = 1))
                group by o.order_id  ";
        ...
        $return['do_quantity'] = $result1['quantity'] ? $result1['quantity'] : 0;
        $return['order_id'] = $result['order_id'] ? $result['order_id'] : 0;
        $return['deliver_order_ids'] = $result_plan['deliver_order_ids'] ? $result_plan['deliver_order_ids'] : 0;
        //}
        if((int)$result['order_status_id'] == 3){
            $return['status'] = 2;

            return $return;
        }

        if((int)$result['order_status_id'] != 12){
            $return['status'] = 4;
            return $return;
        }
    ...
    function  addConsolidatedDoInfo(array $data ){
        global $db  ;
        $order_id = $data['data']['order_id']? (int)$data['data']['order_id'] : '';
        $deliver_order_ids = $data['data']['deliver_order_ids']? trim($data['data']['deliver_order_ids']) : '';
        $invComment = $data['data']['invComment']? $data['data']['invComment'] : '';
        ...
        $warehouse_id = $data['data']['warehouse_id']? $data['data']['warehouse_id'] : '';
        $inventory_user = $data['data']['inventory_user']? $data['data']['inventory_user'] : '';
        $add_user_name_id = $data['data']['add_user_name_id']? $data['data']['add_user_name_id'] : '';
...
//        $result_warehouse = $this->getDeliverOrderInfo($order_id);
        $deliver_order_id = explode(',',$deliver_order_ids);
        foreach ($deliver_order_id as $value) {
            $this->change_come_deliver_order_inv($warehouse_id,$value,1);
        }

        $data_inv['order_id'] = $order_id;

        $sql_consolidated = "select order_id from oc_x_consolidated_order where order_id = '". $data_inv['order_id'] ."' ";
        $query = $db->query($sql_consolidated);
        $result_consolidated = $query->row;
        if($result_consolidated['order_id'] > 0){
            return  false ;
        }
         $sql = "select  count(deliver_order_id) num  from oc_x_deliver_order where order_id = '".$data_inv['order_id']."' group by  order_id  ";
        $query = $db->query($sql);
        $result = $query->row;

        $sql_deliver_order_sorting = " select order_id , deliver_order_id from oc_x_inventory_deliver_order_sorting  where order_id = '".$data_inv['order_id']."'" ;
        
        $query = $db->query($sql_deliver_order_sorting);
        $result_deliver_order_sorting  = $query->row;

        /*插入退货统计表*/
        //异仓分拣本仓出库
//        if (intval($warehouse_id) == 17) {
            if($result_deliver_order_sorting['deliver_order_id'] > 0 ) {
                $return = $this->insert_into_return_quantity($data_inv['order_id'],1,$add_user_name_id);
            }
            //多仓分拣
            if($result['num'] > 1){
                $return = $this->insert_into_return_quantity($data_inv['order_id'],2,$add_user_name_id);
            }
//        } else {
//            if($result['num']  > 1 && $result_deliver_order_sorting['deliver_order_id'] > 0 ){
//                //写入oc_x_inventory_order_return_quantity
//                $sql_return_quantity = " insert into oc_x_inventory_order_return_quantity ( `order_id` , `deliver_order_id` , `product_id` , `order_quantity` , `sorting_quantity` ,`return_quantity` , `uptime` , `added_by` )(SELECT
//                ios.order_id ,
//                ios.deliver_order_id  ,
//	ios.product_id ,
//	sum(dop.quantity)  order_quantity ,
//  if(ios.quantity > sum(dop.quantity)  , sum(dop.quantity) ,ios.quantity )  sorting_quantity ,
//  (SUM(dop.quantity) - ios.quantity) return_quantity  ,
//  NOW(),
//  '".$data['add_user_name_id']."'
//FROM
//	oc_x_inventory_deliver_order_sorting ios
//LEFT JOIN oc_x_deliver_order_product dop ON ios.deliver_order_id = dop.deliver_order_id
//and ios.product_id  =  dop.product_id
//where   ios.deliver_order_id  = '".$result_deliver_order_sorting['deliver_order_id']."' and ios.container_id <= 0
//GROUP BY  ios.product_id) ";
//
//                $query = $db->query($sql_return_quantity);
//
//
//                $sql_return_quantity_container = " insert into oc_x_inventory_order_return_quantity ( `order_id` , `deliver_order_id` , `product_id` , `order_quantity` , `sorting_quantity` ,`return_quantity` , `uptime` , `added_by` )(SELECT
//                ios.order_id ,
//                ios.deliver_order_id  ,
//	ios.product_id ,
//	sum(dop.quantity)  order_quantity ,
...
//  '".$data['add_user_name_id']." '
//FROM
//oc_x_deliver_order_product dop
//LEFT JOIN (SELECT order_id,deliver_order_id,status,container_id,product_id,SUM(quantity) AS quantity FROM oc_x_inventory_order_sorting WHERE deliver_order_id = '".$result_deliver_order_sorting['deliver_order_id']."' AND status= 1 GROUP BY deliver_order_id,product_id,container_id ) ios on ios.deliver_order_id = dop.deliver_order_id  and ios.product_id = dop.product_id and ios.status = 1
//LEFT JOIN oc_x_inventory_deliver_order_sorting dos  ON dos.deliver_order_id = dop.deliver_order_id
//and dos.container_id  =  ios.container_id
//where  ios.deliver_order_id  = '".$result_deliver_order_sorting['deliver_order_id']."' and dos.container_id > 0
//GROUP BY  ios.product_id) ";
//
//                $query = $db->query($sql_return_quantity_container);
//
//
//                $sql_return_sorting = " insert into oc_x_inventory_order_return_quantity ( `order_id` , `deliver_order_id` , `product_id` , `order_quantity` , `sorting_quantity` ,`return_quantity` , `uptime` , `added_by` )(SELECT
//                ios.order_id ,
//                ios.deliver_order_id  ,
//	ios.product_id ,
//	sum(dop.quantity)  order_quantity ,
...
//  '".$data['add_user_name_id']."'
//FROM
//	oc_x_deliver_order dos
//LEFT JOIN (SELECT order_id,deliver_order_id,status,container_id,product_id,SUM(quantity) AS quantity FROM oc_x_inventory_order_sorting WHERE order_id = '".$data_inv['order_id']."' AND status= 1 GROUP BY deliver_order_id,product_id,container_id ) ios on dos.deliver_order_id = ios.deliver_order_id
//LEFT JOIN oc_x_deliver_order_product dop ON ios.deliver_order_id = dop.deliver_order_id
//and ios.product_id  =  dop.product_id
//where  ios.status = 1  and ios.order_id  = '".$data_inv['order_id']."' and dos.warehouse_id = dos.do_warehouse_id
//GROUP BY  ios.product_id) ";
//
//                $query = $db->query($sql_return_sorting);
//
//
//            }
//
//            if($result['num']  > 1 && $result_deliver_order_sorting['deliver_order_id']   == '' ){
//                $sql_return_sorting = " insert into oc_x_inventory_order_return_quantity ( `order_id` , `deliver_order_id` , `product_id` , `order_quantity` , `sorting_quantity` ,`return_quantity` , `uptime` , `added_by` )(SELECT
//                ios.order_id ,
//                ios.deliver_order_id  ,
//	ios.product_id ,
//	sum(dop.quantity)  order_quantity ,
//  if(SUM(ios.quantity) > dop.quantity  , dop.quantity , SUM(ios.quantity) )  sorting_quantity ,
//
//  (dop.quantity - sum(ios.quantity)) return_quantity  ,
//  NOW(),
//  '".$data['add_user_name_id']."'
//FROM
//	oc_x_deliver_order dos
//LEFT JOIN oc_x_inventory_order_sorting ios on dos.deliver_order_id = ios.deliver_order_id
//LEFT JOIN (SELECT deliver_order_id,order_id,price,sum(quantity) AS quantity ,product_id  FROM oc_x_deliver_order_product WHERE order_id = '".$data_inv['order_id']."' GROUP BY deliver_order_id,product_id ) dop ON ios.deliver_order_id = dop.deliver_order_id
//and ios.product_id  =  dop.product_id
//where  ios.status = 1  and ios.order_id  = '".$data_inv['order_id']."' and dos.warehouse_id = dos.do_warehouse_id
//GROUP BY  ios.product_id) ";
//
...
//        }
        ...
        $sql_deliver_order_inv  = "  select inv_comment   from oc_x_deliver_order doo LEFT   join oc_x_deliver_order_inv doi on doo.deliver_order_id = doi.deliver_order_id  where doo.do_warehouse_id = '".$warehouse_id."' and doo.is_repack = 0 and doo.order_id = '".$data_inv['order_id']."' ";
	...
	SELECT
		...
		`frame_meat_count`,
		`foam_ice_count`,
		GROUP_CONCAT(IF(trim(frame_vg_list) = '' OR frame_vg_list = '' OR ISNULL(frame_vg_list),NULL,frame_vg_list)),
		`frame_mi_list`,
		`frame_meat_list`,
		`frame_ice_list`,
		sum(box_count),
	...
	FROM
		oc_x_deliver_order_inv
	WHERE
		 order_id  = '".$data_inv['order_id']."'
GROUP BY order_id ) ";

                $bool =  $db->query($sql_order_inv);
                $this->addOrderInvHistory($data_inv['order_id'],$add_user_name_id);
            ...
            if ($order_inv_order_id['order_id'] > 0) {
                if ($result['num'] == 1) {
                    $this->change_come_deliver_order_inv($warehouse_id,$deliver_order_ids,2);
                }
...
GROUP BY order_id ) ";
        ...
        if($result['num'] == 1 ){
            $sql_return_quantity = "select order_id  from oc_x_inventory_order_return_quantity where order_id  =   '". $data_inv['order_id'] ."'  ";
            $query = $db->query($sql_return_quantity);
            $result_return_quantity   = $query->row;
            if($result_return_quantity['order_id'] > 0 ) {
...
//            $sql = "update oc_x_deliver_order set order_status_id = 6,repack_status_id = 6 where order_id = '" . $data_inv['order_id'] . "'";


//            $bool = $db->query($sql);

//            $this->addOrderSortingHistory($data_inv['order_id'],$add_user_name_id);
//            foreach () {}

//            $this->addDeliverOrderSortingHistory($order_id,$add_user_name_id,2);
//            return array('status' => 1, 'timestamp' => $data_inv['timestamp']);
        ...
        if($bool){
            ...
            $sql = "select frame_vg_list from oc_order_inv  where  order_id =  '".$data_inv['order_id'] ."'" ;
            $query = $db->query($sql);
            $result_order_inv  = $query->row;
            $container_ids = explode(',',$result_order_inv['frame_vg_list']);
            $frame_vg_list = '';
            if (is_array($container_ids)) {
                foreach ($container_ids as $value) {
                    if ((int)$value > 0) {
                        $frame_vg_list .= ','.$value;
                    }
                }
            ...
            $frame_vg_list= trim($frame_vg_list, ',');

            $sql = " update oc_order_inv set frame_vg_list = '".$frame_vg_list."' where order_id = '".$data_inv['order_id'] ."' ";
            $bool =  $db->query($sql);
            $this->addOrderInvHistory($data_inv['order_id']);
...
//return $data;
        ...
        $warehouse_repack = empty($data['data']['warehouse_repack'])? 0: intval($data['data']['warehouse_repack']);
...
IF (doo.is_urgent = 1, '1', '0') urgent,
 ...
 (
   ...
   WHERE  doo.do_warehouse_id = '".$warehouse_id ."' and doo.warehouse_id = '".$to_warehouse_id ."' 	AND o.with_fresh = 0 and (doo.do_warehouse_id = doo.warehouse_id OR (doo.do_warehouse_id != doo.warehouse_id AND doo.box_only != 1)) and doo.order_status_id =  2  ";
...
//return $sql;
        ...
        $return[] = empty($results[$a])?[]:$results[$a];
        return  $return;
    }
    /*zx
    待合单查询*/
    public  function  get_merge_order_status(array $data ){
        global $db;
        $data = json_decode($data, 2);
        $deliver_date = $data['data']['deliver_date']? $data['data']['deliver_date'] : '';
...
and doo.deliver_date = '".$deliver_date."' 
GROUP BY doo.deliver_order_id ORDER BY doo.order_id";
...
GROUP BY doo.order_id";
    ...
    private function addOrderInvHistory($order_id, $added_by=0){
        global $db;
        $sql = "insert into oc_order_inv_log(order_id, box_count, inv_comment, frame_count, frame_vg_list, inv_status, uptime,added_by) 
                select order_id, box_count, inv_comment, frame_count, frame_vg_list, 1, NOW(),'".$added_by."' from oc_order_inv where order_id = '".$order_id."'
                ";

        return $db->query($sql);
    }

    private function addDeliverOrderSortingHistory($order_id, $added_by=0,$type){
        ...
        $sql = "INSERT INTO oc_x_deliver_order_history (`deliver_order_id`, `notify`, `comment`, `date_added`, `order_status_id`, `order_deliver_status_id`, `modified_by`)
                select deliver_order_id, 0, '[SYS]订单分拣', now(),  order_status_id,  order_deliver_status_id, '".$added_by."' from oc_x_deliver_order where ";
        if ($type == 2) {
            $sql .= " order_id = '".$order_id."'";
        } else {
            $sql .= " deliver_order_id = '".$order_id."'";
        }
...
//        }
    ...
    public  function updateDoStatus(array $data ){
        ...
        $sql_order = "select o.order_status_id ,GROUP_CONCAT(DISTINCT doo.order_status_id) do_order_status_id  ,doo.order_id from  oc_x_deliver_order doo left join oc_order o on doo.order_id = o.order_id where doo.order_id = '".$order_id."' ";
        ...
        if($resutl['order_status_id'] == 6  || $resutl['do_order_status_id'] !=6 ) {
            ...
            $sql1 = " select GROUP_CONCAT(DISTINCT deliver_order_id) deliver_order_id  from oc_x_deliver_order where  order_id = '".$resutl['order_id']."' and do_warehouse_id != warehouse_id  ";
            $query = $db->query($sql1);
            $resutl1 = $query->row;

            if($resutl1['deliver_order_id'] > 0 ){
                $sql = "update oc_x_inventory_order_return_quantity set status = 0  where deliver_order_id  IN(".$resutl1['deliver_order_id'].") ";
                $query = $db->query($sql);
            }

            $sql = " update oc_x_deliver_order set order_status_id = 12  where  order_id  ='".$order_id."' and warehouse_id = '".$warehouse_id."'  ";
            $query = $db->query($sql);
            if($query){
                $deliver_order_ids = explode(',',$resutl1['deliver_order_id']);
                foreach ($deliver_order_ids as $value) {
                    $this->addDeliverOrderSortingHistory($value);
                }
...
//            $query = $db->query($sql);
  ...
  // 不通过调拨单合单
    public  function  getContainerDeliver(array $data){
        ...
        $sql = "  select do.deliver_order_id  , do.order_id    from  oc_x_deliver_order do LEFT  JOIN  oc_x_deliver_order_inv doi on  do.deliver_order_id = doi.deliver_order_id left JOIN  oc_order_distr  od  on od.deliver_order_id = doi.deliver_order_id where (doi.frame_vg_list like '%,$count_id%' or  doi.frame_vg_list in ($count_id) ) and do.warehouse_id = '".$warehouse_id."'  and do.order_status_id  in (6,12)   order by doi.order_inv_id  desc limit 1 ";

        $query = $db->query($sql);
        $result = $query->row;


        $sql_order = "select $count_id  container_id,   doo.deliver_order_id , doo.order_id , IF (

	IF (
		doo.warehouse_id = doo.do_warehouse_id,
		doi.inv_comment,
		'1'
	) = 1,
	doi.inv_comment,
...
doo.deliver_order_id , 
doo.order_id , 
min(doi.inv_comment) inv_comment,
dop.product_id ,
...
where dop.product_id =  '".$product_id ."' and doo.warehouse_id = '".$warehouse_id ."' and  o.order_status_id  in (5,8)  ";
...
left join oc_order o on doo.order_id = o.order_id 
left join oc_product p on p.product_id = dop.product_id 
where dop.product_id =  '".$result['product_id'] ."' and doo.warehouse_id = '".$warehouse_id ."' and  o.order_status_id  in (5,8)  ";
...
GROUP BY owri.relevant_id";
    ...
    public function get_frame_vg_list_status(array $data){
        ...
        $sql = "SELECT odd1.warehouse_id,odd1.do_warehouse_id,odd1.is_repack,COUNT(odd2.deliver_order_id) as order_count,odd1.box_only  
FROM oc_x_deliver_order odd1
LEFT JOIN oc_x_deliver_order odd2 ON odd2.order_id = odd1.order_id
WHERE odd1.deliver_order_id = ".$deliver_order_id;
        $query = $db->query($sql);
        $result = $query->row;
        return $result;
    }
    ...
    function getTransferBoxes(array $data){
        ...
        if(!$deliverDate || !$doWarehouseId || !$toWarehouseId){
            ...
            if ($type == 1) {
             ...
             LEFT JOIN oc_order oo ON oo.order_id = o.order_id 
              LEFT JOIN oc_x_deliver_order_inv oi ON o.deliver_order_id = oi.deliver_order_id
              WHERE o.order_status_id IN (6,8,12)  AND o.deliver_date = '".$deliverDate."' AND o.do_warehouse_id = '".$doWarehouseId."' AND o.warehouse_id = '".$toWarehouseId."' AND oo.order_status_id != 3 AND oo.order_status_id != 6 
            ";
            //return $sql;
            ...
            foreach($results as $m){
                ...
                //可调拨订单及商品
                $sql = "
SELECT o.order_id, SUM(odop.quantity) AS quantity,odop.deliver_order_id, odop.product_id,ptw.sku_barcode, o.is_repack, o.do_warehouse_id,o.warehouse_id,op.name,GROUP_CONCAT(o.deliver_order_id) AS deliver_order_id,GROUP_CONCAT(oi.inv_comment) AS inv_comment,GROUP_CONCAT(odop.quantity) AS products_num
FROM oc_x_deliver_order o
LEFT JOIN oc_order oo ON oo.order_id = o.order_id
LEFT JOIN oc_x_deliver_order_inv oi ON o.deliver_order_id = oi.deliver_order_id
LEFT JOIN oc_x_deliver_order_product odop ON o.deliver_order_id = odop.deliver_order_id 
...
WHERE o.order_status_id IN (6,12) AND o.order_deliver_status_id = 1 
AND o.deliver_date = '".$deliverDate."' 
AND o.do_warehouse_id = '".$doWarehouseId."' 
AND o.warehouse_id = '".$toWarehouseId."'
AND oo.order_status_id != 3 AND oo.order_status_id != 6
...
//                return $sql;
    ...
    function addTransferOrder(array $data){
        ...
        //添加主记录
        $sql = "INSERT INTO oc_x_warehouse_requisition (`relevant_status_id`,`from_warehouse`,`to_warehouse`,`date_added`,`added_by`,`status`,`out_type`,`deliver_date`,`shipping_cost`,`warehouse_cost`,`total_cost`)
        VALUES ( 2,'".$doWarehouseId."','".$toWarehouseId."',NOW(),'".$userId."',1,1,'".$deliverDate."','".$shipping_cost."','".$warehouse_cost."','".($shipping_cost+$warehouse_cost)."')";
//return $sql;
        $bool = $bool && $dbm->query($sql);
        $reqId = $dbm->getLastId();
    ...
    function getTransferOrder(array $data){
        ...
        $sql = "SELECT
                ...
                if(b.container_id=0 AND b.product_id >0, SUM(b.num),COUNT(DISTINCT IF(b.container_id>0,b.container_id,NULL))) num
                FROM oc_x_warehouse_requisition a
                LEFT JOIN oc_x_warehouse_requisition_item b  ON a.relevant_id = b.relevant_id
     ...
     *
     * */
    public function searchRequisitionNew( array $data){
        global  $db;
        $date_start = isset($data['data']['date_start']) ? $data['data']['date_start'] : false;
        $date_end = isset($data['data']['date_end']) ? $data['data']['date_end'] : false;
        $filter_out_type = isset($data['data']['filter_out_type']) ? $data['data']['filter_out_type'] : false;
        $filter_out_type_id = 3;
        ...
        if($filter_out_type_id){
            $sql .= " and wr.out_type = '".$filter_out_type_id ."'";
        }
    ...
    /*zx
    仓内调拨单*/
    public function startShipmentNew(array  $data){
        global  $db;
        ...
        //获取该出库单的状态以及所有商品ID
        $sql = "SELECT  wr.relevant_id ,wr.relevant_status_id ,wri.product_id,wri.comment FROM oc_x_warehouse_requisition wr  LEFT JOIN oc_x_warehouse_requisition_item wri ON  wri .relevant_id = wr.relevant_id  WHERE  wr.relevant_id = '".$relevant_id."'
        ORDER BY wri.product_id";
        $query = $db->query($sql);
        $results = $query->rows;
        ...
        $product_ids2 = '';
        foreach($results  as $result){
            $product_ids1 .= ','.($result['product_id']);                           //需要转换的商品
            $product_ids2 .= ','.explode(':',$result['comment'])[0];      //需要被转换的商品
        }
        $product_ids1 = ltrim($product_ids1, ",");
//        return $product_ids1.'@'.$product_ids2;
        $sql = "SELECT  GROUP_CONCAT(IF(wrt.relevant_status_id = 2,wrt.product_id,NULL) ORDER BY wrt.product_id) AS product_id1,
GROUP_CONCAT(IF(wrt.relevant_status_id = 4,wrt.product_id,NULL)) AS product_id2
FROM oc_x_warehouse_requisition_temporary wrt WHERE wrt.relevant_id = '".$relevant_id ."' AND
wrt.product_id IN(".$product_ids1.$product_ids2.")";
//        return $sql;
        $invalid =$db->query($sql)->row;
//        return $invalid;
        $invalidProducts2 = $invalid['product_id1'];                                        //临时表的需要转换的商品
        $submit_products = explode(',',$invalid['product_id2']);                 //临时表的被转换的商品
        $all_products = explode(',',ltrim($product_ids2,','));            //全部需要被转换的商品
        $product_ids1 = explode(',',$product_ids1);                                //全部需要转换的商品
        $submit_product_id = [];                                                             //临时表的被转换的商品对应的需要转换的商品
        foreach ($product_ids1 as $key => $value1){

            //全部被转换的对比临时表的被转换的商品，如果存在就记入对应的需要转换的商品
            foreach ($submit_products as $value2){
                if (intval($value2) == intval($all_products[$key])) {
                    $submit_product_id[] = $value1;
                }
            }

            //全部对比临时表的需要转换的商品，如果存在就去掉，剩下即为不在临时表的
            foreach (explode(',',$invalidProducts2) as $value3){
                if (intval($value1) == intval($value3)) {
                    unset($product_ids1[$key]);
                }
            }
        }
        $invalidProducts = join($product_ids1,',');                                     //不在临时表的

        if($invalidProducts){
            //没插入临时表时的取值
            $sql2 = "SELECT wri.product_id ,wri.relevant_id ,p.name ,pi.inventory ,wri.num ,wri.scale_num,ps.stock_area ,w.title,wr.out_type,wri.comment,ps.sku_barcode,0 AS status
            FROM oc_x_warehouse_requisition_item wri
            LEFT JOIN  oc_x_warehouse_requisition wr ON wr.relevant_id = wri.relevant_id
            LEFT JOIN  oc_product_to_warehouse  ps ON wri.product_id = ps.product_id AND ps.warehouse_id = wr.from_warehouse    
            LEFT JOIN  oc_x_warehouse w ON  wr.to_warehouse = w.warehouse_id
            LEFT JOIN  oc_product p ON wri.product_id = p.product_id 
            LEFT JOIN  oc_product_inventory pi ON wri.product_id = pi.product_id AND wr.from_warehouse = pi.warehouse_id
            WHERE  wri.relevant_id = '".$relevant_id."' AND  wri.product_id IN (".$invalidProducts .")
        ...
        if($invalidProducts2){
            //查询插入临时表的信息
            $sql1 = " SELECT   wrt.product_id ,p.name, ps.stock_area,pi.inventory,wri.num  ,wrt.quantity,wr.out_type,wri.comment,wri.scale_num,ps.sku_barcode,1 AS status
                FROM oc_x_warehouse_requisition_temporary wrt
                LEFT JOIN oc_x_warehouse_requisition_item wri ON  wrt.relevant_id = wri.relevant_id AND wrt.product_id = wri.product_id
                LEFT JOIN  oc_x_warehouse_requisition wr ON wr.relevant_id = wri.relevant_id
                LEFT JOIN oc_product p ON  p.product_id = wrt.product_id
                LEFT JOIN  oc_product_to_warehouse ps ON  ps.product_id = wrt.product_id AND ps.warehouse_id = wr.from_warehouse
                LEFT JOIN  oc_product_inventory pi ON wri.product_id = pi.product_id AND wr.from_warehouse = pi.warehouse_id
                WHERE wrt.relevant_id = '".$relevant_id ."' AND
                wrt.product_id IN (".$invalidProducts2 .")
                GROUP BY  wrt.product_id";
            ...
            $result1 = $query1->rows;
            $result2 = [];
//            return $submit_product_id;
            foreach ($result1 as $vau1) {
                $result2[$vau1['product_id']] =$vau1;
                $result2[$vau1['product_id']]['submit_status'] = 0;
                $result2[$vau1['product_id']]['product_id2'] = explode(':',$vau1['comment'])[0];
                foreach ($submit_product_id as $vau2) {
                    if (intval($vau1['product_id']) == intval($vau2)) {
                        $result2[$vau1['product_id']]['submit_status'] = 1;
                    }
                }
            }
            $return['product1'] = $result2;
        }
    ...
    /*zx
     * 仓内调拨单内容*/
    public function makeGetReadyLists(array  $data){
        global  $db;
        ...
        $warehouse_id = isset($data['data']['warehouse_id']) ? trim($data['data']['warehouse_id']) : false;
        $product_id2 = isset($data['data']['product_id2']) ? trim($data['data']['product_id2']) : false;

//查询插入临时表的信息
        $sql1 = "SELECT wrt.product_id,wri.relevant_id,p.name,ps.sku_barcode,ps.stock_area AS istock_area,pi.inventory,wri.num,wrt.quantity,wr.out_type,wri.comment,wri.scale_num,wrt.stock_area,w.title,p.inv_size AS box_size 
FROM oc_x_warehouse_requisition_temporary wrt
LEFT JOIN oc_x_warehouse_requisition_item wri ON wrt.relevant_id = wri.relevant_id AND wrt.product_id = wri.product_id 
OR wrt.product_id = '".$product_id2."' AND wrt.relevant_id = wri.relevant_id 
LEFT JOIN oc_x_warehouse_requisition wr ON wr.relevant_id = wri.relevant_id
LEFT JOIN oc_product p ON p.product_id = wrt.product_id
LEFT JOIN oc_x_warehouse w ON wr.to_warehouse = w.warehouse_id
LEFT JOIN oc_product_to_warehouse ps ON  ps.product_id = wrt.product_id AND ps.warehouse_id = wr.from_warehouse 
LEFT JOIN oc_product_inventory pi ON ps.product_id = pi.product_id AND wr.from_warehouse = pi.warehouse_id  
WHERE wrt.relevant_id = '".$relevant_id ."' 
AND wri.product_id = '".$product_id ."'";
//return $sql1;
        $query1 = $db->query($sql1);
        $product1 = $query1->rows;
        //没插入临时表时的取值
        $sql2 = "SELECT ps.product_id,wri.relevant_id,p.name,pi.inventory,wri.num,wri.scale_num,ps.sku_barcode,ps.stock_area AS istock_area,w.title,wr.out_type,wri.comment,p.inv_size AS box_size 
FROM oc_x_warehouse_requisition_item wri
LEFT JOIN oc_x_warehouse_requisition wr ON wr.relevant_id = wri.relevant_id
LEFT JOIN oc_product_to_warehouse ps ON wri.product_id = ps.product_id AND ps.warehouse_id = wr.from_warehouse 
OR ps.product_id = '".$product_id2."' AND wr.from_warehouse = ps.warehouse_id
LEFT JOIN oc_x_warehouse w ON wr.to_warehouse = w.warehouse_id
...
AND wri.product_id = '".$product_id ."'";
        ...
        $query2 = $db->query($sql2);
        $product2 = $query2->rows;
//        return $product1;
        $return['products'] = [];
        foreach ($product2 as $value2) {
            $return['products'][$value2['product_id']] = $value2;
        }
        foreach ($product1 as $value1) {
            $return['products'][$value1['product_id']] = $value1;
        }
     ...
     *
     * */
    public function submitProductNew( array $data){
        global  $db;
        $relevant_id = isset($data['data']['relevant_id']) ? trim($data['data']['relevant_id']) : false;
        $product_id = isset($data['data']['product_id']) ? trim($data['data']['product_id']) : false;
        $product_num = isset($data['data']['product_num']) ? trim($data['data']['product_num']) : false;
        $product_stock_area = isset($data['data']['product_stock_area']) ? trim($data['data']['product_stock_area']) : false;
        $status = isset($data['data']['status']) ? trim($data['data']['status']) : false;
        $warehouse_id = isset($data['data']['warehouse_id']) ? trim($data['data']['warehouse_id']) : false;
//        return $data;
        //获取价格以及出库单状态
        $sql = " SELECT wr.relevant_status_id ,wri.price
FROM  oc_x_warehouse_requisition wr
LEFT JOIN  oc_x_warehouse_requisition_item wri ON  wr.relevant_id = wri.relevant_id
WHERE wri.relevant_id = '". $relevant_id ."' AND wri.product_id = '". $product_id."' ";
...
WHERE  wrt.relevant_id = '".$relevant_id."' 
AND wrt.product_id = '". $product_id."' 
AND wrt.warehouse_id = '". $warehouse_id ."'";
        $query1 = $db->query($sql1);

        //是否插入中间表执行不同的sql
        if ($query1->num_rows) {
            //更新中间表的数据
            $sql = "UPDATE oc_x_warehouse_requisition_temporary 
SET quantity = '".$product_num."',
stock_area = '".$product_stock_area ."'
WHERE  relevant_id = '".$relevant_id."' 
AND product_id = '". $product_id."' 
AND relevant_status_id = '".$status."' 
...
//return $sql;
            ...
            //插入中间表
            $sql = " INSERT INTO oc_x_warehouse_requisition_temporary
                ( `relevant_id`,`relevant_status_id`,`product_id`,`price`,`quantity`,`warehouse_id`,`date_added`,`stock_area`)
                VALUES ('".$relevant_id ."','".$status."','".$product_id."','".$result['price']."', '".$product_num."','".$warehouse_id."',NOW(),'".$product_stock_area."')";
//return $sql;
            $query = $db->query($sql);
     ...
     *
     * */
    public function submitProductsNew(array $data){
        global $db;
        $relevant_id = isset($data['data']['relevant_id']) ? $data['data']['relevant_id'] : false;
        $warehouse_user = isset($data['data']['warehouse_user']) ? $data['data']['warehouse_user'] : false;
        $warehouse_id = isset($data['data']['warehouse_id']) ? $data['data']['warehouse_id'] : false;
//        return $data;
        $sql = "SELECT wr.relevant_status_id,wr.out_type,COUNT(wri.product_id) AS count1
FROM oc_x_warehouse_requisition wr 
LEFT JOIN oc_x_warehouse_requisition_item wri ON wri.relevant_id = wr.relevant_id
WHERE wr.relevant_id = '".$relevant_id."'";

        $query = $db->query($sql);
        $result = $query->row;
        $sql2 = "SELECT COUNT(IF(wrt.relevant_status_id = 4,wrt.product_id,NULL)) AS count2,
GROUP_CONCAT(IF(wrt.relevant_status_id = 2,wrt.product_id,NULL),'@',IF(wrt.relevant_status_id = 2,wrt.quantity,NULL)) AS product_ids1,
GROUP_CONCAT(IF(wrt.relevant_status_id = 4,wrt.product_id,NULL),'@',IF(wrt.relevant_status_id = 4,wrt.quantity,NULL)) AS product_ids2
FROM oc_x_warehouse_requisition_temporary wrt 
WHERE wrt.relevant_id = '".$relevant_id."'";
//        return $sql2;
        $query2 = $db->query($sql2);
        $result2 = $query2->row;
//        return $result2;
        if (intval($result['out_type']) == 3) {

            if (intval($result['relevant_status_id']) == 2) {

                if (intval($result['count1']) == intval($result2['count2'])) {
                    ...
                    $db->query($sql);
                    $return = $this->insert_warehouse_requisition_history(6,$relevant_id,$warehouse_user);
                    if ($this->check_result_is_true($return)) {
                        return $return;
                    }
                    /*
                    ...
                    $text1 = "仓内调拨单";
                    $text2 = "仓内调拨单";
                    $product_ids1 = explode(',',$result2['product_ids1']);
                    $product_ids2 = explode(',',$result2['product_ids2']);
                    foreach ($product_ids1 as $value1) {
                        $products1 = explode('@',$value1);
                        $stock_move_item1[] = "'".$station_id."','".$products1[0]."','".(0-intval($products1[1]))."','".$warehouse_id."'),";
                        $inventory_move_item1[] = "'".$station_id."',NOW(),'".$products1[0]."','".(0-intval($products1[1]))."','".$warehouse_id."'),";
                    }
                    /*
                     * 出库前后台库存扣除
                     * */
                    $stock_move1 = "('".$relevant_id."','".$station_id."',UNIX_TIMESTAMP(NOW()),'".$inventory_type_id."',NOW(),'".$warehouse_user."','".$warehouse_id."','".$text1."')";
                    $id1 = ['relevant_id' => $relevant_id,'inventory_type_id' => $inventory_type_id];
                    $return = $this->insert_into_stock_move($stock_move1,$stock_move_item1,$id1);
                    if ($this->check_result_is_true($return)) {
                        return $return;
                    }

                    $inventory_move1 = "('".$relevant_id."','".$station_id."',current_date(),UNIX_TIMESTAMP(),'".$from_station_id."','".$inventory_type_id."',NOW(),'".$warehouse_user."','".$text1."','".$warehouse_id."')";
                    $id1 = ['relevant_id' => $relevant_id,'inventory_type_id' => $inventory_type_id];
                    $return = $this->insert_into_inventory_move($inventory_move1,$inventory_move_item1,$id1);
                    if ($this->check_result_is_true($return)) {
                        return $return;
                    }

                    foreach ($product_ids2 as $value2) {
                        $products2 = explode('@',$value2);
                        $stock_move_item2[] = "'".$station_id."','".$products2[0]."','".(intval($products2[1]))."','".$warehouse_id."'),";
                        $inventory_move_item2[] = "'".$station_id."',NOW(),'".$products2[0]."','".(intval($products2[1]))."','".$warehouse_id."'),";
                    }

                    /*
                     * 入库加前后台库存
                     * */
                    $stock_move2 = "('".$relevant_id."','".$station_id."',UNIX_TIMESTAMP(NOW()),'".$inventory_type_id1."',NOW(),'".$warehouse_user."','".$warehouse_id."','".$text2."')";
                    $id2 = ['relevant_id' => $relevant_id,'inventory_type_id' => $inventory_type_id1];
                    $return = $this->insert_into_stock_move($stock_move2,$stock_move_item2,$id2);
                    if ($this->check_result_is_true($return)) {
                        return $return;
                    }

                    $inventory_move2 = "('".$relevant_id."','".$station_id."',current_date(),UNIX_TIMESTAMP(),'".$from_station_id."','".$inventory_type_id1."',NOW(),'".$warehouse_user."','".$text2."','".$warehouse_id."')";
                    $id2 = ['relevant_id' => $relevant_id,'inventory_type_id' => $inventory_type_id1];
                    $return = $this->insert_into_inventory_move($inventory_move2,$inventory_move_item2,$id2);
                    if ($this->check_result_is_true($return)) {
                        return $return;
                    }
    ...
    function getOrderContainerHistory(array $data){
        ...
        $count_id = $data['data']['count_id']? $data['data']['count_id'] : '';
        $warehouse_id = $data['data']['warehouse_id']? $data['data']['warehouse_id'] : '';
        $search_type = $data['data']['search_type']? (int)$data['data']['search_type'] : 0;
        $container_type = $data['data']['container_type']? (int)$data['data']['container_type'] : 0;
        if ($container_type == 2) {
            $date_s1 = date("Y-m-d",(time()-3600*24*3));
            $date_s2 = date("Y-m-d",(time()-3600*24*9));
            $date_e1 = date("Y-m-d",(time()-3600*24*1));
            $date_e2 = date("Y-m-d",(time()+3600*24*2));
            $sql = "select oo.order_id 订单号, group_concat(distinct concat('[', if(oo.do_warehouse_id=oo.warehouse_id, w.shortname, dw.shortname),']', oo.deliver_order_id,'-',oos.name)) 分拣单,  
if(oi.inv_comment is null, '', oi.inv_comment) 货位号, 
CC.containerList 分拣周转筐,if(doi.inv_comment is null, '', doi.inv_comment) 本仓分拣单货位,
os.name 订单状态, ds.name 配送状态,
o.date_added 下单时间, o.deliver_date 配送日期,
if(sm.inventory_move_id>0, '已出库','') 有出库记录,  
la.logistic_driver_title 司机,ar.name 区域
...
left join oc_x_logistic_allot la on lao.logistic_allot_id = la.logistic_allot_id
inner join (
	select a.order_id, group_concat(distinct container_id) containerList from oc_x_inventory_order_sorting a
	where a.uptime between '".$date_s1."' and '".$date_e1."'
	and a.container_id in (".$count_id.")
	group by a.order_id
) CC on oo.order_id = CC.order_id
where o.order_status_id !=3 and o.deliver_date between '".$date_s2."' and '".$date_e2."'
...
//            return $sql;
        ...
        if ($search_type>0) {
            $sql  =  " select do.deliver_order_id  , do.deliver_date, do.order_id , doi.uptime ,doi.inv_comment , doi.deliver_order_id , od.inventory_name ,CONCAT(doo.container_id,',',doi.frame_vg_list) frame_vg_list, w.shortname warehouse
        from  oc_x_deliver_order do
        LEFT  JOIN  oc_x_order_container doo on  do.deliver_order_id = doo.deliver_order_id
        LEFT  JOIN  oc_x_deliver_order_inv doi on  doo.deliver_order_id = doi.deliver_order_id
        left JOIN  oc_order_distr  od  on od.deliver_order_id = doi.deliver_order_id
        left join oc_x_warehouse w on do.warehouse_id = w.warehouse_id
        where doo.container_id in ($count_id) GROUP BY do.order_id order by doo.order_container_id  desc limit 5";
        } else {
            $sql  =  " select do.deliver_order_id  , do.deliver_date, do.order_id , doi.uptime ,doi.inv_comment , doi.deliver_order_id , od.inventory_name ,doi.frame_vg_list, w.shortname warehouse
        from  oc_x_deliver_order do
        LEFT  JOIN  oc_x_deliver_order_inv doi on  do.deliver_order_id = doi.deliver_order_id
        left JOIN  oc_order_distr  od  on od.deliver_order_id = doi.deliver_order_id
        left join oc_x_warehouse w on do.warehouse_id = w.warehouse_id
        where (doi.frame_vg_list like '%,$count_id%' or  doi.frame_vg_list in ($count_id) ) and do.order_status_id in( 6,8)  order by doi.order_inv_id  desc limit 5";
        }
...
//return $sql;
    ...
    function getInventorySortingReturn(array $data){
        ...
        //Added
        $sql_ext = "select item_key, product_id, sum(if(void=1,quantity,0)) void_quantity, sum(if(void=0,quantity,0)) quantity, sum(void) void from oc_x_inventory_return_item where status = 1 and sorting_date = '".$setDate."' and do_warehouse_id = '".$doWarehouseId."' group by item_key";
        ...
        foreach($sortingReturnsRaw as $m){
            ...
            $sortingReturns[$m['item_key']]['void'] =  array_key_exists($m['item_key'],$returnAdded) ? $returnAdded[$m['item_key']]['void'] : 0;
            $sortingReturns[$m['item_key']]['void_quantity'] =  array_key_exists($m['item_key'],$returnAdded) ? $returnAdded[$m['item_key']]['void_quantity'] : 0;
        }
    ...
    function addInventorySortingReturn(array $data){
        ...
        $opt = $data['data']['opt']? $data['data']['opt'] : false;
        $void = ($opt=='del') ? 1 : 0;
        ...
        $sql_return_move = "INSERT INTO `oc_x_inventory_return_item` (`item_key`, `sorting_date`, `do_warehouse_id`, `product_id`, `quantity`, `status`, `date_added`, `added_by`, `void`)
                          VALUES ('".$item_key."','".$setDate."','".$doWarehouseId."','".$productId."','".$quantity."','1',now(),'".$userId."','".$void."')";
...
//        $bool = $bool && $dbm->query($sql_return_move);
     ...
     * 插入后台库存及明细表
     * */
    public function insert_into_stock_move($stock_move,$stock_move_item,$id){
        global  $dbm;
        if (is_array($id)) {
            $condition = '';
            foreach ($id as $key => $ids) {
                $key = trim($key,"'");
                $ids = trim($ids,"'");
                $condition .= " AND osm.".$key." = '".$ids."' ";
            }
//            return $condition;
            $sql_select = "select inventory_move_id AS stock_move_id from oc_x_stock_move osm where 1=1 ".$condition;
//            return $sql_select;
            $query = $dbm->query($sql_select);
            $result = $query->rows;
//            return $result;
            if ($result) {
                $return = array(
...
//                return $sql1;
                $bool = $bool && $dbm->query($sql);
                $inventory_move_id1 = $dbm->getLastId();
                if ($inventory_move_id1) {
                    $sql = "INSERT INTO oc_x_stock_move_item 
(`inventory_move_id`,`station_id`,`product_id`,`quantity`,`warehouse_id`)
VALUES";
                    foreach ($stock_move_item as $value2) {
                        $sql .= "('".$inventory_move_id1."',".$value2;
                    }
                    $sql = rtrim($sql,',');
//                    return $sql;
                    $bool = $bool && $dbm->query($sql);
     ...
     * 插入前天库存及明细表
     * */
    public function insert_into_inventory_move($inventory_move,$inventory_move_item,$id){
        global  $dbm;
...
//            return $condition;
            $sql_select = "select inventory_move_id from oc_x_inventory_move osm where 1=1 ".$condition;
//            return $sql_select;
            $query = $dbm->query($sql_select);
            $result = $query->rows;
//            return $result;
        ...
        if ($inventory_move) {
            if (is_array($inventory_move_item)) {
                $dbm->begin();
                $bool = true;
                $sql = "INSERT INTO oc_x_inventory_move (`relevant_id`,`station_id`, `date`, `timestamp`, `from_station_id`, `inventory_type_id`, `date_added`, `add_user_name`, `memo`,`warehouse_id`)
VALUES".$inventory_move;
//                return $sql;
                $bool = $bool && $dbm->query($sql);
                $inventory_move_id1 = $dbm->getLastId();
                if ($inventory_move_id1) {
                    $sql = "INSERT INTO `oc_x_inventory_move_item` 
(`inventory_move_id`, `station_id`, `due_date`, `product_id`, `quantity`,`warehouse_id`) 
VALUES";
                    foreach ($inventory_move_item as $value2) {
                        $sql .= "('".$inventory_move_id1."',".$value2;
                    }
                    $sql = rtrim($sql,',');
//                    return $sql;
     ...
     * zx
     * 采购单及明细表插入
     * @param $purchase_order 采购单主表数组
     * @param $purchase_order_product 采购单明细表数组
     * @param $new_field 采购单主表新增字段数组
     * @param $new_children_field 采购单明细表新增字段数组
     * @param $type 根据类型自定义某些值
     * */
    public function insert_into_purchase_order($purchase_order,$purchase_order_product,$type,$new_field,$new_children_field){
        global  $dbm;
//        return $purchase_order;
        if ($type) {
            switch ($type) {
                case 1:
                    ...
                    $date_deliver = date("Y-m-d",time());
                    $checkout_time = date("Y-m-d H:i:s",time());
                    $checkout_status = 2;
                    $checkout_type_id = 2;
                    $status = 5;
                    $quantity = 1;
        ...
        if (is_array($purchase_order)) {
            if (is_array($purchase_order_product)) {
                $dbm->begin();
                $bool = true;

                //增加采购记录
                $sql = "INSERT INTO `oc_x_pre_purchase_order` 
(`station_id`,`date_added`,`date_deliver`,`added_by`,`add_user_name`,`supplier_type`,`checkout_status`,`order_comment`,`warehouse_id`,`status`,`checkout_time`,`checkout_type_id`,`order_type`";
                $sql_right = '';
                if ($new_field) {
                    foreach ($new_field as $field) {
                        $sql .= ",`".$field."`";
                        $sql_right .= ",'".$purchase_order[$field]."'";
                    }
                }

                $sql .= ") VALUES
('".$station."','".$date_added."','".$date_deliver."','".$purchase_order['added_by']."','".$purchase_order['add_user_name']."','".$purchase_order['supplier_type']."','".$checkout_status."','".$purchase_order['order_comment']."','".$purchase_order['warehouse_id']."','".$status."','".$checkout_time."','".$checkout_type_id."','".$order_type."'";
                if ($sql_right) {
                    $sql .= $sql_right;
                }
                $sql .= ")";
//                return $purchase_order_product;
                $bool = $bool && $dbm->query($sql);
                $purchase_order_id = $dbm->getLastId();
                //增加采购

                if ($purchase_order_id) {
                    $sql = "INSERT INTO `oc_x_pre_purchase_order_product` (`purchase_order_id`, station_id, `sku_id`, `product_id`,`date`,`price`,`quantity`,`supplier_quantity`,`real_cost`, `warehouse_id`";
                    if ($new_children_field) {
                        foreach ($new_children_field as $children_field) {
                            $sql .= ",`".$children_field."`";
                        }
                    }
                    $sql .= ") VALUES";

                    foreach ($purchase_order_product as $value2) {
                        $sql .= "('".$purchase_order_id."','".$station."','".$value2['sku_id']."','".$value2['product_id']."','".$date_deliver."','".$value2['purchase_price']."','".(intval($value2['quantity'])*$quantity)."','".(intval($value2['quantity'])*$quantity)."','".$value2['purchase_price']."','".$purchase_order['warehouse_id']."'";
                        if ($new_children_field) {
                            foreach ($new_children_field as $children_field) {
                                $sql .= ",'".$value2[$children_field]."'";
                            }
                        }
                        $sql .= "),";
...
//                    return $sql;
            ...
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => '数据传输错误',
                'return_data' => $purchase_order
            );
            return $return;
        }
    }
    public function check_result_is_true($return){
        if ($return['return_code'] != "SUCCESS") {
            return true;
        }
    }

    public function checkTail(array $data){
         global  $db ;
         $pallet_number = $data['data']['seat_name'];
         // return $pallet_number;
         $sql = "SELECT ci.pallet_number as p_number,p.repack as repack,p.name as name,ci.quantity,p.product_id as product_id
                FROM  oc_x_stock_checks_item as ci
                LEFT JOIN oc_product as p on ci.product_id = p.product_id 
                WHERE ci.pallet_number=".$pallet_number."  
    ...
    //删除确认的框子
    public function annul (array $data) {
      global  $db ;
      $container = $data['data']['container_id'] ? $data['data']['container_id'] : '';
        $user_id = $data['data']['user_id']? $data['data']['user_id'] : '';
        $order_id = $data['data']['order']? $data['data']['order'] : '';
       ...
       $sql = "UPDATE  oc_x_container_deliver_return SET status = 0  WHERE logistic_allot_id = '".$logistic_allot_id."' AND container_id = '".$container."'";
       // return $sql;
       $result = $db->query($sql);
       $info = [];
       if($result) {
...
GROUP BY wrh.relevant_id";
    ...
    /*
     * 根据需要调拨来的do单与扫描的商品或周转筐获取信息
     * */
    function getRelevantInfoByInput(array $data ){
        global $db;
        $product_id = $data['data']['product_id']? $data['data']['product_id'] : false;
        $warehouse_id = $data['data']['warehouse_id']? $data['data']['warehouse_id'] : '';
        $type= $data['data']['type']? intval($data['data']['type']) : false;


        if($product_id){
            $return = $this->get_come_deliver_order_ids($warehouse_id);
            if ($this->check_result_is_true($return)){
                return $return;
            }
            $order_info = $return['return_data']['order_info'];
            $deliver_order_ids = $return['return_data']['deliver_order_ids'];
            $order_array = [];
            if (!empty($deliver_order_ids)) {
                if ($type == 1) {
                    if(strlen($product_id) > 6){

                        $sql = " select product_id   from oc_product_to_warehouse  where  sku_barcode = '". $product_id ."'  and  
warehouse_id = '".$warehouse_id ."'";
                    ...
                    $sql_info = "SELECT doo.deliver_order_id,doo.order_id,p.name,p.product_id,doo.deliver_date,doo.warehouse_id,doo.do_warehouse_id,doo.is_repack,
	(SUM(dop.quantity) -IF (dos.quantity > 0,dos.quantity,0)) quantity,SUM(dop.quantity) AS plan_quantity,IF (dos.quantity > 0,dos.quantity,0) AS sorting_quantity
FROM
	oc_x_deliver_order doo
LEFT JOIN oc_x_deliver_order_product dop ON doo.deliver_order_id = dop.deliver_order_id
LEFT JOIN oc_product p ON dop.product_id = p.product_id
LEFT JOIN oc_x_inventory_deliver_order_sorting dos ON dos.product_id = dop.product_id
AND dos.deliver_order_id = dop.deliver_order_id
WHERE
	doo.deliver_order_id IN (".$deliver_order_ids.")
AND dop.product_id = '".$product_id."'
GROUP BY
	doo.order_id,dop.product_id
ORDER BY
	quantity DESC";

                } else if ($type == 2) {
                    $sql_info = "SELECT ".$product_id." AS container_id,doi.order_id,doi.deliver_order_id,odo.deliver_date,odo.warehouse_id,odo.do_warehouse_id,odo.is_repack,
	( 1 -IF (dos.container_id > 0,1,0)) quantity,1 as plan_quantity,IF (dos.container_id > 0,1,0) AS sorting_quantity
FROM
	oc_x_deliver_order_inv doi 
LEFT JOIN oc_x_deliver_order odo ON odo.deliver_order_id = doi.deliver_order_id
LEFT JOIN oc_x_inventory_deliver_order_sorting dos ON dos.deliver_order_id = doi.deliver_order_id AND dos.container_id = '".$product_id."'
WHERE
    doi.deliver_order_id IN (".$deliver_order_ids.")
    AND (doi.frame_vg_list LIKE '%,".$product_id."%' OR  doi.frame_vg_list IN (".$product_id."))";
                }
//                                return $sql_info;
                $query = $db->query($sql_info);
                $result_info = $query->rows;
                if (empty($result_info)) {
                    $return = array(
                        'return_code' => 'ERROR',
                        'return_msg'  => '该订单本仓未分拣完或者无该商品的订单',
                        'return_data' => ''
                    );
                    return $return;

                }
                foreach ($result_info as $value2) {
                    $order_array[$value2['order_id']] = $value2;
                    $order_array[$value2['order_id']]['inv_comment'] = $order_info[$value2['order_id']]['inv_comment'];
                }
                $order_array_order = [];
                foreach ($order_array as $value1) {
                    $inv_comment = explode('@',$value1['inv_comment'])[1];
                    $order_array_order[$inv_comment.$value1['order_id']] = $order_array[$value1['order_id']];
                }
            } else {
                $return = array(
                    'return_code' => 'ERROR',
            ...
            $result_infos = [];
            $result_infos['array1'] = $order_array_order;
            if ($type == 2) {
                $result_infos['array2'] = $this->getOrderInfoByCount(['data'=>['warehouse_id'=>$warehouse_id,'count_id'=>$product_id]]);
            }
            return $result_infos;
        }else{
            return 2 ;
     ...
     * 查询需要合单的DO单的信息
     * */
    function get_order_information_to_merge(array $data ){
        global $db;
        $warehouse_id = $data['data']['warehouse_id']? $data['data']['warehouse_id'] : false;
        $psize = isset($data['data']['psize']) ? intval($data['data']['psize']) : false;
        $start_row = isset($data['data']['start_row']) ? intval($data['data']['start_row']) : false;
        $condition_type = isset($data['data']['condition_type']) ? intval($data['data']['condition_type']) : false;
        $order_status_id = !empty($data['data']['order_status_id']) ? intval($data['data']['order_status_id']) : false;
        $deliver_date = isset($data['data']['deliver_date']) ? $data['data']['deliver_date'] : false;
//return $psize;
        if($warehouse_id){
            $return = $this->get_come_deliver_order_ids($warehouse_id,$psize,$start_row,$deliver_date,$condition_type,$order_status_id);
            if ($this->check_result_is_true($return)){
                return $return;
            }
            $pages = $return['return_data']['pages'];
            $count = $return['return_data']['count'];
            $order_info = $return['return_data']['order_info'];
            $deliver_order_ids = $return['return_data']['deliver_order_ids'];
            $order_ids = $return['return_data']['order_ids'];
            $order_array = [];
            $deliver_order_array = [];
            //查询需要合单的订单关联的DO单的基础信息
            $sql_info = "SELECT doo.deliver_order_id,doo.warehouse_id,doo.do_warehouse_id,oxw.title,doo.order_status_id AS order_status_id2,dos.name AS name2,doo.is_repack,
    doo.order_id,oo.order_status_id,os.name
FROM
	oc_x_deliver_order doo
LEFT JOIN oc_x_warehouse oxw ON oxw.warehouse_id = doo.do_warehouse_id
LEFT JOIN oc_order oo ON oo.order_id = doo.order_id
LEFT JOIN oc_x_deliver_order_status dos ON dos.order_status_id = doo.order_status_id
LEFT JOIN oc_order_status os ON os.order_status_id = oo.order_status_id
WHERE
	doo.order_id IN (".$order_ids.")
GROUP BY
	doo.deliver_order_id
ORDER BY
	order_id DESC ";
...
//                                return $sql_info;
            ...
            $result_info = $query->rows;
            //查询需要调拨过来的do单的计划值
            $sql_info2 = "SELECT dop.deliver_order_id,
    SUM(IF(p.product_id > 0,dop.quantity,0)) AS plan_quantity,IF(doi.frame_count>0,doi.frame_count,0) AS plan_container_count
FROM
	oc_x_deliver_order_product dop
LEFT JOIN oc_product p ON dop.product_id = p.product_id AND p.repack = 0
LEFT JOIN oc_x_deliver_order_inv doi ON doi.deliver_order_id = dop.deliver_order_id 
WHERE
	dop.deliver_order_id IN (".$deliver_order_ids.")
GROUP BY
	dop.deliver_order_id
ORDER BY
	deliver_order_id DESC";
//                                return $sql_info2;
            $query2 = $db->query($sql_info2);
            $result_info2 = $query2->rows;
            foreach ($result_info2 as $value2) {
                $deliver_order_array[$value2['deliver_order_id']] = $value2;
                $deliver_order_array[$value2['deliver_order_id']]['sorting_quantity'] = 0;
                $deliver_order_array[$value2['deliver_order_id']]['container_count'] = 0;
            }
            //查询调拨过来的DO单投篮的情况
            $sql_info3 = "SELECT dos.deliver_order_id,
    SUM(IF(dos.product_id > 0,dos.quantity,0)) AS sorting_quantity,
    COUNT(DISTINCT IF(dos.container_id > 0,dos.container_id,NULL)) AS container_count
FROM
	oc_x_inventory_deliver_order_sorting dos  
WHERE
	dos.deliver_order_id IN (".$deliver_order_ids.")
GROUP BY
	dos.deliver_order_id
ORDER BY
	deliver_order_id DESC";
//            return $sql_info3;
            $query3 = $db->query($sql_info3);
            $result_info3 = $query3->rows;
            foreach ($result_info3 as $value3) {
                $deliver_order_array[$value3['deliver_order_id']]['sorting_quantity'] = $value3['sorting_quantity'];
                $deliver_order_array[$value3['deliver_order_id']]['container_count'] = $value3['container_count'];
            }
//return $result_info;
            //将所有信息整合
            foreach ($result_info as $value) {
                $order_id = $value['order_id'];
                $deliver_order_id = $value['deliver_order_id'];
                $order_array[$order_id]['order_status_id'] = $value['order_status_id'];
                $order_array[$order_id]['order_id'] = $order_id;
                $order_array[$order_id]['name'] = $value['name'];
                $order_array[$order_id]['inv_comment'] = $order_info[$order_id]['inv_comment'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['deliver_order_id'] = $deliver_order_id;
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['do_warehouse_id'] = $value['do_warehouse_id'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['warehouse_id'] = $value['warehouse_id'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['title'] = $value['title'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['order_status_id'] = $value['order_status_id2'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['name'] = $value['name2'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['is_repack'] = $value['is_repack'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['plan_quantity'] = $deliver_order_array[$deliver_order_id]['plan_quantity'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['sorting_quantity'] = $deliver_order_array[$deliver_order_id]['sorting_quantity'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['container_count'] = $deliver_order_array[$deliver_order_id]['container_count'];
                $order_array[$order_id]['deliver_orders'][$deliver_order_id]['plan_container_count'] = $deliver_order_array[$deliver_order_id]['plan_container_count'];

            }
            $order_array_order = [];
            foreach ($order_info as $value1) {
                $order_array_order['@'.$value1['order_id']] = $order_array[$value1['order_id']];
            }
            $array = ['data'=>$order_array_order,'pages'=>$pages,'count'=>$count];
            $return = array(
                'return_code' => 'SUCCESS',
                'return_msg'  => '',
                'return_data' => $array
    ...
    /*
     * zx
     * 获取需要调拨过来合单的do单和本仓内do单的货位号
     * */
    function get_come_deliver_order_ids($warehouse_id,$psize,$start_row,$deliver_date,$condition_type,$order_status_id){
        global  $db ;
        $order_info = [];
        $deliver_order_ids = '';
        $order_ids = '';
        $start_order_id = 700000;
//        return $start_row;
        $sql_count = "SELECT
COUNT(o.order_id) count ";
        $sql_select = "SELECT
GROUP_CONCAT(doo.deliver_order_id) AS deliver_order_id,
o.order_id,
IF (
doi.inv_comment > 0,
GROUP_CONCAT(1, '@', doi.inv_comment),
GROUP_CONCAT(0, '@', doit.inv_comment)
) AS inv_comment ";
        $sql =  "
FROM
	oc_order o
LEFT JOIN oc_x_deliver_order doo ON o.order_id = doo.order_id
AND doo.warehouse_id = o.warehouse_id
AND (doo.do_warehouse_id != doo.warehouse_id or (doo.do_warehouse_id = doo.warehouse_id and doo.is_repack = 1))
AND doo.order_deliver_status_id IN (1, 2)
LEFT JOIN oc_x_deliver_order doot ON doot.order_id = doo.order_id
AND doot.deliver_order_id != doo.deliver_order_id
AND doot.do_warehouse_id = o.warehouse_id
LEFT JOIN oc_x_deliver_order_inv doi ON doi.order_id = doot.order_id
AND doi.deliver_order_id = doot.deliver_order_id
LEFT JOIN oc_x_deliver_order_inv doit ON doit.order_id = doo.order_id
AND doit.deliver_order_id = doo.deliver_order_id
WHERE
o.order_id > '".$start_order_id."' ";
        if (!empty($deliver_date)) {
            $sql .= " AND o.deliver_date = '".$deliver_date."' ";
        }
        $sql .= "
AND o.warehouse_id = '".$warehouse_id."'";
        if (!empty($order_status_id) && $order_status_id >1) {
            $sql .= " AND o.order_status_id = '".$order_status_id."' ";
        } else if ($order_status_id  == 1){
            $sql .= " AND o.order_status_id > '".$order_status_id."' ";
        }
        $sql .= " AND doo.deliver_order_id > 0 ";
        if (empty($condition_type)) {
            $sql .= "
AND o.order_status_id IN (5,8)
AND o.order_deliver_status_id = 1
AND doo.order_status_id IN(6,12) 
AND (
	doot.order_status_id IN (6, 12)
	OR doot.deliver_order_id IS NULL
)
";
        }
        if(isset($start_row) && isset($psize)){
            $sql_count .= $sql;
            $query_count = $db->query($sql_count);
            $result_count = $query_count->row;
            $count = intval($result_count['count']);
            $pages = ceil($count/$psize);

            $sql .=  " GROUP BY
	doo.order_id
ORDER BY
	o.order_status_id,inv_comment limit  ".$start_row*$psize.",".$psize ;
        } else {
            $sql .= " GROUP BY
	doo.order_id
ORDER BY
...
//        return $sql_select;
        ...
        $deliver_order_ids = ltrim($deliver_order_ids,',');
        $order_ids = ltrim($order_ids,',');

        $return = array(
            'return_code' => 'SUCCESS',
            'return_msg'  => '',
            'return_data' => ['order_info'=>$order_info,'deliver_order_ids'=>$deliver_order_ids,'order_ids'=>$order_ids,'count'=>$count,'pages'=>$pages]
        );
        return $return;
     ...
     * 合单时修改order_inv数据
     * */
    function change_come_deliver_order_inv($warehouse_id,$order_id,$type){
        global  $db ;

            $sql = "SELECT
	doo.deliver_order_id,
	doo.order_id,doi.inv_comment,doi.deliver_order_id AS deliver_order_id2 
FROM
    oc_x_deliver_order doo
LEFT JOIN oc_x_deliver_order_inv doi ON doi.deliver_order_id = doo.deliver_order_id
WHERE
(doo.do_warehouse_id != doo.warehouse_id or (doo.do_warehouse_id = doo.warehouse_id and doo.is_repack = 1))
AND doo.warehouse_id = '" . $warehouse_id . "'
AND doo.deliver_order_id = '".$order_id."'
GROUP BY
	doo.deliver_order_id";
//        return $sql;
        $query = $db->query($sql);
        $result = $query->row;
        $deliver_order_id = $result['deliver_order_id'];
        $sqls = "SELECT
	GROUP_CONCAT(
		DISTINCT
		IF (
...
FROM
    oc_x_deliver_order doo 
LEFT JOIN oc_x_inventory_deliver_order_sorting dos ON dos.deliver_order_id = doo.deliver_order_id
WHERE
doo.deliver_order_id = '".$deliver_order_id."'
GROUP BY
	doo.deliver_order_id";
        ...
        if (!empty($results)) {
            if ($type == 1) {
                if (empty($result['deliver_order_id2'])) {
                    $sql_update = "INSERT INTO `oc_x_deliver_order_inv` (`deliver_order_id`,`box_count`,`frame_count`,`frame_vg_list`,`inv_comment`,`uptime`,`date_modified`,`order_id`) VALUES
('".$deliver_order_id."','".$results['box_count']."','".$results['frame_count']."','".$results['frame_vg_list']."',9999,NOW(),NOW(),'".$results['order_id']."') ";
//                    return $sql_update;
                    $db->query($sql_update);
                } else {
                    $sql_update = "UPDATE oc_x_deliver_order_inv SET 
box_count = '".$results['box_count']."',
frame_count = '".$results['frame_count']."',
frame_vg_list = '".$results['frame_vg_list']."', 
uptime = NOW()
WHERE deliver_order_id = '".$deliver_order_id."'";
                ...
                $sql_update = "UPDATE oc_order_inv SET 
box_count = '".$results['box_count']."',
frame_count = '".$results['frame_count']."',
frame_vg_list = '".$results['frame_vg_list']."', 
inv_comment = '".$result['inv_comment']."', 
uptime = NOW()
WHERE order_id = '".$order_id."'";
                $db->query($sql_update);

                $this->addOrderInvHistory($order_id);
    ...
    /*
     * zx
     * 插入rerturn_quantity数据
     * */
    function insert_into_return_quantity($order_id,$type,$added_by){
        global  $db ;
        $plan_array = [];
        $insert_into_array = [];
        $sql1 = "SELECT
    dop.product_id,SUM(dop.quantity) AS order_quantity
FROM
	 oc_order_product dop 
...
//        return $sql1;
        ...
        foreach ($results1 as $value1) {
            $plan_array[$value1['product_id']] = $value1;
        }
        //取出合单中间表的数据
        if ($type == 1) {
            $sql_check = "SELECT
...
FROM
	oc_x_deliver_order doo
LEFT JOIN oc_x_inventory_order_return_quantity dop ON dop.order_id = doo.order_id
WHERE
	doo.order_id = '".$order_id."'
AND doo.warehouse_id != doo.do_warehouse_id
GROUP BY
	doo.order_id LIMIT 0,1";
            $query_check = $db->query($sql_check);
            $results_check = $query_check->row;
            if (intval($results_check['product_id'])>0) {
                $sql = "UPDATE oc_x_inventory_order_return_quantity SET status = 0 WHERE order_id ='".$results_check['order_id']."' ";
                $db->query($sql);
            }
            //整件商品
            $sql2 = "SELECT
	...
	ios.product_id,
	SUM(dop.quantity) order_quantity,
	SUM(ios.quantity) sorting_quantity

FROM
	oc_x_inventory_deliver_order_sorting ios
LEFT JOIN (SELECT deliver_order_id,order_id,price,sum(quantity) AS quantity ,product_id  FROM oc_x_deliver_order_product WHERE order_id = '".$order_id."' GROUP BY product_id ) dop ON ios.deliver_order_id = dop.deliver_order_id
AND ios.product_id = dop.product_id
WHERE
	ios.order_id = '".$order_id."'
...
//        return $sql2;
            ...
            if (!empty($results2)) {
                foreach ($results2 as $value2) {
                    ...
                    $sorting_quantity = intval($value2['sorting_quantity']);
                    $order_quantity = intval($plan_array[$product_id]['order_quantity']);
                    if ($order_quantity) {
                        if ($order_quantity > $sorting_quantity) {
                            $insert_into_array[$product_id] = $value2;
                            $insert_into_array[$product_id]['order_quantity'] = $order_quantity;
                            $insert_into_array[$product_id]['return_quantity'] = $order_quantity-$sorting_quantity;
                        } else {
                            $insert_into_array[$product_id] = $value2;
                            $insert_into_array[$product_id]['order_quantity'] = $order_quantity;
                            $insert_into_array[$product_id]['sorting_quantity'] = $order_quantity;
                            $insert_into_array[$product_id]['return_quantity'] = 0;
                        }
                    }
                }
            }
...
FROM
	oc_x_inventory_deliver_order_sorting dos
LEFT JOIN oc_x_inventory_order_sorting ios ON ios.deliver_order_id = dos.deliver_order_id
AND dos.container_id = ios.container_id
AND ios.status = 1
WHERE
...
//        return $sql3;
            ...
            if (!empty($results3)) {
                foreach ($results3 as $value3) {
                    ...
                    $sorting_quantity = intval($value3['sorting_quantity']);
                    $order_quantity = intval($plan_array[$product_id]['order_quantity']);
                    if ($order_quantity) {
                        if ($order_quantity > $sorting_quantity) {
                            $insert_into_array[$product_id] = $value3;
                            $insert_into_array[$product_id]['order_quantity'] = $order_quantity;
                            $insert_into_array[$product_id]['return_quantity'] = $order_quantity-$sorting_quantity;
                        } else {
                            $insert_into_array[$product_id] = $value3;
...
WHERE
	ios.status = 1
AND ios.order_id = '".$order_id."'
AND dos.warehouse_id = dos.do_warehouse_id AND dos.is_repack = 0
GROUP BY
	ios.product_id";
//        return $sql2;
            $query2 = $db->query($sql2);
...
//        return $insert_into_array;
        if (!empty($insert_into_array)) {
            $sql_insert = " insert into oc_x_inventory_order_return_quantity 
 ( `order_id` , `deliver_order_id` , `product_id` , `order_quantity` , `sorting_quantity` ,`return_quantity` , `uptime` , `added_by` ) 
 VALUES";
            foreach($insert_into_array as $insert_array){
                $sql_insert .= "
('".$insert_array['order_id']."','".$insert_array['deliver_order_id']."','".$insert_array['product_id']."','".$insert_array['order_quantity']."','".$insert_array['sorting_quantity']."','".$insert_array['return_quantity']."',NOW(),'".$added_by."'),";
            }
...
//            return $sql_insert;
    ...
    /*
     * 获取分拣明细数据
     * @deliver_order_id int 分拣单
     * @product_type int 获取类型 1获取整件商品;2按周转筐获取散件商品;3按商品种类获取散件商品
     * */
    function catOrderProductInfo(array $data){
        global $db;
        $deliver_order_id = $data['data']['deliver_order_id']? $data['data']['deliver_order_id'] : false;
        $product_type = $data['data']['product_type']? intval($data['data']['product_type']) : false;
//return $psize;
        if(!$deliver_order_id){
        ...
        switch($product_type)
        {
            case 1:
                $sql_info1 = "SELECT 
        SUM(dop.`quantity`) `plan_quantity`,dop.`product_id`,op.`name`
    FROM
        oc_x_deliver_order_product dop
    LEFT JOIN oc_product op ON op.product_id = dop.product_id
    WHERE
        dop.deliver_order_id = '".$deliver_order_id."'
    AND op.repack = 0
    GROUP BY
        dop.product_id
    ORDER BY
        dop.quantity DESC ";
                $query1 = $db->query($sql_info1);
                $result_info1 = $query1->rows;


                $sql_info2 = "SELECT
        SUM(ios.`quantity`) `quantity`,
        ios.`product_id`
    FROM
        oc_x_inventory_order_sorting ios
    WHERE
        ios.deliver_order_id = '".$deliver_order_id."'
    AND (ios.container_id = 0 OR ios.container_id IS NULL)
    AND ios.status = 1
    GROUP BY
        product_id
    ORDER BY
        quantity DESC";
                ...
                $result_info2 = $query2->rows;
                foreach ($result_info2 as $value2) {
                    $array_sorting[$value2['product_id']] = $value2['quantity'];
                }
...
//return $array_sorting;
    ...
    ORDER BY
        quantity DESC";
                ...
                $result_info3 = $query3->rows;
                foreach ($result_info3 as $value3) {
                    $array_merge[$value3['product_id']] = $value3['quantity'];
                }

//return $array_merge;
                foreach ($result_info1 as $value1) {
                    $product_id = $value1['product_id'];
                    $array_plan[$product_id] = $value1;
                    $array_plan[$product_id]['sorting_quantity'] = empty($array_sorting[$product_id]) ? 0 : $array_sorting[$product_id] ;
                    $array_plan[$product_id]['merge_quantity'] = empty($array_merge[$product_id]) ? 0 : $array_merge[$product_id] ;
                }

                if (empty($array_plan)) {
                    $return = array(
                        'return_code' => 'ERROR',
                        'return_msg'  => '该分拣单无整件商品',
                        'return_data' => ''
                    );
                } else {
                    $return = array(
    ...
    ORDER BY
        dop.quantity DESC ";
                ...
                $result_info1 = $query1->rows;
                foreach ($result_info1 as $value1) {
                    $array_plan[$value1['product_id']] = $value1;
                }
    ...
    ORDER BY
        quantity DESC";
                ...
                $result_info3 = $query3->rows;
                foreach ($result_info3 as $value3) {
                    $array_merge[$value3['container_id']] = intval($value3['container_id']);
                }


                foreach ($result_info2 as $value2) {
                    $container_id = $value2['container_id'];
                    $product_id = $value2['product_id'];
                    $array_sorting[$container_id]['container_id'] = $container_id;
                    $array_sorting[$container_id]['sorting_container'] = 1;
                    $array_sorting[$container_id]['merge_container'] = empty($array_merge[$container_id]) ? 0 : 1;
                    $array_sorting[$container_id]['product_id'][$product_id] = $array_plan[$product_id];
                    $array_sorting[$container_id]['product_id'][$product_id]['sorting_quantity'] = $value2['quantity'];
                    $array_sorting[$container_id]['product_id'][$product_id]['merge_quantity'] = empty($array_merge[$container_id]) ? 0 : $value2['quantity'];
                }
                foreach ($array_merge as $key4 => $value4){
                    if (empty($array_sorting[$key4])) {
                        $array_sorting[$key4]['container_id'] = $key4;
                        $array_sorting[$key4]['sorting_container'] = 0;
                        $array_sorting[$key4]['merge_container'] = 1;
                        $array_sorting[$key4]['product_id'] = [];
                    }
                }
                if (empty($array_sorting)) {
                    $return = array(
                        'return_code' => 'ERROR',
                        'return_msg'  => '该分拣单无分拣数据',
                        'return_data' => ''
                    );
                } else {
                    $return = array(
    ...
    FROM
        oc_x_inventory_order_sorting ios
    LEFT JOIN oc_product op ON op.product_id = ios.product_id
    WHERE
        ios.deliver_order_id = '".$deliver_order_id."'
    AND ios.container_id > 0
    AND ios.status = 1
    ...
    //                                return $sql_info;
        ...
        ios.`product_id`
    FROM
        oc_x_inventory_deliver_order_sorting idos
    LEFT JOIN oc_x_inventory_order_sorting ios on ios.container_id = idos.container_id AND ios.status = 1
    WHERE
        ios.deliver_order_id = '".$deliver_order_id."'
    AND ios.container_id>0
    GROUP BY
    ...
    function checkInRightTime(){
        $start_time = strtotime(date("Y-m-d"),time());
        $end_time = ((strtotime(date("Y-m-d"),time())+20*60));
        $return = [];
        if ($start_time <= time() && time() <= $end_time) {
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => '系统正在备份，为防止数据丢失，请于2：20之后再操作',
                'return_data' => ''
            );
            return $return;
        }
...
//整件批次分拣
    function getBoxBatchInfo(array $data){
        global $db;
...
//        $sql = "select op.product_id, pw.stock_area, pw.sku_barcode, p.name, sum(op.quantity) plan_quantity, if(sum(ios.quantity) is null, 0, sum(ios.quantity)) quantity from oc_x_deliver_order o
//                left join oc_x_deliver_order_product op on op.deliver_order_id = o.deliver_order_id
//                left join oc_x_inventory_order_sorting ios on ios.deliver_order_id = op.deliver_order_id and ios.product_id = op.product_id
//                left join oc_product p on op.product_id = p.product_id
//                left join oc_product_to_warehouse pw on p.product_id = pw.product_id and pw.warehouse_id = ".$doWarehouseId."
//                where o.batch_id = 0 and o.deliver_date = '".$deliverDate."' and o.do_warehouse_id=".$doWarehouseId." and o.warehouse_id = '".$toWarehouseId."' and p.repack=0
//                    and (o.order_status_id in (6,8) or o.box_only=0)
//                group by op.product_id";

        $sql = "select count(distinct op.product_id) products, sum(op.quantity - if(ios.quantity is null, 0, ios.quantity)) quantity
                from oc_x_deliver_order o
                left join oc_order oo on o.order_id = oo.order_id
                left join oc_x_deliver_order_product op on op.deliver_order_id = o.deliver_order_id
                left join oc_x_inventory_order_sorting ios on ios.deliver_order_id = op.deliver_order_id and ios.product_id = op.product_id
                left join oc_product p on op.product_id = p.product_id
                where o.batch_id = 0 and o.do_warehouse_id <> o.warehouse_id and o.deliver_date = '".$deliverDate."' and o.do_warehouse_id=".$doWarehouseId." and o.warehouse_id = '".$toWarehouseId."' and p.repack=0
                    and oo.order_status_id in (2,5,8)
                    and (o.order_status_id in (6,8) or o.box_only=1)
                ";
        $query = $db->query($sql);
        $results = $query->row;
    ...
    function getBoxBatchOrders(array $data){
        global $db;
        ...
        $doWarehouseId = $data['data']['doWarehouseId']? (int)$data['data']['doWarehouseId'] : false;
        $toWarehouseId = $data['data']['toWarehouseId']? (int)$data['data']['toWarehouseId'] : false;
        $search_type = $data['data']['search_type']? (int)$data['data']['search_type'] : false;
        ...
        $sql = "
            select a.batch_id, w.title warehouse, a.status batch_status_id,a.deliver_date, s.name status_name, a.date_added, sum(b.plan_quantity) quantity
            from oc_x_batch_sorting a
            left join oc_x_batch_sorting_status s on a.status  = s.batch_status_id
            left join oc_x_warehouse w on a.warehouse_id = w.warehouse_id
            left join oc_x_batch_sorting_item b on a.batch_id = b.batch_id
            where a.status !=3  and a.do_warehouse_id = '".$doWarehouseId."' ";
        if ($search_type == 1) {
            $sql .= "and a.status = '5' ";
        } else {
            $sql .= " and a.warehouse_id = '".$toWarehouseId."' and a.deliver_date = '".$deliverDate."' ";
        }
        $sql .= "
            group by a.batch_id";
        $query = $db->query($sql);
    ...
    function addBoxBatchOrder(array $data){
        global $dbm;
        ...
        $sql="INSERT INTO `oc_x_batch_sorting` (`warehouse_id`, `do_warehouse_id`, `repack`, `deliver_date`, `date_added`, `status`, `added_by`)
              VALUES
              ('".$toWarehouseId."','".$doWarehouseId."', 0, '".$deliverDate."', NOW(), 1, '".$userId."');";
        $bool = $bool && $dbm->query($sql);
        $batch_id = $dbm->getLastId();

        $sql = "INSERT INTO `oc_x_batch_sorting_item` (`batch_id`, `deliver_order_id`, `order_id`, `product_id`, `plan_quantity`)
            select '".$batch_id."', o.deliver_order_id, o.order_id, op.product_id,  op.quantity-if(ios.quantity is null, 0 , ios.quantity) plan_quantity
            from oc_x_deliver_order o
            left join oc_order oo on o.order_id = oo.order_id
            left join oc_x_deliver_order_product op on op.deliver_order_id = o.deliver_order_id
            left join oc_x_inventory_order_sorting ios on ios.deliver_order_id = op.deliver_order_id and ios.product_id = op.product_id
            ...
            having plan_quantity > 0 ";
        $bool = $bool && $dbm->query($sql);

        $sql = "update oc_x_batch_sorting_item a left join oc_x_deliver_order b on a.deliver_order_id = b.deliver_order_id
                set b.batch_id = a.batch_id
    ...
    function updateBoxBatchOrder(array $data){
        global $dbm;
        ...
        $reqId = $data['data']['reqId']? $data['data']['reqId'] : '';
        $productId = $data['data']['productId']? $data['data']['productId'] : '';
        $qty = $data['data']['qty']? $data['data']['qty'] : '';
        $add_user = $data['data']['userId']? $data['data']['userId'] : '';
        if(!$reqId){
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => 'ERR105: missing batch id',
                'return_data' => ''
            );
        }

        $sql = "UPDATE oc_x_batch_sorting SET status = 5 WHERE status !=3 and batch_id = '".$reqId."'";
        $bool = $bool && $dbm->query($sql);

        $sql = "DELETE FROM oc_x_batch_sorting_swap WHERE batch_id = '".$reqId."' and product_id = '".$productId."'";
        $bool = $bool && $dbm->query($sql);

        $sql = "INSERT INTO `oc_x_batch_sorting_swap` (`batch_id`, `product_id`, `plan_quantity`, `quantity`, `move_flag`, `date_added`, `added_by`)
                VALUES ('".$reqId."', '".$productId."', 0, '".$qty."', 0, NOW(), '".$add_user."');";
        $bool = $bool && $dbm->query($sql);

        if (!$bool) {
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => 'ERR106: update batch order fail.',
                'return_data' => ''
            );
            $dbm->rollback();
        }else {
    ...
    function cancelBoxBatchOrder(array $data){
        global $dbm;
        ...
        $sql = "SELECT status FROM oc_x_batch_sorting WHERE batch_id = ".$reqId;
        $query = $dbm->query($sql);
        $batchStatus = $query->row;
        if ($batchStatus['status'] == 6) {
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => 'ERR107: invalid status.',
                'return_data' => ''
        ...
        $sql = "UPDATE oc_x_batch_sorting SET status = 3 WHERE status !=6 and batch_id = '".$reqId."'";
        $bool = $bool && $dbm->query($sql);

        $sql = "UPDATE oc_x_deliver_order SET batch_id = 0 WHERE batch_id = '".$reqId."'";
        $bool = $bool && $dbm->query($sql);
        //$this->insert_warehouse_requisition_history(3,$reqId,$add_user);
        ...
        if (!$bool) {
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => 'ERR106: cancel batch order fail.',
                'return_data' => ''
            );
            $dbm->rollback();
        }else {
    ...
    function getBoxBatchOrderItem(array $data){
        global $db;
        $reqId = $data['data']['reqId']? (int)$data['data']['reqId'] : false;
        if(!$reqId){
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => 'ERR105: missing batch id',
        ...
        $sql = "select a.batch_id, a.status batch_status_id, s.name batch_status, w.title warehouse, dw.title doWarehouse, a.date_added, count(distinct b.product_id) products, sum(b.plan_quantity) quantity
                from oc_x_batch_sorting a
                left join oc_x_batch_sorting_status s on a.status  = s.batch_status_id
                left join oc_x_warehouse w on a.warehouse_id = w.warehouse_id
                left join oc_x_warehouse dw on a.do_warehouse_id = dw.warehouse_id
                left join oc_x_batch_sorting_item b on a.batch_id = b.batch_id
                where a.batch_id = '".$reqId."'
                group by a.batch_id";
        $query = $db->query($sql);
        $batchInfo = $query->row;


        $sql = "select a.batch_id, b.product_id, p.name, pw.stock_area, pw.sku_barcode, sum(b.plan_quantity) quantity
                from oc_x_batch_sorting a
                left join oc_x_batch_sorting_status s on a.status  = s.batch_status_id
                left join oc_x_warehouse w on a.warehouse_id = w.warehouse_id
                left join oc_x_batch_sorting_item b on a.batch_id = b.batch_id
                left join oc_product p on b.product_id = p.product_id
                left join oc_product_to_warehouse pw on p.product_id = pw.product_id and pw.warehouse_id = a.do_warehouse_id
                where a.batch_id = '".$reqId."'
                group by a.batch_id, b.product_id
                ";
        $query = $db->query($sql);
        $batchDetailRaw = $query->rows;
        $batchDetail = array();


        $sql = "select b.product_id, sum(b.quantity) quantity
                from oc_x_batch_sorting a
                left join oc_x_batch_sorting_swap b on a.batch_id = b.batch_id
                where a.batch_id = '".$reqId."'
                group by a.batch_id, b.product_id having product_id > 0
                ";
        $query = $db->query($sql);
        $swapDetailRaw = $query->rows;

        foreach($batchDetailRaw as $m){
            $batchDetail[$m['product_id']] = $m;
            $batchDetail[$m['product_id']]['swap'] = 0;
        }

        if(sizeof($swapDetailRaw)){
            foreach($swapDetailRaw as $m){
                $batchDetail[$m['product_id']]['swap'] = $m['quantity'];
            }
        }
    ...
    function submit_box_product_batch(array $data) {
        global $dbm;
        $reqId = $data['data']['reqId']? (int)$data['data']['reqId'] : false;
        $username = $data['data']['username']? $data['data']['username'] : false;
        if(!$reqId){
            $return = array(
                'return_code' => 'ERROR',
                'return_msg'  => 'ERR105: missing batch id',
        ...
        $dbm->begin();
        $bool = true;
        $sql = "UPDATE oc_x_batch_sorting SET status = 6 WHERE batch_id = '".$reqId."'";
        $bool && $dbm->query($sql);
        $sql = "UPDATE oc_x_deliver_order SET order_status_id = 6,box_status_id = 6 WHERE order_status_id != 3 AND order_status_id != 12 AND batch_id = '".$reqId."'";
        $bool && $dbm->query($sql);


        //Alex: 补特殊订单状态记录-改为分拣中， 先记录-再修改
        $sql = "INSERT INTO oc_order_history (`order_id`, `notify`, `comment`, `date_added`, `order_status_id`, `order_payment_status_id`, `order_deliver_status_id`)
                select b.order_id, 0, '[SYS]订单整件波次分拣', now(),  5 order_status_id, b.order_payment_status_id, b.order_deliver_status_id
                from oc_x_deliver_order a inner join oc_order b on a.order_id = b.order_id
                where a.batch_id = '".$reqId."'  and b.order_status_id = 2";
        $bool && $dbm->query($sql);
        $sql = "UPDATE  oc_x_deliver_order a INNER JOIN oc_order b on a.order_id = b.order_id  SET b.order_status_id = 5 WHERE b.order_status_id = 2 AND a.batch_id = '".$reqId."'";
        $bool && $dbm->query($sql);

        $sql_insert = [];
        $sql = "select a.batch_id, b.product_id, sum(b.plan_quantity) quantity,b.deliver_order_id,b.order_id
                from oc_x_batch_sorting a
                left join oc_x_batch_sorting_item b on a.batch_id = b.batch_id
                where a.batch_id = '".$reqId."'
                group by b.deliver_order_id, b.product_id
                ";
        $query = $dbm->query($sql);
        $batchDetailRaw = $query->rows;
        ...
        $swapDetailRaw = $query->rows;
        $batchDetail = array();
        if (!empty($swapDetailRaw)) {
            foreach ($swapDetailRaw as $value1) {
                $batchDetail[$value1['product_id']] = $value1;
            }
        }

        if (!empty($batchDetailRaw)) {
            foreach ($batchDetailRaw as $value2) {
                if (!empty($batchDetail[$value2['product_id']])) {
                    $plan_quantity = intval($value2['quantity']);
                    $total_quantity = intval($batchDetail[$value2['product_id']]['quantity']);
                    if ($plan_quantity < $total_quantity) {
                        $sql_insert[$value2['deliver_order_id'].'@'.$value2['product_id']] = $value2;
                        $batchDetail[$value2['product_id']]['quantity'] = $total_quantity - $plan_quantity;
                    } else {
                        $sql_insert[$value2['deliver_order_id'].'@'.$value2['product_id']] = $value2;
                        $sql_insert[$value2['deliver_order_id'].'@'.$value2['product_id']]['quantity'] = $total_quantity;
                        $batchDetail[$value2['product_id']]['quantity'] = 0;
                    }
                }
            }
        }

        $sql = "INSERT INTO `oc_x_inventory_order_sorting` ( `order_id`, `deliver_order_id`, `container_id`, `product_id`, `quantity`, `uptime`, `move_flag`, `status`, `added_by`) VALUES";
        if (!empty($sql_insert)) {
            foreach ($sql_insert as $values) {
                $sql .= "('".$values['order_id']."','".$values['deliver_order_id']."',0,'".$values['product_id']."','".$values['quantity']."',NOW(),0,1, '".$username."'),";
            }
            $sql = rtrim($sql,',');
            $bool && $dbm->query($sql);
            $start_deliver = 400000;
            $sql_deliver = "INSERT INTO `oc_x_deliver_order_inv` (`deliver_order_id`,`frame_count`,`frame_vg_list`,`box_count`,`inv_comment`,`uptime`,`date_modified`,`order_id`)
(SELECT
	odo.deliver_order_id,
	0,
	'',
	if(ISNULL(dos.box_count),0,dos.box_count),
	9999,
	NOW(),
	NOW(),
	odo.order_id
...
LEFT JOIN (
	SELECT
		sum(ios.quantity) as box_count,ios.deliver_order_id
	FROM
		oc_x_inventory_order_sorting ios
	LEFT JOIN oc_x_deliver_order doo ON doo.deliver_order_id = ios.deliver_order_id
	WHERE doo.deliver_order_id > '".$start_deliver."'
AND doo.batch_id = '".$reqId."'
AND doo.box_only = 1 GROUP BY ios.deliver_order_id
having ios.deliver_order_id > 0
) dos on dos.deliver_order_id = odo.deliver_order_id
WHERE
	odo.deliver_order_id > '".$start_deliver."'
AND odo.batch_id = '".$reqId."'
AND odo.box_only = 1
AND ISNULL(doi.deliver_order_id)
GROUP BY odo.deliver_order_id 
HAVING odo.deliver_order_id > 0
)";
            $bool && $dbm->query($sql_deliver);
        }
    ...
    function get_print_box_products(array $data){
        global $dbm;
	...
	a.batch_id,
	w.title warhouse,
	a. STATUS batch_status_id,
	s. NAME batch_status_name,
	a.date_added,
	b.product_id,op.name,ptw.stock_area,ptw.sku_barcode,b.plan_quantity
FROM
	oc_x_batch_sorting a
LEFT JOIN oc_x_batch_sorting_status s ON a. STATUS = s.batch_status_id
LEFT JOIN oc_x_warehouse w ON a.warehouse_id = w.warehouse_id
LEFT JOIN oc_x_batch_sorting_item b ON a.batch_id = b.batch_id
LEFT JOIN oc_product op ON op.product_id = b.product_id
LEFT JOIN oc_product_to_warehouse ptw ON ptw.product_id = b.product_id and ptw.do_warehouse_id = a.do_warehouse_id
WHERE
	a.batch_id = 108
GROUP BY
	batch_id,product_id ORDER BY stock_area DESC ";
    ...
    /*
    * 获取未完成的散件仓的含整件商品的分拣单和数量
    * @type          1提交缺货；2取消缺货
    *
    * */
    function get_repack_product_deliver_order(array $data){

        global $db;
        $date_start = $data['data']['date_start']? $data['data']['date_start'] : false;
        $date_end = $data['data']['date_end']? $data['data']['date_end'] : $date_start;
        $order_id = $data['data']['order_id']? $data['data']['order_id'] : false;
        $warehouse_id = $data['data']['warehouse_id']? intval($data['data']['warehouse_id']) : false;
        $do_warehouse_id = $data['data']['do_warehouse_id']? intval($data['data']['do_warehouse_id']) : false;
	...
	odo.do_warehouse_id,
	odo.is_repack,
	COUNT(DISTINCT IF(op.repack=0,odop.product_id,NULL)) AS type_num,
	sum(
...
FROM
	oc_x_deliver_order odo
LEFT JOIN oc_order oo ON oo.order_id = odo.order_id
LEFT JOIN oc_x_deliver_order_product odop ON odop.deliver_order_id = odo.deliver_order_id
LEFT JOIN oc_product op ON op.product_id = odop.product_id
WHERE
	DATE(odo.deliver_date) BETWEEN '".$date_start."'
AND '".$date_end."'
AND odo.do_warehouse_id = '".$do_warehouse_id."' ";
        if ($warehouse_id) {
            $sql .= " AND odo.warehouse_id = '".$warehouse_id."' ";
        }
        $sql .= "
AND oo.order_status_id IN (2, 4, 5)
AND oo.order_deliver_status_id = 1
AND (
	(
		odo.order_status_id = 2
		AND odo.is_repack = 0
	)
	OR (
		odo.order_status_id = 6
		AND odo.is_repack = 1
	)
)
GROUP BY
...
";
    ...
    *
    * */
    function choose_repack_product_deliver_order(array $data){

        global $db;
        $deliver_order_ids = $data['data']['deliver_order_ids']? $data['data']['deliver_order_ids'] : false;
        $user_id = $data['data']['user_id']? $data['data']['user_id'] : false;
...
LEFT JOIN oc_product op ON op.product_id = odop.product_id
WHERE
	odo.deliver_order_id IN (".$deliver_order_ids.")
AND op.repack = 0
GROUP BY
	deliver_order_id,
	product_id
...
";
        ...
        if (!empty($result)) {
            foreach ($result as $value) {
                if (empty($result_item[$value['prodcut_id']])) {
                    $result_item[$value['product_id']] = $value;
                } else {
                    $result_item[$value['product_id']]['quantity'] += $value['quantity'];
                }
            }
        }
        $sql_insert = [];
        $bool = true;
        $db->begin();
        $sql_insert = " INSERT INTO oc_x_batch_sorting (warehouse_id,do_warehouse_id,section_num,repack,status,date_added,added_by) VALUES 
 ('".$warehouse_id."','".$do_warehouse_id."','',0,1,NOW(),'".$user_id."')";
        $bool && $db->query($sql_insert);
        $batch_id = $db->getLastId();

        $sql_insert_item = "INSERT INTO oc_x_batch_sorting_item(batch_id,deliver_order_id,order_id,product_id,quantity,status) VALUES";
        foreach ($result as $value1) {
            $sql_insert_item .= "('".$batch_id."','".$value1['deliver_order_id']."','".$value1['order_id']."','".$value1['product_id']."','".$value1['quantity']."',1),";
        }
        $sql_insert_item = rtrim($sql_insert_item,',');
        $bool && $db->query($sql_insert_item);

        $sql_insert_swap = "INSERT INTO oc_x_batch_sorting_swap(batch_id,product_id,plan_quantity,quantity,move_flag,date_added,added_by,status) VALUES";
        foreach ($result_item as $value2) {
            $sql_insert_swap .= "('".$batch_id."','".$value2['product_id']."','".$value2['quantity']."',0,0,'','',1),";
        }
        $sql_insert_swap = rtrim($sql_insert_swap,',');
        $bool && $db->query($sql_insert_swap);
    ...
    function get_batch_sorting_lists(array $data){
    ...
    *
    * */
    function foreman_check_product_history($condition,$user_id){
        global $db;
        $sql = "INSERT INTO `oc_x_foreman_check_product_history` (`check_product_id`,`product_id`,`stock_area`,`warehouse_id`,`foreman_id`,`check_date`,`date_added`,`date_modify`,`check_result_id`,`check_status_id`,`status`,`add_user_id`,`date_add`)
(SELECT `check_product_id`,`product_id`,`stock_area`,`warehouse_id`,`foreman_id`,`check_date`,`date_added`,`date_modify`,`check_result_id`,`check_status_id`,`status`,".$user_id.",NOW() FROM oc_x_foreman_check_product WHERE 1=1 ".$condition.")";
        $db->query($sql);
    }
    /*
    * 提交或取消缺货
    ...
    *
    * */
    function foreman_check_product_update($type,$data,$user_id,$history,$update){
        global $db;
        $condition = '';
        $update_value = '';
        $table = '';
        ...
        if (!empty($data) && is_array($data)) {
            foreach ($data as $key1 => $value1) {
                $condition .= " AND ".$table.$key1."='".$value1."' ";
            }
        }
        switch ($type) {
            case 1:
                ...
                $sql = "INSERT INTO `oc_x_foreman_check_product` (`date_added`,`date_modify`,`foreman_id`,`product_id`,`stock_area`,`warehouse_id`,`check_date`)
VALUES (NOW(),NOW(),'".$user_id."',";
                $sql .= $update_value.")";
//                return $sql;
                return $db->query($sql)->num_row;
                break;
            case 2:

                if (is_array($update)) {
                    foreach ($update as $key2 => $value2) {
                        $update_value .= $key2." = '".$value2."',";
                    }
                }
                $sql = "UPDATE `oc_x_foreman_check_product` SET ".$update_value." foreman_id='".$user_id."' ,date_modify=NOW() WHERE status=1 ".$condition;
//                return $sql;
                if ($history == 1) {
                    $this->foreman_check_product_history($condition,$user_id);
                }
//                return $sql;
                return $db->query($sql)->num_row;
                break;
            case 3:
                ...
                $sql = "SELECT
	fcp.product_id,fcp.warehouse_id,fcp.date_modify,fcp.foreman_id,wu.username,fcr.memo result_memo,fcs.memo status_memo,oxw.title,oxw.shortname,fcp.stock_area,ptw.stock_area ago_stock_area,op.`name`,fcp.check_product_id,fcp.check_date,fcp.check_result_id,fcp.check_status_id
FROM
	`oc_x_foreman_check_product` fcp
LEFT JOIN oc_w_user wu ON wu.user_id = fcp.foreman_id
LEFT JOIN oc_x_foreman_check_result fcr on fcr.check_result_id = fcp.check_result_id AND fcr.status = 1 
LEFT JOIN oc_x_foreman_check_status fcs on fcs.check_status_id = fcp.check_status_id AND fcs.status = 1 
LEFT JOIN oc_x_warehouse oxw on oxw.warehouse_id = fcp.warehouse_id AND oxw.status = 1 
LEFT JOIN oc_product_to_warehouse ptw on ptw.product_id = fcp.product_id and ptw.warehouse_id = fcp.warehouse_id 
LEFT JOIN oc_product op on op.product_id = fcp.product_id 
WHERE fcp.status = 1  ";
                $sql .= $condition;
//                return $sql;
    ...
    function foreman_check_product(array $data){
        global $db;
        $user_id = $data['data']['user_id']? $data['data']['user_id'] : false;
        $product_id = $data['data']['product_id']? $data['data']['product_id'] : false;
        $warehouse_id = $data['data']['warehouse_id']? $data['data']['warehouse_id'] : false;
        $check_date = $data['data']['check_date']? date("Y-m-d",strtotime($data['data']['check_date'])) : false;
        $check_status = $data['data']['check_status']? $data['data']['check_status'] : false;
        $stock_area = $data['data']['stock_area']? $data['data']['stock_area'] : false;
//        if ($check_status == 1) {
//            $result_info = $this->foreman_check_product_update(2,['warehouse_id'=>$warehouse_id,'product_id'=>$product_id,'check_date'=>$check_date],$user_id,1);
//            $return = array(
//                'return_code' => 'SUCCESS',
//                'return_msg'  => '',
//                'return_data' => ''
...
//
//        } else {
            $result_info = $this->foreman_check_product_update(3,['warehouse_id'=>$warehouse_id,'product_id'=>$product_id,'check_date'=>$check_date],$user_id,0);
//            return $result_info;
            if (!empty($result_info[0])) {
                $return = array(
                    'return_code' => 'SUCCESS',
                    'return_msg'  => 'OK',
                    'return_data' => $result_info[0]
                );
            } else {
                $result_info = $this->foreman_check_product_update(1,'',$user_id,0,[$product_id,$stock_area,$warehouse_id,$check_date]);
//                return $result_info;
                $return = array(
                    'return_code' => 'SUCCESS',
                    'return_msg'  => '',
...
//        }
    ...
    *
    * */
    function get_foreman_check_results(array $data){
        global $db;
        $sql = "SELECT * FROM `oc_x_foreman_check_result` where status = 1";
        $result = $db->query($sql)->rows;
        $return = array(
            'return_code' => 'SUCCESS',
            'return_msg'  => '',
    ...
    function get_foreman_check_product(array $data){
        $user_id = $data['data']['user_id']? $data['data']['user_id'] : false;
        $product_id = $data['data']['product_id']? $data['data']['product_id'] : false;
        $warehouse_id = $data['data']['warehouse_id']? $data['data']['warehouse_id'] : false;
        $check_date = $data['data']['check_date']? date("Y-m-d",strtotime($data['data']['check_date'])) : false;
        $result_info = $this->foreman_check_product_update(3,['warehouse_id'=>$warehouse_id,'check_date'=>$check_date],$user_id,0);
//        return $result_info;
        if (!empty($result_info[0])) {
            $return = array(
                'return_code' => 'SUCCESS',
                'return_msg'  => '',
                'return_data' => $result_info
            );
        } else {
            $return = array(
                'return_code' => 'ERROR',
    ...
    *
    * */
    function submit_foreman_check_product(array $data){
        $user_id = $data['data']['user_id']? $data['data']['user_id'] : false;
        $product_id = $data['data']['product_id']? (int)$data['data']['product_id'] : false;
        $check_product_id = $data['data']['check_product_id']? (int)$data['data']['check_product_id'] : false;
        $warehouse_id = $data['data']['warehouse_id']? (int)$data['data']['warehouse_id'] : false;
        $check_date = $data['data']['check_date']? date("Y-m-d",strtotime($data['data']['check_date'])) : false;
        $check_result = $data['data']['check_result']? $data['data']['check_result'] : false;
        if ($check_product_id>0) {
            $data = ['check_product_id'=>$check_product_id];
        } else {
            $data = ['warehouse_id'=>$warehouse_id,'product_id'=>$product_id,'check_date'=>$check_date];
        }
        $result_info = $this->foreman_check_product_update(2,$data,$user_id,1,['check_result_id'=>$check_result]);
//        return $result_info;
        if (!empty($result_info)) {
            $return = array(
                'return_code' => 'SUCCESS',
    ...
    *
    * */
    function submitReturnProduct(array $data){

        global $db;
        $order_id = $data['data']['order_id']? $data['data']['order_id'] : false;
        $type = $data['data']['type']? intval($data['data']['type']) : false;
        $container_id = $data['data']['container_id']? intval($data['data']['container_id']) : false;
        $product_id = $data['data']['product_id']? intval($data['data']['product_id']) : false;
        $quantity = $data['data']['quantity']? intval($data['data']['quantity']) : false;
        $added_by = $data['data']['added_by']? intval($data['data']['added_by']) : false;
        $sql_update = [];
//return $psize;
        switch($type){
            case 1:
                ...
                if ($product_id > 0) {
                    $sql_select = "SELECT
	iorq.order_quantity,sorting_quantity,iorq.product_id
FROM
	`oc_x_inventory_order_return_quantity` iorq 
WHERE
	iorq.deliver_order_id = '".$order_id."'
AND iorq.product_id = '".$product_id."'
AND iorq.status = 1
GROUP BY
	iorq.product_id";
                    $query = $db->query($sql_select);
                    $result_info = $query->row;
                    $sql_update[$product_id] = "UPDATE oc_x_inventory_order_return_quantity SET 
`sorting_quantity` = '".(intval($result_info['order_quantity'])-$quantity)."',
`return_quantity` = '".$quantity."',
`uptime`= NOW(),
`added_by` = '".$added_by."' 
WHERE deliver_order_id = '".$order_id."' AND product_id = '".$product_id."' AND status = 1 ";
                } else if ($product_id == 0) {
                    $sql_select = "SELECT
	SUM(ios.quantity) quantity,iorq.order_quantity,sorting_quantity,ios.product_id
FROM
	`oc_x_inventory_order_sorting` ios
LEFT JOIN `oc_x_inventory_order_return_quantity` iorq ON ios.product_id = iorq.product_id AND iorq.status = 1
WHERE
	ios.deliver_order_id = '".$order_id."'
AND ios.container_id = '".$container_id."'
AND ios. status = 1
GROUP BY
	ios.product_id";
                    $query = $db->query($sql_select);
                    $result_info = $query->rows;
                    foreach ($result_info as $value) {
                        $sql_update[$value['product_id']] = "UPDATE oc_x_inventory_order_return_quantity SET 
`sorting_quantity` = `sorting_quantity` - ".intval($value['quantity']).",
`return_quantity` = `return_quantity` + ".intval($value['quantity']).",
`uptime`= NOW(),
`added_by` = '".$added_by."' 
WHERE deliver_order_id = '".$order_id."' AND product_id = '".$value['product_id']."' AND status = 1 ";
                    }
                }
                return $sql_update;
                if (!empty($sql_update)) {
            ...
            case 2:
                $added_by = $added_by.'@2';
                $sql_update = "update oc_x_inventory_order_return_quantity
SET `sorting_quantity` = `order_quantity` - 0,
 `return_quantity` = 0,
  `uptime` = NOW(),
 `added_by` = '".$added_by."'
WHERE
	deliver_order_id = '".$order_id."'
AND RIGHT(`added_by`,2) = '@1'
AND STATUS = 1";
                $db->query($sql_update);
                $return = array(
                    'return_code' => 'SUCCESS',
