                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data != '') {
                            ...
                            var other_type=0;
                            var full_num=null;
                            var scatter_num=null;
                            var null_sort_scatter_num=null;
                            var over_allot_scatter_num=0;
                            var sorting_scatter_num=0;
                            var audit_scatter_num=0;
                            var over_scatter_num=0;
                            var other_statu_scatter_num=0;
                            $.each(data, function (index, obj) {
                                if (obj.null_sort_scatter_num == undefined) {//[整]件存在
                                    sort_num+=parseInt(obj.sort_num);
                                    null_sort_num+=parseInt(obj.null_sort_num);
                                    already_allot+=parseInt(obj.already_allot);
                                    sorting_num+=parseInt(obj.sorting_num);
                                    to_audit_num+=parseInt(obj.to_audit_num);
                                    over_sort_num+=parseInt(obj.over_sort_num);
                                    other_type+=parseInt(obj.other_type);
                                    tr += '<tr>';
                                    ...
                                    sort_num+=parseInt(obj.sort_num);
                                    null_sort_num+=parseInt(obj.null_sort_full_num);
                                    already_allot+=parseInt(obj.over_allot_full_num);
                                    sorting_num+=parseInt(obj.sorting_full_num);
                                    to_audit_num+=parseInt(obj.audit_full_num);
                                    over_sort_num+=parseInt(obj.over_full_num);
                                    other_type+=parseInt(obj.other_statu_full_num);
                                    ...
                                    //     tr += '<br>[整]' + obj.full_num + '<br>' + obj.scatter_num;
                                    // } else {
                                    //     tr += '<br>' + obj.full_num;
                                    // }
                                    tr +='</td>';
                                    tr += '<td>' + obj.sort_type + '</td>';
                                    if(parseInt(obj.is_repack)==1){
                                        null_sort_scatter_num+=parseInt(obj.null_sort_scatter_num);
                                        over_allot_scatter_num+=parseInt(obj.over_allot_scatter_num);
                                        sorting_scatter_num+=parseInt(obj.sorting_scatter_num);
                                        audit_scatter_num+=parseInt(obj.audit_scatter_num);
                                        over_scatter_num+=parseInt(obj.over_scatter_num);
                                        other_statu_scatter_num+=parseInt(obj.other_statu_scatter_num);
                                        full_num+=parseInt(obj.full_num);
                                        scatter_num+=parseInt(obj.scatter_num);
                            ...
                            if (full_num == null) {
                                ...
                                tt += '<td>'  +sort_num;
                                tt += '<br>[整]'+full_num+'<br>'+scatter_num;
                                tt += '</td>';
                            }
                            tt += '<td>'  + '</td>';
                            if (null_sort_scatter_num == null) {
                                tt += '<td>' + null_sort_num +'</td>';
                                tt += '<td>' + already_allot+ '</td>';
                                tt += '<td>' + sorting_num+'</td>';
                                tt += '<td>' + to_audit_num+'</td>';
                                tt += '<td>' + over_sort_num+'</td>';
                                tt += '<td>' + other_type+'</td>';
                            }else {
                                tt += '<td>[整]' + null_sort_num+'<br>'+null_sort_scatter_num + '</td>';
                                tt += '<td>[整]' + already_allot+'<br>'+over_allot_scatter_num + '</td>';
                                tt += '<td>[整]' + sorting_num+'<br>'+sorting_scatter_num + '</td>';
                                tt += '<td>[整]' + to_audit_num+'<br>'+audit_scatter_num+ '</td>';
                                tt += '<td>[整]' + over_sort_num+'<br>'+over_scatter_num + '</td>';
                                tt += '<td>[整]' + other_type+'<br>'+other_statu_scatter_num + '</td>';
                            }
