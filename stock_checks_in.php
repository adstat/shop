...
<div id="message" class="message style_light" style="">搬仓入库</div>
...
<script>
            ...
            success : function (response, status, xhr){
                if(response){
                    ...
                    $.each(jsonData, function(index,value){
                        ...
                        if(value.status == 1 ){
                            html += "<td>";
                            html +='<input class="qtyopt" type="button" value="入库"  style="font-size: 0.8rem ;width: 2.5rem; " id="Inventory'+value.pallet_number +'" onclick="javascript:addStockIn('+ value.stock_check_id +')" >';
        ...
        $.ajax({
            ...
            success : function (data) {
                ...
                for(var i = 0; i < data.length; i++) {
                    ...
                    $('<tr class="barcodeHolder" id="mark_1"><td>'+data[i].p_number+'</td><td>商品ID ：'+data[i].product_id+'  ，商品名 ：'+data[i].name+'</td><td>商品数量：'+data[i].quantity+'</td><td>'+repack+'</td></tr>').insertAfter(value.parent().parent());
                }

               value.attr('mark','1');
                        // alert(html);return false;
                        // value.parent().parent().after(html);
    ...
    function addStockIn(pallet_number) {
        var  driver_number = $("#plate_number").val();
        ...
        $.ajax({
            type : 'POST',
            url : 'invapi.php?method=addStockIn&isback='+$('#isBack').val(),
            data : {
                method : 'addStockIn',
                data:{
                    driver_number: driver_number,
