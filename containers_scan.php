...
<button class="invopt" id="return_index_i" style="width:4em;" onclick="javascript:location='i.php?auth=xsj2015inv';">返回</button>
    <button class="invopt" id="return_index" style="display:none;width:4em;" onclick="javascript:location='i.php?auth=xsj2015inv';">返回</button>
    ...
    <div id="content" style="display: block">
        ...
        <div id="inv_control" align="center" >
            <div id="invMethods" style="display: none;">
                ...
                <button id="inventoryFrameIn" class="invopt" style="display: block" onclick="javascript:inventoryMethodHandler('inventoryFrameIn');">周转筐批量排查</button><br>
            ...
            <div id="issue" name="issue" method="POST" style="display: none;">
                ...
                <div id="issue_info" style="display: none">
                   ...
                   退货原因: <select id = 'issue_reason'>
                    ...
                    <div>
                    <input type="button"   class="submit"    value="记录整单退货"  onclick="redistr();" >
...
<div style="display: none">
    ...
    function inventoryMethodHandler(method){
        ...
        if(method == 'inventoryFrameIn'){
           ...
           if(method == 'deliverReturnProduct'){
            ...
            if(method == 'deliverReruenWholeProducts'){
                console.log('Start deliverReruenWholeProducts');
                $('#message').html('操作整单退货，需要先记录整单退货');
                $('#isBack').val(1);
            }
    ...
    function addProduct2(v){
        ...
        var id = parseInt(v.container_id)
        if(id !== ''){
            ...
            var html = '<tr class="barcodeHolder" id="bd'+ id +'">' +
                ...
                '<td><span name="productBarcode" >' +v.type_name+ '</span></td>' +
                            ...
                            success : function (response, status, xhr){
                                if(response){
                                    ...
                                    if(jsonData.status == 1){
                                        var container_item = jsonData.container_ids;
                                    if (parseInt(container_item.type) == 8) {
                                        var player = $("#player2")[0];
                                        player.play();
                                        alert(container_item.container_id+"是临时周转箱");
                                        $("#product").val("");
                                    } else {
                                        var player = $("#player3")[0];
                                        player.play();
                                        addProduct2(container_item);
                                    }
            ...
            success : function (response, status, xhr){
                if(response){
                    ...
                    $.each(jsonData, function(index,value){
                        ...
                        if(value.in_part == 1){
                            var confirmReutrnButton = ' <br/>散<input    style="width: 2rem"  class="addprod style_red" id="submitReturnSingleProduct"   type="button" value="确认" onclick="if(confirm(\'确认['+value.product_id+']退货吗？\')){javascript:confirmReturnShelves('+value.product_id+','+value.in_part+','+value.box_size+');}">';
                        ...
                        if(value.in_part == 0){
                            var confirmReutrnButton = ' <br/>整<input    style="width: 2rem"  class="addprod style_red" id="submitReturnSingleProduct"   type="button" value="确认" onclick="if(confirm(\'确认['+value.product_id+']退货吗？\')){javascript:confirmReturnShelves('+value.product_id+','+value.in_part+','+value.box_size+');}">';
                            if(parseInt(value.return_confirmed) ==  1){
                        ...
                        html += '<tr class="barcodeHolder" >' +
                            ...
                            '<td id="return_added'+ value.product_id+value.in_part+'" >'+returnQty+'</td>' +
                            '<td style="width:5px"><input id="shelves_confirm'+ value.product_id+ value.in_part+'"  value="'+ value.quantity +'"></td>' +
                            '<td style="width:4em; text-align: center">' +
                            '<span id="returntype'+ value.product_id+'">退整件</span>' +
                            '<input class="qty" type="hidden" id="boxqty'+ value.product_id +value.in_part +'"  name="boxqty'+  value.product_id +'" value="'+value.box_quantity+'" />' +
                            '<input class="qty" style="display: none; background-color: #fff; border: 1px #ccc solid; margin: 5px 0" id="returnboxqty'+  value.product_id +value.in_part+'" name="returnboxqty'+  value.product_id +'" value="1" />' +
                            '<input class="qty" type="hidden" id="return_id'+ value.product_id +value.in_part+'" name="boxqty'+  value.product_id +'" value="'+value.return_id+'" />' +
                            '<input class="addprod style_green returnType" style="display: none" id="returnbox'+value.product_id+value.in_part+'" type="button" value="改退整件" onclick="returnBox1('+ value.product_id+','+ value.in_part+');">' +
                            '<input class="addprod style_green returnType" style="display: inline" id="returnpart'+value.product_id+value.in_part+'" type="button" value="改退散件" onclick="returnPart1('+ value.product_id+','+ value.in_part+');">' +
                            '</td>' +
                            '<td>'+ parseFloat(value.total).toFixed(2) + '</td>' +
                            '</tr>';
...
<script>
    ...
    function reDistrList(){
        ...
        $("#issue_order_list").show();
        $("#issue_order_product_list").hide();
        $.ajax({
            type: 'POST',
            url: 'invapi.php',
            data: {
    ...
    //操作整单退货
    function  handleRedistr(order_id){

        $.ajax({
            type: 'POST',
            url: 'invapi.php',
            data: {
                method: 'handleRedistr',
                data : {
                    order_id : order_id ,
                }
            ...
            success: function (response) {
                ...
                if(jsonData == 1){
                    ...
                    alert('必须先记录整单退且订单配送失败的才能操作整单退货');
                    return false;
                }
            }
        });
    ...
    // 货架上货
    function confirmReturnShelves(product_id,in_part,box_size){

        var shelves_quantity  = $("#shelves_confirm"+product_id+in_part).val();
        var return_quantity = $("#return_added"+product_id+in_part).text();
       var  add_user_name  =  '<?php echo  $_COOKIE['inventory_user_id'] ?>';
       var warehouse_id = $("#warehouse_id").text();
        var return_set = $("#return_id"+product_id+in_part).val();
