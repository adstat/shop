    ...
    <div id="content" style="display: block">
        ...
        <div id="inv_control" align="center">
            ...
            <div id="merge_relevant_orders" style="display: none">
                ...
                <input class="submit" type="button" value="入库合并调拨单" onclick="javascript:merge_relevant_orders();"/></div>
            <input id="print_relevants_merge" style="display: none" class="submit" type="button" value="打印面单" onclick="javascript:print_relevants_merge();"/>
    ...
    * 商品扫描触发*/
    function handleProductList(container_ids){
        ...
        * 获得扫描或手输的条码*/
        ...
        var warehouse_out_type = parseInt($('#warehouse_out_type').val());
        if (warehouse_out_type == 4 && warehouse_id1 != warehouse_id2) {
            /*zx
            * 扫描相同商品减数量*/
    ...
    function delPurchaseOrderRelevantToInv(product_id,container_id){
        ...
        if(confirm("是否确认删除已扫描数据？")){
            ...
            $('.relevantlist').each(function(){
                relevant_id.push(parseInt($(this).text()));
            });
            var warehouse_out_type = parseInt($('#warehouse_out_type').val());
            /*zx
            * 退货调拨单出库删除*/
                        ...
                        if (1 == 1) {
                            ...
                            if (warehouse_id1 == warehouse_id2) {
                                if (value.relevant_status_id == 6) {
                                    if (value.out_type == 1) {
                                        html += '<button  class="invopt" style="display: inline" onclick="javascript:goWindowUrl(' + value.relevant_id + ');">开始合单</button>';
                                    }
                                }
                                if (value.relevant_status_id == 4) {
                                    ...
                                    html += '<button  class="invopt" style="display: inline" onclick="javascript:orderInventory(' + value.relevant_id + ',' + 6 + ');">查看</button>';
                                }
                            } else {
