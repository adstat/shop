	...
	iorq.order_id = '" . $order_id . "'
	 group by iorq.product_id)";
                ...
                if ($query) {
                    ...
                    $this->addReturn(array($order_id), $result['warehouse_id']);
//                    $sql_order_move = "select  order_id from oc_x_stock_move where order_id = '".$order_id."' and inventory_type_id  = 12  ";
//
//                    $query = $db->query($sql_order_move);
//                    $result_order_move = $query->row;
...
";
 ...
 ('".$warehouse_id."' , '".$stock_area."' , '".$warehouse_section_id."'   , NOW(), '".$inventory_user_id."')";
    ...
    public  function  getOrderByFrame(array  $data){
        ...
        $logistic_allot_id = $data['data']['logistic_allot_id'] ? $data['data']['logistic_allot_id'] : '';
        $warehouse_id = $data['data']['warehouse_id'] ? $data['data']['warehouse_id'] : '';
        $add_user_id = $data['data']['add_user_id'] ? $data['data']['add_user_id'] : '';
...
//                group by cm.container_id order by cm.order_id  ";
        $sql = "SELECT cdr.container_id,cdr.order_id,cdr.checked,cdr.warehouse_id,cdr.logistic_allot_id,la.logistic_driver_title FROM oc_x_container_deliver_return cdr
LEFT JOIN oc_x_logistic_allot la ON la.logistic_allot_id = cdr.logistic_allot_id WHERE 
cdr.logistic_allot_id = '".$logistic_allot_id."'";
//        return $sql;
...
WHERE
	la.logistic_allot_id = '".$logistic_allot_id."'
GROUP BY
	cm.order_id
ORDER BY
	cm.order_id  ";
            ...
            if (!empty($array_data)) {
                ...
                if (empty($array_container[intval($container_id)])) {
                    ...
                    if (!empty($result)) {
                        $sql = "INSERT INTO `oc_x_container_deliver_return` 
(`logistic_allot_id`, `warehouse_id`, `order_id`, `container_id`, `add_w_user_id`, `date_added`, `checked`, `status`) VALUES ";
                        $sql .= "('" . $logistic_allot_id . "','" . $warehouse_id . "','" . $order_id . "','" . $container_id . "','" . $user_id . "',NOW(),1,1)";
                    } else {
                        $return = array(
                            'return_code' => 'ERROR',
                            'return_msg'  => '周转筐号有误',
                    ...
                    $sql = "UPDATE  oc_x_container_deliver_return SET checked = 1 WHERE logistic_allot_id = '".$logistic_allot_id."' AND container_id = '".$container_id."'";
...
GROUP BY  ios.product_id )";
        ...
        if($result['order_id']){
            ...
            $sql = "update oc_x_deliver_order set order_status_id = 6 where order_id = '". $result_order['order_id'] ."'";
            $db->query($sql);
            foreach ($arr_deliver_order_id as $deliver_order_id){
...
//            $sql =  "select  return_confirmed  , product_id  from oc_return_product  where warehouse_id = '".$warehouse_id."' and product_id = '".$product_id ."'  and return_confirmed = 0     ";
    ...
    function delOneInventoryOrderSoring(array $data){
        global $db;
        $order_id= $data['data']['order_id']? $data['data']['order_id'] : '';
        $container_id = $data['data']['container_id']? $data['data']['container_id'] : '';
        $user_id = $data['data']['user_id']? $data['data']['user_id'] : '';
        $product_id=$data['data']['product_id']? $data['data']['product_id'] : '';
        $sql= " update  oc_x_deliver_order_inv set '".$container_id."'=0 where  deliver_order_id = '".$container_id ."' AND product_id = '".$product_id ."'  ";
        $query = $db->query($sql);
        $result = $query->row;
    ...
    function  confirmDoRelevant(array $data ){
        ...
        if($result['order_id'] > 0 ){
            if (intval($result['quantity']) >= intval($result['quantity2'])) {
                ...
                $sql = "update oc_x_inventory_deliver_order_sorting set quantity = quantity + ".intval($quantity)." where warehouse_id = '".$warehouse_id."' and product_id = '".$product_id."' and deliver_order_id = '".$deliver_order_id."'";
...
//            }
    ...
    function  addConsolidatedDoInfo(array $data ){
        ...
        $this->change_come_deliver_order_inv($warehouse_id,$result_warehouse['order_id'],1);
...
GROUP BY order_id ) ";
            ...
            if ($order_inv_order_id['order_id'] > 0) {
                if ($result['num'] == 1) {
                    $this->change_come_deliver_order_inv($warehouse_id,$data_inv['order_id'],2);
                }
...
IF (doo.is_urgent = 1, '1', '0') urgent,
 ...
 (
   ...
   WHERE  doo.do_warehouse_id = '".$warehouse_id ."' and doo.warehouse_id = '".$to_warehouse_id ."' and (doo.do_warehouse_id = doo.warehouse_id OR (doo.do_warehouse_id != doo.warehouse_id AND doo.box_only != 1)) and doo.order_status_id =  2  ";
...
//        }
    ...
    public  function updateDoStatus(array $data ){
        ...
        $sql_order = "select o.order_status_id ,doo.order_status_id do_order_status_id  ,doo.order_id from  oc_x_deliver_order doo left join oc_order o on doo.order_id = o.order_id where doo.deliver_order_id = '".$order_id."' ";
        ...
        if($resutl['order_status_id'] == 6  || $resutl['do_order_status_id'] !=6 ) {
            ...
            $sql1 = " select deliver_order_id  from oc_x_deliver_order where  order_id = '".$resutl['order_id']."' and do_warehouse_id != warehouse_id  ";
            $query = $db->query($sql1);
            $resutl1 = $query->row;

            if($resutl1['deliver_order_id'] > 0 ){
                $sql = "update oc_x_inventory_order_return_quantity set status = 0  where deliver_order_id  = '".$resutl1['deliver_order_id']."' ";
                $query = $db->query($sql);
            }
        ...
        if(!$deliverDate || !$doWarehouseId || !$toWarehouseId){
            ...
            if ($type == 1) {
             ...
             LEFT JOIN oc_order oo ON oo.order_id = o.order_id 
              ...
              WHERE o.order_status_id IN (6,8)  AND o.deliver_date = '".$deliverDate."' AND o.do_warehouse_id = '".$doWarehouseId."' AND o.warehouse_id = '".$toWarehouseId."' AND oo.order_status_id != 3 AND oo.order_status_id != 6 
            ";
            //return $sql;
...
WHERE o.order_status_id IN (6) AND o.order_deliver_status_id = 1 
...
warehouse_id = '".$warehouse_id ."'";
                    ...
                    $sql_info = "SELECT doo.deliver_order_id,doo.order_id,p.name,p.product_id,doo.deliver_date,
	(SUM(dop.quantity) -IF (dos.quantity > 0,dos.quantity,0)) quantity
FROM
	oc_x_deliver_order doo
LEFT JOIN oc_x_deliver_order_product dop ON doo.deliver_order_id = dop.deliver_order_id
...
ORDER BY
	quantity DESC";
                    ...
                    $sql_info = "SELECT ".$product_id." AS container_id,doi.order_id,doi.deliver_order_id,odo.deliver_date,
	( 1 -IF (dos.container_id > 0,1,0)) quantity
FROM
...
//return $psize;
        if($warehouse_id){
            $return = $this->get_come_deliver_order_ids($warehouse_id,$psize,$start_row,$deliver_date,$condition_type);
            if ($this->check_result_is_true($return)){
                return $return;
            }
            ...
            $sql_info = "SELECT doo.deliver_order_id,doo.warehouse_id,doo.do_warehouse_id,oxw.title,doo.order_status_id AS order_status_id2,dos.name AS name2,
...
//return $result_info;
     ...
     * */
    function get_come_deliver_order_ids($warehouse_id,$psize,$start_row,$deliver_date,$condition_type){
        global  $db ;
        $order_info = [];
        $deliver_order_ids = '';
        $order_ids = '';
...
AND doo.do_warehouse_id != o.warehouse_id
...
WHERE
doo.do_warehouse_id != '" . $warehouse_id . "'
AND doo.warehouse_id = '" . $warehouse_id . "'
AND doo.order_id = '".$order_id."'
GROUP BY
	doo.deliver_order_id";
//        return $sql;
        $query = $db->query($sql);
...
GROUP BY
	doo.deliver_order_id LIMIT 0,1";
...
FROM
	oc_x_inventory_deliver_order_sorting ios
LEFT JOIN (SELECT deliver_order_id,order_id,price,sum(quantity) AS quantity ,product_id  FROM oc_x_deliver_order_product WHERE order_id = '".$order_id."' GROUP BY deliver_order_id,product_id ) dop ON ios.deliver_order_id = dop.deliver_order_id
AND ios.product_id = dop.product_id
WHERE
...
AND dos.warehouse_id = dos.do_warehouse_id
GROUP BY
	ios.product_id";
//        return $sql2;
            $query2 = $db->query($sql2);
...
//                group by op.product_id";
    ...
    function submit_box_product_batch(array $data) {
        ...
        $bool && $dbm->query($sql);
        $sql = "UPDATE oc_x_deliver_order SET order_status_id = 6,box_status_id = 6 WHERE order_status_id != 3 AND batch_id = '".$reqId."'";
        $bool && $dbm->query($sql);
