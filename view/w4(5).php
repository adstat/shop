...
function chkGoods(product) {
    ...
    if((product.length == 6) || reg.test(product)){
       ...
       handleFrameList('vg',product);
        var look=getFrameAllProduct(product);
        console.log(look);
        alert('getFrameAllProduct');
...
function isNullProduct() {
    ...
    var frame=frameNum.split(',');
    $.each(frame,function(k,v){
        var str=$("#show_container_"+v).children().children();
        var count=str.indexOf(str);
        var nowSum=str?count:parseInt(0);
        return nowSum;
    })
}
/*
...
function soundEffectInit(){
    ...
    * 货位号扫描*/
    function handleStockArea(tmptxt,warehouse_id,is_repack){
        ...
        // console.log(tmptxt);
        if (warehouse_id == 14) {
            if ((3000 < tmptxt && tmptxt <= 4000) || tmptxt< 1000) {
