 ...
 * Time: 14:10
 */
class ControllerLogisticLogisticAllot extends Controller
{
    public function index(){
        ...
        $data['breadcrumbs'][] = array(
            'text' => '订单分派汇总',
            'href' => $this->url->link('logistic/logistic_allot', 'token=' . $this->session->data['token'], 'SSL')
        );

        $this->load->model('logistic/logistic');
        //$data['allot_info'] = json_encode($this->getAllotInfo(1), JSON_UNESCAPED_UNICODE);
        $data['allot_info'] = json_encode(array(), JSON_UNESCAPED_UNICODE);
        ...
        $data['line'] = json_encode($this->model_logistic_logistic->getLineData(), JSON_UNESCAPED_UNICODE);
        ...
        $this->response->setOutput($this->load->view('logistic/logistic_allot.tpl', $data));
    }
