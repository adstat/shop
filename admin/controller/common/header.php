...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		//调用系统化启动时候加载的仓库信息，渲染到header模板中
		$data['warehouse'] = $this->warehouse->getWarehouse();

		//获取当前URL,并且去除之前url中的全局仓库
		$url_array = explode('&',$this->request->server['REQUEST_URI']);
		foreach($url_array as $k => $value){
			if(strpos($value,'filter_warehouse_id_global')){
				unset($url_array[$k]);
			}
		}

		$refresh_url = implode('&',$url_array);
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) && ($this->request->get['token'] != $this->session->data['token'])) {
            ...
            /* Currently, We don't need these info
             *
			// Orders
