...
class ControllerCatalogPrice extends Controller
{
    ...
    public function upload_post(){
        ...
        for($i=1; $i<=$allRow-1; $i++){
            ...
            $products[$i]['purchase_in_pcs']   = (float)$currentSheet->getCell('C' . $m)->getValue();
    ...
    public function upload_confirm(){
        ...
        if($this->request->server['REQUEST_METHOD'] == 'POST' && $this->request->post['uploaded_file'] &&
            ...
            //增加采购
            $sql = "INSERT INTO `oc_x_purchase_order_product` (`purchase_order_id`, `product_id`, `weight_class_id`, `date`, `purchase_in_pcs`, `purchase_price_500g`, `purchase_qty_500g`, `purchase_total`) VALUES ";
            $m=0;
            foreach($products as $product){
                $sql .= "(".$purchase_order_id.", '".$product['product_id']."', 0, '".$date."', '".$product['purchase_in_pcs']."', '".$product['purchase_price_500g']."', '".$product['purchase_qty_500g']."', '".$product['purchase_total']."')";
            ...
            $sql = "update oc_x_purchase_order_product A left join oc_product B on A.product_id = B.product_id set A.weight_class_id = B.weight_class_id, A.weight = B.weight where A.date = '".$date."' and A.status = 1";
            $bool = $bool && $this->db->query($sql);
