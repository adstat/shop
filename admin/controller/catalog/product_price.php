...
class ControllerCatalogProductPrice extends Controller
{
    ...
    {
        $this->document->setTitle('商品价格维护');
        $data['heading_title'] = '商品价格维护';
        ...
        $data['action_upload']  = $this->url->link('catalog/product_price/upload_post', 'token=' . $this->session->data['token'], 'SSL');
        $data['action_adjust'] = $this->url->link('catalog/product_price/adjust_post', 'token=' . $this->session->data['token'], 'SSL');

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');
        $this->response->setOutput($this->load->view('catalog/product_price.tpl', $data));
    }
    ...
    public function upload_post(){
        ...
        $target = DIR_UPLOAD .'product_price/upload_' . date('YmdHis') . '_' . $this->user->getId() . '.' . $file_extension;
        ...
        //过滤重复项目
        $checkRepeat = array_count_values($product_ids);
        $data['repeatIds'] = array();
        foreach($checkRepeat as $key=>$val){
            if($val > 1){
                $data['repeatIds'][$key] = $val;
            }
        }
        if(sizeof($data['repeatIds'])){
            $data['error_warning'] = "商品信息重复";
        }
        ...
        $this->document->setTitle('商品价格维护－上传文件');
        $data['heading_title'] = '商品价格维护－上传文件';
        $data['breadcrumbs'] = array();
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('text_home'),
            'href' => $this->url->link('common/dashboard', 'token=' . $this->session->data['token'], 'SSL')
        ...
        $data['action_confirm'] = $this->url->link('catalog/product_price/upload_confirm', 'token=' . $this->session->data['token'], 'SSL');
        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');
        $this->response->setOutput($this->load->view('catalog/product_price_upload_post.tpl', $data));
    }
    ...
    public function upload_confirm(){
        ...
        if($this->request->server['REQUEST_METHOD'] == 'POST' && $this->request->post['uploaded_file'] &&
            ...
            for($i=1; $i<=$allRow-1; $i++){
                ...
                $products[$i]['product_id'] = (int)$currentSheet->getCell('A' . $m)->getValue();
                $products[$i]['name']    = $currentSheet->getCell('B' . $m)->getValue();
                $products[$i]['ori_retail_price']   = (float)$currentSheet->getCell('C' . $m)->getValue();
                $products[$i]['retail_price']   = (float)$currentSheet->getCell('D' . $m)->getValue();
                $products[$i]['memo']   = $currentSheet->getCell('E' . $m)->getValue();
            ...
            $data['footer'] = $this->load->controller('common/footer');
            $this->response->setOutput($this->load->view('catalog/product_price.tpl', $data));
            $this->response->redirect($this->url->link('catalog/product_price', 'token=' . $this->session->data['token'], 'SSL'));
        }else{
            exit('文件或文件上传时间请求错误.');
        }
    }
    ...
    public function getUploadPriceByDate(){
        ...
        $sql = "select
            A.product_id, B.name,  A.time, A.last_retail_price, A.upload_last_retail_price,  A.upload_retail_price,
            concat(C.lastname, C.firstname) addedby, C.username,  A.memo,  A.checked, A.check_time, D.upload_times, D.upload_history
