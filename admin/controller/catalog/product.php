...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_product->addProductWs($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$edit_preson_id = $this->user->getId();
			$filter_warehouse_id_global = $this->warehouse->getWarehouseIdGlobal();

			$this->model_catalog_product->editProduct($this->request->get['product_id'], $this->request->post,$edit_preson_id,$filter_warehouse_id_global);
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_product->editProductWs($this->request->get['product_id'], $this->request->post);
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_product_type_id'])) {
			$filter_product_type_id = $this->request->get['filter_product_type_id'];
		} else {
			$filter_product_type_id = null;
		}
		...
		if (isset($this->request->get['filter_page_id'])) {
			$url .= '&filter_page_id=' . urlencode(html_entity_decode($this->request->get['filter_page_id'], ENT_QUOTES, 'UTF-8'));
		}
		...
		if (isset($this->request->get['filter_product_type_id'])) {
			$url .= '&filter_product_type_id=' . $this->request->get['filter_product_type_id'];
		}
		...
		//重新加载语言包，确保当前脚本的语言包与header中语言包不冲突
		$this->load->language('catalog/product');
		...
		$data['add'] = $this->url->link('catalog/product/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
                $data['add_ws'] = $this->url->link('catalog/product/add_ws', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$filter_data = array(
			...
			'limit'           => $filter_page_id ? $filter_page_id : $this->config->get('config_limit_admin')
		);

		$this->load->model('tool/image');

		$product_total = $this->model_catalog_product->getTotalProducts($filter_data,$data['filter_warehouse_id_global']);
...
//		$product_total = sizeof($results)?sizeof($results):0;
		...
		foreach ($results as $result) {
			...
			$limit_num = ($result['max_limit']-$result['min_limit']) > 0 ? $result['max_limit'].'-'.$result['min_limit'] : $result['min_limit'];
			if($result['warehouse_num']){
				if($result['warehouse_num'] == $result['s_w_num']){
					$warehouse_sale = "[所有仓库在售]";
				}else{
					$warehouse_sale = "[多仓在售]";
				}
			}else{
			...
			$data['products'][] = array(
                ...
                //'stationTitle'=>$stationInfo[$result['station_id']]['name'],

				'date_new_on' => $result['date_new_on'],
				'date_new_off' => $result['date_new_off'],
...
//				'price'      => $result['price'],
				...
				'w_size'     => $result['w_size'],
				'weight_inv_flag' => $result['weight_inv_flag'],
				...
				'edit'       => $result['product_type'] == 3 ? $this->url->link('catalog/product/edit_ws', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL') : $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL')
			);
		}
		...
		$data['categories'] = $this->model_catalog_category->getCategories(array('sort' => 'name','filter_status'=>1));
		//商品平台
		...
		//商品type分类
		$data['all_product_type_id'] = $this->model_station_station->getProductTypeList();
		...
		$data['filter_product_type_id'] = $filter_product_type_id;
		$data['filter_page_id'] = $filter_page_id;
	...
	protected function getFormBasic() {
		$data['header'] = $this->load->controller('common/header');
		...
		if (isset($this->error['name'])) {
			...
			$data['error_name'] = array();
                ...
                if (isset($this->error['product_sku_id'])) {
			$data['error_product_sku_id'] = $this->error['product_sku_id'];
		} else {
			$data['error_product_sku_id'] = '';
		}
		...
		//权限控制
		$userGroupId =  $this->user->getGroupId();
		$passedGroupArr = unserialize(ALLOW_PRODUCT_HIGHT_MANAGE);

		if(in_array($userGroupId,$passedGroupArr)){
			$data['rbac'] = true;
		}else{
			$data['rbac'] = false;
		}
		...
		$data['purchase_cost'] = $product_id ? $this->model_catalog_product->getPurchasePrice($product_id) : 0;
		$data['protect_price'] = $product_id ? $this->model_catalog_product->getPriceProtect($product_id) : 0;

		$product_info = $product_id ? $this->model_catalog_product->getProduct($product_id) : array();
                
                $data['date_new_on'] = $product_id ? $product_info['date_new_on'] : '0000-00-00';
                $data['date_new_off'] = $product_id ? $product_info['date_new_off'] : '0000-00-00';
		...
		$data['name'] = $product_id ? $product_info['name'] : '';
		...
		$data['price_protect']  = $product_id ? $product_info['price_protect'] : '';
		$data['cashback']    = $product_id ? $product_info['cashback'] : 0;
		$data['retail_price']    = $product_id ? $product_info['retail_price'] : '';
		...
		$data['maximum'] = $product_id ? $product_info['maximum'] : '50';
		...
		$data['shelf_life'] = $product_id ? $product_info['shelf_life'] : '';
		$data['issupportstore'] = $product_id ? $product_info['issupportstore'] : '1';
		$data['sort_order'] = $product_id ? $product_info['sort_order'] : '1';

                $data['inv_class_sort'] = $product_id ? $product_info['inv_class_sort'] : 0;
                $data['storage_mode_id'] = $product_id ? $product_info['storage_mode_id'] : 0;
                
                $data['product_type'] = $product_id ? $product_info['product_type'] : 1;
                $data['station_id'] = $product_id ? $product_info['station_id'] : 1;
                $data['weight_inv_flag'] = $product_id ? $product_info['weight_inv_flag'] : 0;
				$data['agent_id'] = $product_id ? $product_info['agent_id'] : 0;
				$data['is_gift'] = $product_id ? $product_info['is_gift'] : 0;
				$data['repack'] = $product_id ? $product_info['repack'] : 0;
				$data['is_replenish_gift'] = $product_id ? $product_info['is_replenish_gift'] : 0;
				$data['instock'] = $product_id ? $product_info['instock'] : 0;
				$data['is_selected'] = $product_id ? $product_info['is_selected'] : 0;
				$data['is_soon_to_expire'] = $product_id ? $product_info['is_soon_to_expire'] : 0;

                $data['safestock'] = $product_id ? $product_info['safestock'] : 0;
		...
		$data['product_type_ids'] = $this->model_user_warehouse_distribution->getProductTypeList();
		$data['weight_class_id'] = $product_id ? $product_info['weight_class_id'] : $this->config->get('config_weight_class_id');
		$data['product_type_ids'] = $product_id ? $product_info['product_type_id'] : '';

                $data['unit_size'] = $product_id ? $product_info['unit_size'] : '';
                $data['unit_weight_class_id'] = $product_id ? $product_info['unit_weight_class_id'] : '';
                
                $data['inv_size'] = $product_id ? $product_info['inv_size'] : '';
		// 尺寸
...
//		}
        ...
        foreach ($product_discounts as $product_discount) {
            $data['product_discounts'][] = array(
				...
				'username'   => $product_discount['username']
            );
        }
...
//        }
		...
		$data['product_description'] = $product_id ? $this->model_catalog_product->getProductDescriptions($product_id) : array();
                ...
                //商品原料
                $data['product_sku'] = $product_id ? $this->model_catalog_product->getProductSku($product_id) : array();
                $data['product_sku_id'] = isset($data['product_sku']['sku_id']) ? $data['product_sku']['sku_id'] : 0;
		// 其它
		$data['error_warning'] = '';
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		...
		$warehouses = $this->model_station_station->getWarehouseBelongToStation($station_id,$data['filter_warehouse_id_global']);
		$data['warehouses'] = $warehouses;

		//寻找该商品对应仓库里的价格等信息
		$warehousePrices = $this->model_catalog_product->getProductInAllWarehouse($product_id);

		$data['warehousePrices'] = $warehousePrices;

		$this->response->setOutput($this->load->view('catalog/product_form_basic.tpl', $data));
	}

        protected function getFormBasicWs() {
		$this->document->addStyle('view/javascript/validform/validform_v5.3.2.css');
		$this->document->addScript('view/javascript/validform/validform_v5.3.2_min.js');

		$data['heading_title'] = "散菜商品";
		$data['text_enabled'] = $this->language->get('text_enabled');
		$data['text_disabled'] = $this->language->get('text_disabled');
		$data['text_none'] = $this->language->get('text_none');
		$data['text_yes'] = $this->language->get('text_yes');
...
//var_dump($data['error_price']);die;
		...
		if (!isset($this->request->get['product_id'])) {
			$data['action'] = $this->url->link('catalog/product/add_ws', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/product/edit_ws', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}
		$data['cancel'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['token'] = $this->session->data['token'];
		...
		$data['sort_order'] = $product_id ? $product_info['sort_order'] : '1';
                ...
                $data['weight_type'] = $product_id ? $product_info['weight_type'] : '';
                $data['box_size'] = $product_id ? $product_info['box_size'] : '';
                $data['box_weight_class_id'] = $product_id ? $product_info['box_weight_class_id'] : '';
                $data['unit_size'] = $product_id ? $product_info['unit_size'] : '';
                $data['inv_size'] = $product_id ? $product_info['inv_size'] : '';
                $data['unit_weight_class_id'] = $product_id ? $product_info['unit_weight_class_id'] : '';
                $data['unit_price'] = $product_id ? $product_info['unit_price'] : '';
                $data['fix_discount'] = $product_id ? $product_info['fix_discount'] : '';
                $data['weightloss_rate'] = $product_id ? $product_info['weightloss_rate'] : '';
                $data['purchase_cost'] = $product_id ? $product_info['purchase_cost'] : '';
                $data['warehouse_cost'] = $product_id ? $product_info['warehouse_cost'] : '';
                $data['repack_cost'] = $product_id ? $product_info['repack_cost'] : '';
                $data['package_weight'] = $product_id ? $product_info['package_weight'] : '';
        ...
        $data['button_save'] = '保存';
        $data['button_cancel'] = '取消';
		$this->response->setOutput($this->load->view('catalog/product_form_basic_ws.tpl', $data));
	}
...
//		}
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && !isset($this->request->get['product_id'])) {
                ...
                */
                if (!$this->request->post['product_sku_id'] || (utf8_strlen($this->request->post['product_sku_id']) < 1)) {
                        $this->error['product_sku_id'] = "请选择原料";
                }
                if ((utf8_strlen($this->request->post['sku']) < 1)) {
                        $this->error['sku'] = "请输入SKU/商品编码";
                }
...
//echo '<pre>';var_dump($this->error);die;
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_product_id'])) {
            ...
            $stationInfo = $this->model_catalog_product->getStationInfo();
            // Station Info return array('station_id' => array('name'=> 'Station Name',...))

            foreach ($results as $result) {
                //Alex, Disable Product Option
//				$option_data = array();
//
...
//				}
				...
				$json[] = array(
                    ...
                    'station' => $stationInfo[$result['station_id']]['name'],
					//'option'     => $option_data,
        ...
        public function product_sku_autocomplete() {
		$json = array();
		...
		if (isset($this->request->get['filter_product_sku'])) {
			...
			$results = $this->model_catalog_product->getProductSkus($filter_data);

			foreach ($results as $result) {
				
				$json[] = array(
					'product_sku_id' => $result['sku_id'],
					'name'       => strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8')),
	...
	public function specialhistory() {
		$this->load->language('catalog/product');
		...
		$results = $this->model_catalog_product->getSpecialPriceHistory($this->request->get['product_id'], ($page - 1) *10, 10);
		$history_total = $this->model_catalog_product->getTotalSpecialPriceHistory($this->request->get['product_id']);

		$data['specialhistory'] = array();
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('catalog/product/specialhistory', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('catalog/product_special_history.tpl', $data));
	}
	...
	public function producthistory() {
		...
		$results = $this->model_catalog_product->getProductModifyHistory($this->request->get['product_id'], ($page - 1) *10, 10);
		$history_total = $this->model_catalog_product->getTotalProductModifyHistory($this->request->get['product_id']);

		$data['modifyhistory'] = array();
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('catalog/product/producthistory', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('catalog/product_modify_history.tpl', $data));
	}
}