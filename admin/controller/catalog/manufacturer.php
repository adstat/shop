...
class ControllerCatalogManufacturer extends Controller {
	...
	protected function getForm() {
		...
		//制造商品牌
		if (isset($this->request->post['series_name'])) {
			$series = $this->request->post['series_name'];
		} elseif (isset($this->request->get['manufacturer_id'])) {
			$series =  $this->model_catalog_manufacturer->getManufacturerSeries($this->request->get['manufacturer_id']);
		} else {
			$series = array();
		}

		$data['series_name'] = array();
		foreach($series as $sery){
			$data['series_name'][] = array(
				'name'	=> $sery['name'],
				'manufacturer_id'	=> $sery['manufacturer_id'],
			);
		}
