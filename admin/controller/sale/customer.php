...
//            }
//
//            $this->response->redirect($this->url->link('sale/customer', 'token=' . $this->session->data['token'] . $url,
//                'SSL'));
        }
...
//		}
        ...
        $data['delete'] =
            $this->url->link('sale/customer/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
...
//		}
        ...
        $pagination->url =
            $this->url->link('sale/customer', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
//		$data['filter_ip'] = $filter_ip;
    ...
    protected function getForm()
    {
        ...
        $language = [
            'column_select_name', 'column_bd', 'column_merchant_name', 'column_merchant_address',
            'entry_shipping_phone', 'entry_address_name', 'entry_address_firstname', 'tab_order'
        ];
//        $language = array_merge($language_common, $language);
        foreach ($language as $value) {
            $data[$value] = $this->language->get($value);
        ...
        $data['text_form'] = !isset($this->request->get['customer_id']) ? $this->language->get('text_add') :
...
//        }
        ...
        if (isset($this->error['error_merchant_address'])) {
            $data['error_merchant_address'] = $this->error['error_merchant_address'];
        } else {
            $data['error_merchant_address'] = '';
        }
...
//		}
        ...
        if (!isset($this->request->get['customer_id'])) {
            $data['action'] =
                $this->url->link('sale/customer/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
            $data['action'] = $this->url->link('sale/customer/edit',
                'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url,
                'SSL');
        }
        ...
        if (isset($this->request->get['customer_id'])) {
            ...
            $order_infos = $this->model_sale_customer->getOrder($this->request->get['customer_id'],0,$this->config->get('config_limit_admin'));
            foreach ($order_infos as &$order_info) {
                $order_info['station_id'] = $this->model_station_station->getStationNameById($order_info['station_id']);
                $order_info['bd_id'] = $this->model_common_common->getBdNameByBdId($order_info['bd_id']);

                $order_info['order_status_id'] =
                    $this->model_common_common->getOrderStatusName($order_info['order_status_id']);
                $order_info['order_payment_status_id'] =
                    $this->model_common_common->getPaymentName($order_info['order_payment_status_id']);
                $order_info['order_deliver_status_id'] =
                    $this->model_common_common->getOrderDeliverName($order_info['order_deliver_status_id']);
            }
...
//		}
        if (isset($this->request->post['merchant_name'])) {
            ...
            $data['merchant_name'] = $customer_info['merchant_name'];
        } else {
            $data['merchant_name'] = '';
        }

        if (isset($this->request->post['merchant_address'])) {
            $data['merchant_address'] = $this->request->post['merchant_address'];
        } elseif (!empty($customer_info)) {
            $data['merchant_address'] = $customer_info['merchant_address'];
        } else {
            $data['merchant_address'] = '';
        }

        if (isset($this->request->post['bd_id'])) {
            $data['bd_id'] = $this->request->post['bd_id'];
        } elseif (!empty($customer_info)) {
            $data['bd_id'] = $customer_info['bd_id'];
        } else {
            $data['bd_id'] = '';
        }

        if (isset($this->request->post['is_nopricetag'])) {
            $data['is_nopricetag'] = $this->request->post['is_nopricetag'];
        } elseif (!empty($customer_info)) {
            $data['is_nopricetag'] = $customer_info['is_nopricetag'];
        } else {
            $data['is_nopricetag'] = 0;
        }
...
//		}
        ...
        $data['transaction_types'] = $this->model_sale_customer->getTransactionType();

        $data['admin_user_id'] = $this->user->getId();
        if (isset($this->request->get['customer_id'])) {
            $data['order_ids'] = $this->model_sale_customer->getOrderIds($this->request->get['customer_id']);

            $data['customer_container_move'] =
                $this->model_sale_customer->getContainerMove($this->request->get['customer_id']);
        }
    ...
    protected function validateForm()
    {
        ...
        $custom_fields =
            $this->model_sale_custom_field->getCustomFields(array('filter_customer_group_id' => $this->request->post['customer_group_id']));

        foreach ($custom_fields as $custom_field) {
            if (($custom_field['location'] == 'account') && $custom_field['required'] &&
                empty($this->request->post['custom_field'][$custom_field['custom_field_id']])
            ) {
                $this->error['custom_field'][$custom_field['custom_field_id']] =
...
//                    (utf8_strlen($value['postcode']) < 2 || utf8_strlen($value['postcode']) > 10)
//                ) {
...
//
//                foreach ($custom_fields as $custom_field) {
//                    if (($custom_field['location'] == 'address') && $custom_field['required'] &&
//                        empty($value['custom_field'][$custom_field['custom_field_id']])
//                    ) {
//                        $this->error['address'][$key]['custom_field'][$custom_field['custom_field_id']] =
//                            sprintf($this->language->get('error_custom_field'), $custom_field['name']);
//                    }
//                }
//            }
//        }
    ...
    public function history()
    {
        ...
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('sale/customer/history',
            'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] .
            '&page={page}', 'SSL');

        $data['pagination'] = $pagination->render();

        $data['results'] =
            sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0,
                ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10),
                $history_total, ceil($history_total / 10));
    ...
    public function transaction()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') &&
            $this->user->hasPermission('modify', 'sale/customer')
        ) {
            $this->model_sale_customer->addTransaction($this->request->get['customer_id'],
                ...
                $this->request->post['transaction_order_id'], $this->request->post['transaction_type']);
        ...
        $results =
            $this->model_sale_customer->getTransactions($this->request->get['customer_id'], ($page - 1) * 10, 10);
        ...
        foreach ($data['transactions'] as $t_k => $t_v) {
            if ($t_v['change_id'] > 0) {
                $data['transactions'][$t_k]['date_url'] =
                    SITE_URI . '/www/admin/inv_change.php' . '?change_id=' . $t_v['change_id'];
            }
            if ($t_v['return_id'] > 0) {
                $data['transactions'][$t_k]['date_url'] = $this->url->link('sale/return/edit',
                    'token=' . $this->session->data['token'] . '&return_id=' . $t_v['return_id'], 'SSL');
            }
        }


        $data['balance'] =
            $this->currency->format($this->model_sale_customer->getTransactionTotal($this->request->get['customer_id']),
        ...
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('sale/customer/transaction',
            'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] .
            '&page={page}', 'SSL');
        ...
        $data['results'] =
            sprintf($this->language->get('text_pagination'), ($transaction_total) ? (($page - 1) * 10) + 1 : 0,
                ((($page - 1) * 10) > ($transaction_total - 10)) ? $transaction_total : ((($page - 1) * 10) + 10),
                $transaction_total, ceil($transaction_total / 10));
    ...
    public function reward()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') &&
            ...
            $this->model_sale_customer->addReward($this->request->get['customer_id'],
                $this->request->post['description'], $this->request->post['points']);
...
//                'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added']))
        ...
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('sale/customer/reward',
            'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] .
            '&page={page}', 'SSL');
        ...
        $data['results'] =
            sprintf($this->language->get('text_pagination'), ($reward_total) ? (($page - 1) * 10) + 1 : 0,
                ((($page - 1) * 10) > ($reward_total - 10)) ? $reward_total : ((($page - 1) * 10) + 10), $reward_total,
                ceil($reward_total / 10));
    ...
    public function ip()
    {
        ...
        foreach ($results as $result) {
            ...
            $data['ips'][] = array(
                ...
                'filter_ip' => $this->url->link('sale/customer',
                    'token=' . $this->session->data['token'] . '&filter_ip=' . $result['ip'], 'SSL'),
                'ban_ip' => $ban_ip_total
            );
        }
        ...
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('sale/customer/ip',
            'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] .
            '&page={page}', 'SSL');

        $data['pagination'] = $pagination->render();

        $data['results'] = sprintf($this->language->get('text_pagination'), ($ip_total) ? (($page - 1) * 10) + 1 : 0,
            ((($page - 1) * 10) > ($ip_total - 10)) ? $ip_total : ((($page - 1) * 10) + 10), $ip_total,
            ceil($ip_total / 10));
    ...
    public function customfield()
    {
        ...
        $custom_fields =
            $this->model_sale_custom_field->getCustomFields(array('filter_customer_group_id' => $customer_group_id));
    ...
    public function order()
    {
        ...
        if (isset($this->request->get['customer_id'])) {
            ...
            $this->load->model('station/station');
            $this->load->model('common/common');
            $num = $this->config->get('config_limit_admin');
            $start = ($this->request->get['page']-1) * $num;
            $order_infos = $this->model_sale_customer->getOrder($this->request->get['customer_id'], $start, $num);
            foreach ($order_infos as &$order_info) {
                $order_info['station_id'] = $this->model_station_station->getStationNameById($order_info['station_id']);
                $order_info['bd_id'] = $this->model_common_common->getBdNameByBdId($order_info['bd_id']);
                $order_info['order_status_id'] =
            ...
            $this->load->model('sale/customer');
            $customer_total = $this->model_sale_customer->getOrderTotal($this->request->get['customer_id']);
            if (isset($this->request->get['page'])) {
                $page = $this->request->get['page'];
            } else {
                $page = 1;
            ...
            $pagination->page = $page;
            $pagination->limit = $this->config->get('config_limit_admin');
            $pagination->url = $this->url->link('{page}');
