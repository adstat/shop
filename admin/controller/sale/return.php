...
class ControllerSaleReturn extends Controller {
	...
	public function delete() {
		...
		$this->load->model('sale/return');
                ...
                $return_ope = $_POST['return_ope'];

		$return = $this->model_sale_return->editReturnStatus($return_id,$return_ope);
                if($return['error'] == 1){
                    $json['success'] = 0;
                }
                else{
	...
	public function update_credits(){
		...
		$return = $this->model_sale_return->editReturnCredits($return_id,$return_ope);
		if($return['error'] == 1){
			$json['success'] = 0;
		}
		else{
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['returns'][] = array(
				...
				'date_added'    => date($this->language->get('date_format_short'), strtotime($result['return_added'])),
				'date_modified' => date($this->language->get('date_format_short'), strtotime($result['return_modified'])),
				'edit'          => $this->url->link('sale/return/edit', 'token=' . $this->session->data['token'] . '&return_id=' . $result['return_id'] . $url, 'SSL'),
                                ...
                                'return_credits' => $result['return_credits'],
                                'return_inventory_flag' => $result['return_inventory_flag'],
                                'username' => $result['username'],
				'credits_returned' => $result['credits_returned'],
				'return_action_id' => $result['return_action_id'],
				'return_total' => $result['return_total'],
				'return_real_quantity' => $result['return_real_quantity'],
	...
	protected function getForm() {
		...
		$data['return_statuses'] = $this->model_localisation_return_status->getReturnStatuses();
                ...
                $data['return_products'] = $this->model_sale_return->getReturnProducts(array('filter_return_id'=>$data['return_id']));
                
                foreach($data['return_products'] as $k=>$v){
                    $data['return_products'][$k]['product_url'] = $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $v['rp_product_id'], 'SSL');
                    $data['return_products'][$k]['order_url'] = $this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $v['order_id'], 'SSL');
                }
                
                
                /*
                echo "<pre>";
                print_r($data['return_products']);
                echo "</pre>";
                */
