<?php
class ControllerUserContainer extends Controller {
	private $error = array();
	...
	protected function getList() {
		if (isset($this->request->get['filter_container_id'])) {
			...
			$filter_container_id = null;
                ...
                if (isset($this->request->get['filter_container_instore'])) {
			$filter_container_instore = $this->request->get['filter_container_instore'];
		} else {
			$filter_container_instore = null;
		}
			...
			$filter_container_outdate = null;
                ...
                if (isset($this->request->get['filter_container_indate'])) {
			$filter_container_indate = $this->request->get['filter_container_indate'];
		} else {
			$filter_container_indate = null;
		}
                
                if (isset($this->request->get['filter_container_status'])) {
			$filter_container_status = $this->request->get['filter_container_status'];
		} else {
			$filter_container_status = null;
		}
		...
		if (isset($this->request->get['filter_container_type'])) {
			...
			$url .= '&filter_container_outdate=' . $this->request->get['filter_container_outdate'];
                ...
                if (isset($this->request->get['filter_container_indate'])) {
			$url .= '&filter_container_indate=' . $this->request->get['filter_container_indate'];
		}

		if (isset($this->request->get['filter_container_status'])) {
			$url .= '&filter_container_status=' . $this->request->get['filter_container_status'];
		}
		...
		$data['add'] = $this->url->link('user/container/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('user/container/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');

		$data['containers'] = array();

		$filter_data = array(
			'filter_container_id'              => $filter_container_id,
			'filter_container_instore'             => $filter_container_instore,
			'filter_container_type' => $filter_container_type,
                        'filter_container_outdate' => $filter_container_outdate,
                        'filter_container_indate' => $filter_container_indate,
                        
			'filter_container_status'            => $filter_container_status,
		...
		$container_total = $this->model_user_container->getTotalContainers($filter_data);

		$results = $this->model_user_container->getContainers($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['containers'][] = array(
				...
				'status'          => $result['status'] ? "可用" : "废弃",
				'instore' => $result['instore'] ? "在库" : "在商家",
				
				'edit'           => $this->url->link('user/container/edit', 'token=' . $this->session->data['token'] . '&container_id=' . $result['container_id'] . $url, 'SSL')
			);
		}
		...
		if (isset($this->request->get['filter_container_instore'])) {
			$url .= '&filter_container_instore=' . $this->request->get['filter_container_instore'];
		}
		...
		$data['sort_container_id'] = $this->url->link('user/container', 'token=' . $this->session->data['token'] . '&sort=container_id' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('user/container', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($container_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($container_total - $this->config->get('config_limit_admin'))) ? $container_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $container_total, ceil($container_total / $this->config->get('config_limit_admin')));

		$data['filter_container_id'] = $filter_container_id;
		$data['filter_container_status'] = $filter_container_status;
		$data['filter_container_type'] = $filter_container_type;
                $data['filter_container_outdate'] = $filter_container_outdate;
                $data['filter_container_indate'] = $filter_container_indate;
		$data['filter_container_instore'] = $filter_container_instore;
		...
		$data['footer'] = $this->load->controller('common/footer');
                ...
                $data['container_no_move_order'] = $this->model_user_container->getContainerNoMoveOrders();
                
		$this->response->setOutput($this->load->view('user/container_list.tpl', $data));
	}
