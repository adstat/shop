  ...
  <div class="container-fluid">
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-pencil"></i>商品价格数据上传</h3>
      </div>
      <div class="panel-body">
          <div class="alert alert-warning">
              <span style="font-size: 1.2rem">上传的商品价格将于每日15:30及23:30自动更新！</span>当日上传的相同商品编号的价格数据，仅最后一次有效
          </div>
          ...
          <div class="tab-content">
            <div class="tab-pane active" id="tab-general">
              <div class="tab-content">
                  <div class="alert alert-info">EXECEL文件不可带有公式，商品编号不可重复，数据表头：[商品编号], [商品名称], [原零售价], [新零售价], [备注(30字内)]</div>
                  <form action="<?php echo $action_upload; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
                      <div class="form-group">
                          ...
                          <div class="col-sm-2">
                              <label class="control-label">22:30-次日01:59期间不能上传</label>
                              <?php
                                $now = (int)date("Hi", time()+8*3600);
                                if( ($now > '200'&& $now <'2230')){ ?>
                                <button type="submit" class="btn btn-primary">商品价格上传</button>
                              <?php } else{ ?>
                                <label class="control-label">[价格维护中...]</label>
                              <?php } ?>
                          </div>
                      </div>
                  </form>
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $.each(data, function(i, n){
                    ...
                    html += '  <td class="text-left">' + n.check_time + '</td>';
