...
<div id="content">
  ...
  <div class="container-fluid">
      <div class="alert alert-info">
          <i class="fa fa-exclamation-circle"></i> 例：链接接到主分类，在链接中填写(斜体部分，红色数字为分类编号)：<strong><i>javascript:showParentProducts(<span style="color: red">251</span>);</i></strong> <br />
          <i class="fa fa-exclamation-circle"></i> 例：链接到活动分类，在链接中填写(斜体部分，红色数字为分类编号)：<strong><i>javascript:showActivityProducts(<span style="color: red">1001</span>);</i></strong>
      </div>
      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-banner" class="form-horizontal">
          ...
          <!--<div class="form-group">
            ...
            <div class="col-sm-10">
              <select name="station_id" id="input-stations" class="form-control">
                <?php foreach($stationList as $station){ ?>
                <?php if($station['station_id'] == $station_id){ ?>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-3">
              <div class="input-group date">
                <input type="text" name="date_start" value="<?php echo $date_start; ?>" placeholder="<?php echo $entry_date_start; ?>"  id="input-date-start" class="form-control" />
          ...
          <div class="form-group">
            <label class="col-sm-2 control-label" for="input-model">平台仓库列表</label>
            <div class="col-sm-10">
            <?php foreach($stationList as $stations){ ?>
              <div class="row">
                <div class="col-md-2">
                  <div class="checkbox">
                    <label>
                      <input onclick="setCheckValue(this.value,this.checked);" class="parent_check<?php echo $stations['station_id']; ?>" type="checkbox"  name="station_warehouse[station][]" value="<?php echo $stations['station_id']; ?>" <?php if(in_array($stations['station_id'],$banner_stations)){ ?> checked = "checked" <?php } ?> ><?php echo $stations['name']; ?>
                    </label>
                  </div>
                </div>
                <div class="col-md-8">
                  <?php foreach($station_warehouse[$stations['station_id']] as $warehouses){ ?>
                  <div class="row">
                    <div class="checkbox">
                      <label>
                        <input onclick="setChildCheck(this.value,this.checked);" class="child_check<?php echo $stations['station_id']; ?>" type="checkbox" name="station_warehouse[warehouse][<?php echo $warehouses['warehouse_id']; ?>]" value="<?php echo $stations['station_id']; ?>" <?php if(in_array($warehouses['warehouse_id'],$banner_warehouses)){ ?> checked = "checked" <?php } ?> ><?php echo $warehouses['title']; ?>
                      </label>
                    </div>
                  </div>
                  <?php } ?>
...
function setCheckValue(data,check){
  $('.child_check'+data).prop("checked", check);

}
function setChildCheck(data,check){
  //取得所有child选中的数据,如果child的选中数为0，则parent为不勾选状态
  var num = $(".child_check"+data+":checked").length;
  if(num){
    $('.parent_check'+data).prop("checked", true);
  }else{
    $('.parent_check'+data).prop("checked", false);
  }
}
