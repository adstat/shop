            ...
            <table class="table table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  <td class="text-left"><?php if ($sort == 'r.product') { ?>
                    ...
                    <?php } else { ?>
                    <a href="<?php echo $sort_product; ?>">提交人</a>
                    <?php } ?></td>
                  <td class="text-left">
                      <a href="<?php echo $sort_model; ?>">退货金额</a>
                  </td>
                  <td class="text-left"><?php if ($sort == 'r.model') { ?>
                    <a href="<?php echo $sort_model; ?>" class="<?php echo strtolower($order); ?>"><?php echo $column_model; ?></a>
                    <?php } else { ?>
                    <a href="<?php echo $sort_model; ?>">退还余额</a>
                    <?php } ?></td>
                  
                  <td class="text-left">
                    <a href="<?php echo $sort_model; ?>">退货入库</a>
                  </td>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-left"><?php echo $return['username']; ?></td>

                  <td>
                      <?php echo $return['return_total']; ?>
                      <!--<?php echo '</br>';?>
                      <?php echo '件数：'.$return['return_real_quantity']; ?>-->
                  </td>
                  <td class="text-left"><?php echo $return['return_credits']; ?></td>
                  <td class="text-left"><?php echo $return['return_inventory_flag'] == 1 ? "是" : "否"; ?></td>
                  <td class="text-left">
                      <?php echo $return['status']; ?>
                      <?php echo '</br>'; ?>
                      <span class="conreturn_<?php echo $return['return_id']; ?>">
                      <?php if($return['credits_returned'] == 1){  ?>
                      <?php echo '已退余额'; ?>
                      <?php }else{ ?>
                      <?php echo '未退余额'; ?>
                      <?php } ?>
                  ...
                  <td class="text-right">
                      <?php if($return['credits_returned'] == 0 && $return['return_status_id'] == 2 && $return['return_credits'] > 0){ ?>
                      <span id="handle_credits_returned_<?php echo $return['return_id'];?>">
                      <button return_ope = "2" return_id="<?php echo $return['return_id'];?>"  data-loading-text="加载中..." class="btn btn-primary button-con-return" id="button-con-return-<?php echo $return['return_id'];?>"><i class="fa fa-plus-circle"></i> 退余额</button>

                      </span>
                      <?php } ?>

                      <?php if($return['return_status_id'] == 1){ ?>
                      <span id="handle_return_status_<?php echo $return['return_id'];?>">
                        <button return_ope = "2" return_id="<?php echo $return['return_id'];?>"  data-loading-text="加载中..." class="btn btn-primary button-sub-return" id="button-2-return-<?php echo $return['return_id'];?>"><i class="fa fa-plus-circle"></i> 确认</button>
                        <button return_ope = "3" return_id="<?php echo $return['return_id'];?>" data-loading-text="加载中..." class="btn btn-primary button-can-return"  id="button-3-return-<?php echo $return['return_id'];?>"><i class="fa fa-plus-circle"></i> 取消</button>
                      </span>
                      <?php } ?>
                      <a href="<?php echo $return['edit']; ?>" data-toggle="tooltip" title="查看" class="btn btn-primary"><i class="fa fa-eye"></i></a>
                  </td>
                </tr>
                <?php } ?>
                <?php } else { ?>
...
$('.button-con-return').on('click',function(){
    var return_id = $(this).attr("return_id");
    var return_ope = $(this).attr("return_ope");

    if(confirm("确认操作？")){
        $.ajax({
            url: 'index.php?route=sale/return/update_credits&token=<?php echo $token; ?>',
            type: 'post',
            data: {return_id : return_id, return_ope : return_ope},
            dataType: 'json',
            beforeSend: function() {
                $("#button-con" + "-return-" + return_id).button('loading');


            },
            complete: function() {

                $("#button-" + return_ope + "-return-" + return_id).button('reset');
            },
            success: function(json) {
                var jsonData = json;

                if(jsonData.success == 0){
                    alert("该退货信息不是仓库出库前提交,请刷新页面");
                    return false;
                }
                $("#handle_credits_returned_" + return_id).hide();
                $(".conreturn_"+return_id).text('已退余额');
            },
...
$('.button-sub-return,.button-can-return').on('click', function() {
    ...
    if(confirm("确认操作？")){
        $.ajax({
                url: 'index.php?route=sale/return/update_return&token=<?php echo $token; ?>',
                type: 'post',
                data: {return_id : return_id, return_ope : return_ope},
                dataType: 'json',
                beforeSend: function() {
                        $("#button-" + return_ope + "-return-" + return_id).button('loading');
                ...
                success: function(json) {
                        ...
                        if(jsonData.success == 0){
                            alert("不能重复确认或取消退货,请刷新页面");
                            return false;
                        }
                        $("#handle_return_status_" + return_id).hide();
                },			
                error: function(xhr, ajaxOptions, thrownError) {
                }
        });
