  ...
  <div class="container-fluid">
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-pencil"></i>采购数据导入</h3>
      </div>
      <div class="panel-body">
          <div class="alert alert-warning">上传指定日期的采购数据将覆盖现有日期全部数据，仅最后一次上传数据有效</div>
          <ul class="nav nav-tabs">
            <li class="active"><a href="#tab-general" data-toggle="tab">上传数据</a></li>
            <li><a href="#tab-list" data-toggle="tab" onclick="loadPurchaseList();">采购数据上传列表</a></li>
          </ul>
                  ...
                  <form action="<?php echo $action_upload; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
                      <div class="form-group">
                          ...
                          <div class="alert alert-info">
                              ...
                              <br />EXCEL数据要求7列，表头格式为：[原料ID], [原料名称],<strong>[蔬果毛重价]</strong>,[采购斤价],[采购斤数],[采购合计],[采购员]
                          </div>

                          <div class="col-sm-2">
                              <label class="control-label" style="font-size: 110%; ">采购入库日期</label>
            ...
            <div class="tab-pane" id="tab-list">
                <div class="table-responsive">
                    <table id="purchase-list" class="table table-striped table-bordered table-hover">
                        <thead>
                        <tr>
                            <td class="text-center">采购单编号</td>
                            <td class="text-left">采购数据日期</td>
...
<script type="text/javascript">
    ...
    function loadPurchaseList(){
        $.ajax({
            type: 'GET',
            url: 'index.php?route=purchase/purchase/purchaseList&token=<?php echo $_SESSION['token']; ?>',
            dataType: 'json',
    ...
    function loadPurchaseDetail(purchase_order_id){
        $.ajax({
            type: 'GET',
            url: 'index.php?route=purchase/purchase/getPurchaseDetail&token=<?php echo $_SESSION["token"]; ?>&purchase_order_id='+purchase_order_id,
            dataType: 'json',
            success: function(data){
                console.log(data);
                ...
                html += '<caption style="font-size: 16px;  text-align:center; font-weight: bold;">采购入库单#'+purchase_order_id+'</caption>';
                html += '   <thead>';
                html += '       <tr>';
                html += '           <td>原料编号</td>';
                html += '           <td>原料名称</td>';
                ...
                html += '</table>';
                html += "  <div style='text-align: right; font-size: 12px; font-weight: bold; margin: 0 0 30px 0;'>以上采购入库单#" + purchase_order_id;
                html += '   <button style="color:#df8505" onclick="$(\'.purchase_order_detail\').hide();">关闭</button>';
                html += "  </div>";
                html += "</div>";
