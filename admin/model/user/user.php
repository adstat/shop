...
class ModelUserUser extends Model {
	...
	public function editUser($user_id, $data) {
		...
		if ($data['password']) {
			...
			//$this->db->query("UPDATE `" . DB_PREFIX . "user` SET salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(md5(md5($salt).md5($data['password']))) . "' WHERE user_id = '" . (int)$user_id . "'");
		}
	}
	...
	public function editPassword($user_id, $password) {
		...
		//$this->db->query("UPDATE `" . DB_PREFIX . "user` SET salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(md5(md5($salt).md5($data['password']))) . "', code = '' WHERE user_id = '" . (int)$user_id . "'");
	}
	...
	public function getUsers($data = array()) {
		$sql = "SELECT A.*, B.user_group_id, B.name group_name FROM `" . DB_PREFIX . "user` A LEFT JOIN `" . DB_PREFIX . "user_group` B on A.user_group_id = B.user_group_id ";
