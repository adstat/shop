...
//		print_r($data['series_name']);die;
		//修改保存的制造商名下的品牌
		$this->db->query("DELETE FROM " . DB_PREFIX . "manufacturer_series WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");
		if (isset($data['series_name']))  {
			foreach($data['series_name'] as $series_name){
				$this->db->query("INSERT INTO " . DB_PREFIX . "manufacturer_series SET series_name = '" . $this->db->escape($series_name['series_name']) . "',manufacturer_id = '". (int)$manufacturer_id ."'");
			}
		}
	...
	public function getManufacturerSeries($manufacturer_id) {
		$sql = "select ms.manufacturer_id, ms.series_name name
			from oc_manufacturer_series ms
			left join oc_manufacturer m on m.manufacturer_id = ms.manufacturer_id
			where ms.manufacturer_id = '". $manufacturer_id ."'";
