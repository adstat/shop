<?php
class ModelCatalogProductInv extends Model {
    ...
    public function editProduct($product_id, $data) {
        ...
        $sql = "UPDATE " . DB_PREFIX . "product
            ...
            weight_range_most = '" . $this->db->escape($data['weight_range_most']) . "',
            storage_mode_id = " . (int)$data['storage_mode_id'] . ",
            inv_class = " . (int)$data['inv_class'] . ",
            produce_group_id = " . (int)$data['produce_group_id'] . ",
            scan_product = " . (int)$this->db->escape($data['scan_product']) . ",
            product_type_id = " . (int)$this->db->escape($data['product_type_id']) . "
...
//        $sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
        ...
        $sql = "select p.*,pd.* from oc_product p
              left join oc_product_description  pd ON (p.product_id = pd.product_id)
              left join oc_product_to_warehouse pw on pw.product_id = p.product_id
              WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";

        if(!empty($data['filter_warehouse_id_global']) && $data['filter_warehouse_id_global']){
            $sql .= " AND pw.warehouse_id = '".(int)$data['filter_warehouse_id_global']."'";
        }
        ...
        if (!empty($data['filter_model'])) {
            $sql .= " AND p.product_id =  '" . $this->db->escape($data['filter_model']) . "'";
        }
...
//        $sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
        $sql = "select COUNT(DISTINCT p.product_id) AS total from oc_product p
              left join oc_product_description  pd ON (p.product_id = pd.product_id)
              left join oc_product_to_warehouse pw on pw.product_id = p.product_id
              WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
...
//        $sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
    ...
    public function getProductInvClasses() {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_inv_class where product_inv_class_id != 5");

        return $query->rows;
    }
    
    public function getProduceGroups() {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "x_produce_group ");

        return $query->rows;
    }

    public function getProductInAllWarehouse($product_id){
        $sql = "select if(pw.warehouse_id is not null,pw.warehouse_id,0) warehouse_id,
				pw.sku_barcode,pw.retail_barcode,pw.barcode_list
				from oc_product p
				left join oc_product_to_warehouse pw on pw.product_id = p.product_id
        ...
        $warehouse_info = $query->rows;

        $warehouse_products = array();

        if(sizeof($warehouse_info)){
            foreach($warehouse_info as &$value){
                if($value['barcode_list'] != null){
                    $barcode_list = unserialize($value['barcode_list']);
                    $value['barcode_list'] = implode(',',$barcode_list);
                }
                $warehouse_products[$value['warehouse_id']] = array(
                    'warehouse_id' =>$value['warehouse_id'],
                    'barcode_list' =>$value['barcode_list'],
                    'sku_barcode' =>$value['sku_barcode'],
                );
            }
        }
    ...
    public function editProductBarcode($product_id,$data=array()){
        if(!empty($data['sku_barcode'])){
            //首先排查该编码有没有被其它商品占用,如果被占用则不允许保存Todo
...
//var_dump($serach);die;
            if(!$serach){
                ...
                if($data['warehouse'] != '*'){
                    $sql .=" ,warehouse_limit = '".$data['warehouse']."'";
                }
    ...
    public function getBarcodeHistory($product_id){
        $sql = "select ps.sku_barcode_id, ps.product_id,p.name,ps.sku_barcode,if(ps.box = 1,'是','否')box, if(ps.warehouse_limit > 0,w.title,'无')warehouse_limit
            from oc_product_sku_barcode ps
            left join oc_product p on p.product_id = ps.product_id
            left join oc_x_warehouse w on w.warehouse_id = ps.warehouse_limit
    ...
    public function updateBarcode($product_id,$sku_barcode_id,$data=array()){
        //首先排查该编码有没有被其它商品占用,如果被占用则不允许保存Todo
        $sql_q = "select count(sku_barcode_id) total from oc_product_sku_barcode where sku_barcode = '".$data['sku']."' and product_id <> ".$product_id;
        ...
        if(!$serach){
            $sql = "update oc_product_sku_barcode set
                ...
                box = '".(int)$data['box']."',
                warehouse_limit = '".(int)$data['warehouse']."'
                where sku_barcode_id = '".(int)$sku_barcode_id."'";
