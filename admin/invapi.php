...
define('APIURL', SITE_URI . '/api/xmlrpc/v1/index.php');
...
function rpcRequest($post){
    ...
    $request = array(
        ...
        'station_id' => isset($post['station_id']) ? $post['station_id'] : '',
    ...
    if($response->faultcode()==0) {
        ...
        if($post['method'] == 'inventory_login'){
    //            setcookie("inventory_user",$result['user']['username'],time()+2*3600+8*3600);
    //            setcookie("inventory_user_id",$result['user']['user_id'],time()+2*3600+8*3600);
    //            setcookie("user_group_id",$result['user']['user_group_id'],time()+2*3600+8*3600);
    //            setcookie("warehouse_id",$result['user']['warehouse_id'],time()+2*3600+8*3600);
...
if($method == 'getPurchaseOrders'){
    ...
    $data['date'] = $date;   
    $data['order_status_id'] = $_POST['order_status_id'];
    $data['purchase_order_id'] = isset($_POST['purchase_order_id']) ? $_POST['purchase_order_id'] : '';
...
if($method == 'getInventoryUserOrder'){
    ...
    $date = !empty($_POST['date']) ? $_POST['date'] : '';
    $data['date'] = $date;
    $data['order_status_id'] = !empty($_POST['order_status_id'])?$_POST['order_status_id']: '';
    ...
    if(!empty($orders['data'])){
        foreach($orders['data'] as $key=>$value){
            $inventory_orders[$value['order_id']][$value['product_type_id']] = $value['quantity'];
...
if($method == 'addPurchaseOrderProductStation'){
    ...
    if(date("H:i",time()+8*3600) > "21:30" && date("H:i",time()+8*3600) < "21:40" ){
...
if($method == 'addCheckProductToInv'){
    ...
    if(date("H", time()+8*3600) > 20){
        $data['date'] = date("Y-m-d", time()+8*3600+24*3600);
    }
    else{
        $data['date'] = date("Y-m-d", time()+8*3600);
    }
...
if($method == 'addReturnDeliverProduct'){
    ...
    $dataInv['timestamp'] = time()+8*60*60;
...
if($method == 'inventoryCheckSingleProduct'){
    ...
    $dataInv['date'] = date("Y-m-d", time()+8*3600);
...
if($method == 'getInventoryIn'){
    ...
    $data = array(
        'date' => $_POST['getdate'] ? $_POST['getdate'] : date("Y-m-d", time()+8*3600)
    );
    $planned = init($method, 0, json_encode($data), $station_id);
...
if($method == 'getAddedReturnDeliverProduct'){
   ...
   if(empty($_COOKIE['inventory_user_id'])){
    ...
    $data = array(
        'date' => isset($_POST['searchDate'])? date('Y-m-d', strtotime($_POST['searchDate'])) :date("Y-m-d", time()+8*3600),
        'add_user_name' => $_COOKIE['inventory_user_id'],
